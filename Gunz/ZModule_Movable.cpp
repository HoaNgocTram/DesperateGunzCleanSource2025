...
{
	// Custom: Moved GRAVITY_CONSTANT value to fConst
	float fConst = GRAVITY_CONSTANT;
	...
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_GRAVITY) && !ZGetGameClient()->GetMatchStageSetting()->IsQuestDrived()
		&& ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		int nGravityPercent = ZGetGameClient()->GetMatchStageSetting()->GetModifierValue(MMOD_GRAVITY);

		if (nGravityPercent < 0 || nGravityPercent > 300)
			nGravityPercent = 100; // Normal gravity (100 percent)

		fConst *= (nGravityPercent / 100.0f);
	}

	m_Velocity.z =
		max(m_Velocity.z - fConst * fDelta, -MAX_FALL_SPEED);
}
