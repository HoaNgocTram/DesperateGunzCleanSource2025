...
void ZCrossHair::DrawCrossHair(MDrawContext* pDC, MBitmap** ppBitmaps, MPOINT& center,
	float fSizeFactor, float fCAFactor, bool bPickRed)
{
	int sizex = 32;
	int sizey = 32;
	...
	if (ppBitmaps[CH_CENTER])
	{
		...
		MCOLOR oldColor = pDC->GetBitmapColor();
		if (bPickRed)
		{
			pDC->SetBitmapColor(ZGetApplication()->m_nMaxRange, oldColor.g, oldColor.b, oldColor.a);
		}
		pDC->Draw(center.x - sizex / 2, center.y - sizey / 2, sizex, sizey);
		pDC->SetBitmapColor(oldColor);
	}
...
void ZCrossHair::Draw(MDrawContext* pDC)
{
	...
	const float sizefactor = Z_ETC_CROSS_SIZE / 100.0f;
#else
	...
	case ZCS_NORMAL:
	{
		DrawCrossHair(pDC, m_pBitmaps, center, sizefactor, fFactor, false);
	}
	break;
	case ZCS_PICKENEMY:
	{
		if (m_pPickBitmaps[CH_CENTER] != NULL)
			DrawCrossHair(pDC, m_pPickBitmaps, center, sizefactor, fFactor, true);
		else DrawCrossHair(pDC, m_pBitmaps, center, sizefactor, fFactor, true);
	}
	break;
#else
	switch (m_nStatus)
...
#ifdef _CROSS_SIZE_BOX
	float sizefactor = Z_ETC_CROSS_SIZE / 100.0f;
#else
...
#ifdef _PICKOFF
	DrawCrossHair(pDC, pBitmaps, center, sizefactor, fFactor, false);
#else
