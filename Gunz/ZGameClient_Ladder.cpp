		...
		case MPROPOSAL_LADDER_INVITE:
			{
				...
				// 여기서 실제로 요청
				ZPostLadderRequestChallenge(ppMember, nCount, 0, 0, 0, 0, 0);
			}
			break;
		case MPROPOSAL_CLAN_INVITE:
		{
			...
			ZPostLadderRequestChallenge(ppMember, nCount, nBalancedMatching, AntiLead, GameType, nDodge, nRobase);
		}
		break;
		};
...
void ZGameClient::OnLadderLaunch(const MUID& uidStage, const char* pszMapName, bool LadderWar)
{
#ifdef _FLAHS
	RFlashWindow();
	RForceForeground(g_hWnd);
	...
	m_uidStage = uidStage;
	strcpy(m_szStageName, "Ladder Game Competitive");
