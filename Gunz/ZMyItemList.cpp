...
bool ZMyItemList::CheckTypeWithListFilter(int type, bool bEnchantItem)
{
	...
	else if (m_ListFilter == zequip_item_filter_cashitem) { return true; }

	return false;
}

// Custom: Sub-Filter
bool ZMyItemList::CheckTypeWithListSubFilter(int type)
{
	if( m_ListFilter == zequip_item_filter_all || m_ListSubFilter == 0 )
		return true;

	// Hard-coded shit y0
	if( m_ListFilter == zequip_item_filter_melee )
	{
	...
	else if( m_ListFilter == zequip_item_filter_range )
	{
	...
	else if( m_ListFilter == zequip_item_filter_custom )
	{
	...
	else if (m_ListFilter == zequip_item_filter_cashitem)
	{
		if (m_ListSubFilter == 1 && (type == MWT_DAGGER || type == MWT_DUAL_DAGGER))
			return true;
		else if (m_ListSubFilter == 2 && type == MWT_KATANA)
			...
			return true;
		else if (m_ListSubFilter == 5 && (type == MWT_SHOTGUN || type == MWT_SAWED_SHOTGUN))
			return true;
		else if (m_ListSubFilter == 6 && (type == MWT_SMG || type == MWT_SMGx2))
			return true;
		else if (m_ListSubFilter == 7 && (type == MWT_RIFLE || type == MWT_MACHINEGUN))
			return true;
		else if (m_ListSubFilter == 8 && (type == MWT_PISTOL || type == MWT_PISTOLx2))
			return true;
		else if (m_ListSubFilter == 9 && (type == MWT_REVOLVER || type == MWT_REVOLVERx2))
			return true;
		else if (m_ListSubFilter == 10 && type == MWT_ROCKET)
			return true;
		else if (m_ListSubFilter == 11 && type == MWT_SNIFER)
			return true;
		else if (m_ListSubFilter == 12 && (type == MWT_MED_KIT || type == MWT_REPAIR_KIT || type == MWT_BULLET_KIT || type == MWT_FOOD))
			return true;
	}
...
void ZMyItemList::SerializeZItemList()
{
	...
	for (int i = 0; i < (int)m_ItemIndexVector.size(); i++)
	{
		...
		if (itor != m_ItemMap.end())
		{
			...
			if (false == CheckTypeWithListSubFilter( pItemDesc->m_nWeaponType.Ref() )) continue;

			if (pEdit && strlen(pEdit->GetText()) > 0)
			{
				if (!StrStrI(pItemDesc->m_pMItemName->Ref().m_szItemName, pEdit->GetText()))
					continue;
			}

			// Custom: filter cash items.
			if (m_ListFilter == zequip_item_filter_cashitem && !pItemDesc->IsCashItem()) continue;
...
void ZMyItemList::SerializeQItemList()
{
	...
	for ( MQUESTITEMNODEMAP::iterator itor = m_QuestItemMap.begin();  itor != m_QuestItemMap.end();  itor++)
	{
		...
		//Don't show quest items when searching equipment.
...
void ZMyItemList::SetItemsAll(MTD_ItemNode* pItemNodes, const int nItemCount)
{
	...
	for (int i = 0; i < nItemCount; i++) {
		...
		if (bIsRentItem  == false) {		///< 일반 아이템			
			pNewItemNode->Create(pItemNodes[i].uidItem, pItemNodes[i].nItemID, pItemNodes[i].nCount, false, RENT_MINUTE_PERIOD_UNLIMITED, 0);
		} else {							///< 기간제 아이템
