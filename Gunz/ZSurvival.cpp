...
#ifndef _DEBUG
	strFileNameSurvivalMap += ""; // MEF NULL
#endif
...
bool ZSurvival::OnQuestGameInfo(MCommand* pCommand)
{
	...
	m_GameInfo.SetCurrSectorIndex(pQuestGameInfo->nSectorIndexID);
...
{
	MUID uidPlayer;
	unsigned long int nAddedXP = 0;
	unsigned long int nAddedBP = 0;
	unsigned long int nExpPercent = 0;
	pCommand->GetParameter(&uidPlayer, 0, MPT_UID);
	pCommand->GetParameter(&nAddedXP, 1, MPT_UINT);
	pCommand->GetParameter(&nAddedBP, 2, MPT_UINT);
	pCommand->GetParameter(&nExpPercent, 3, MPT_UINT);
	...
	if (ZGetCharacterManager()->Find(uidPlayer) == ZGetGame()->m_pMyCharacter)
	{
		...
		ZGetMyInfo()->SetXP(ZGetMyInfo()->GetXP() + nAddedXP);
		ZGetMyInfo()->SetLevelPercent(nExpPercent);
		ZGetScreenEffectManager()->SetGaugeExpFromMyInfo();
		ZGetMyInfo()->SetBP(ZGetMyInfo()->GetBP() + nAddedBP);
		ZGetScreenEffectManager()->AddExpEffect(nAddedXP);		// 획득 경험치 표시 없애기
		sprintf(szText, "You have won %d XP and %d BP for clearing the round.", nAddedXP, nAddedBP);
