...
void ZSoundEngine::PlaySEFire(MMatchItemDesc *pDesc, float x, float y, float z, bool bHero)
{
	...
	if( pDesc->m_nType == MMIT_RANGE || pDesc->m_nType == MMIT_CUSTOM  ) {
		...
		if(pDesc->m_nWeaponType.Ref() != MWT_SHOTGUN || pDesc->m_nID == 19930 /* Custom: Mayuri Shotgun sound is already szFireSndName, no 2d version */) {
		...
		if (bHero) {
			...
			if(ZGetConfiguration()->GetAudio()->b3DSound == false) {
...
{
	// Custom: MAIET didn't check if pDesc == NULL
	if(NULL!=pDesc && strcmp("rubber", pDesc->m_szWeaponByFiber) == 0)
	{
...
// Custom: 3d sound implementation
void ZSoundEngine::PlaySEFire(MMatchItemDesc *pDesc, float x, float y, float z, bool bPlayer)
{
	...
	if (pDesc->m_nType.Ref() == MMIT_RANGE || pDesc->m_nType.Ref() == MMIT_CUSTOM)
	{
		...
		if (bPlayer)
		{
			...
			if (ZGetGame() && ZGetConfiguration()->GetEtc()->bShotgunSound)
			{
			...
			if (ZGetConfiguration()->GetAudio()->b3DSound && pDesc->m_nID != 19930)
			{
				sprintf(szBuffer, "%s_optional_2d", szSndName);
			}
			else
...
{
	static char m_stSndFileName[MAX_BGM][64] = { "Intro.mp3",
												"Theme.mp3",
												"Game1.mp3",
												"Game2.mp3",
												"Game3.mp3",
...
bool ZSoundEngine::IsPlaying()
{
	return ZGetSoundFMod()->IsPlaying();
}
