...
{
#ifdef _QUEST
	if (ZGetGameTypeManager()->IsQuestDerived(GetMatchType()) || GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE) return;
#endif
	...
	if (!IsWaitForRoundEnd() && ZGetGame()->m_pMyCharacter)
	{
		...
		if (ZGetGame()->m_pMyCharacter->IsDie())
		{
			...
			if (m_nSoloSpawnTime >= RESPAWN_DELAYTIME_AFTER_DYING_MAX)	
			{
				SoloSpawn();
			}
...
void ZMatch::SoloSpawn()
{
	...
	if (GetMatchType() == MMATCH_GAMETYPE_DUEL || GetMatchType() == MMATCH_GAMETYPE_DUELTOURNAMENT || MyTeam == MMT_SPECTATOR) return;
#else
...
{
	//bool bSpecial = (GetMatchType() != MMATCH_GAMETYPE_INFECTED || GetMatchType() != MMATCH_GAMETYPE_SPY);
	if (IsTeamPlay() && (GetMatchType() != MMATCH_GAMETYPE_INFECTED || GetMatchType() != MMATCH_GAMETYPE_SPY))

	{
			...
			itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				{
					if ((m_pStageSetting->GetStageSetting()->bTeamAlternateSpawns) && GetCurrRound() > 0 && GetCurrRound() % 2)
					{
						if (i == 0)
							nSpawn = 1;
						else
	...
	else if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SKILLMAP)
	{
		...
		ZMapSpawnData* pSpawnData = ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloData(rand() % ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloCount());
	...
	else if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_TURBO)
	{
		ZMapSpawnData* pSpawnData = ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloData(rand() % ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloCount());
	...
	else if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_IS))
	{
		ZMapSpawnData* pSpawnData = ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloData(rand() % ZGetGame()->GetMapDesc()->GetSpawnManager()->GetSoloCount());
...
#ifdef _PICKOFF
	ZGetApplication()->m_nMinRange = RandomNumber(190, 198);
	ZGetApplication()->m_nMaxRange = RandomNumber(190, 215);
#endif
...
void ZMatch::OnForcedEntry(ZCharacter* pCharacter)
{
	...
	if (pCharacter == ZGetGame()->m_pMyCharacter)
	{
		...
		{
			// 팀플레이면 스팩테이터
			if (IsWaitForRoundEnd() || ZGetGameTypeManager()->IsQuestChallengeOnly(GetMatchType()) || ZGetGameTypeManager()->IsQuestDerived(GetMatchType()))
			{
				pCharacter->SetVisible(false);
				pCharacter->ForceDie();
	...
	else
	{
		...
		if (IsWaitForRoundEnd() && (GetRoundState() != MMATCH_ROUNDSTATE_FREE) ||
			ZGetGameTypeManager()->IsQuestChallengeOnly(GetMatchType()) || ZGetGameTypeManager()->IsQuestDerived(GetMatchType()))
		{
			if (pCharacter != NULL)
			{
				pCharacter->SetVisible(false);
...
DWORD ZMatch::GetMatchTime(void)
{
...
void ZMatch::GetTeamPlayers(int* pnRedTeam, int* pnBlueTeam, int* pnTotalPlayers) 
{
