...
//	MBitmap* GetBitmapIn() { return m_pBitmapIn; }
	...
	void AddPlayer(MUID& puid, ePlayerState state, int  nLevel,char* szName, char* szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade, int nPWPoints);

	// mode PLAYERLISTMODE_STAGE
	void AddPlayer(MUID& puid, MMatchObjectStageState state, int nLevel, char* szName, char* szClanName, unsigned int nClanID, bool isMaster, MMatchTeam nTeam, int duelTournamentGrade, int nPWPoints);
