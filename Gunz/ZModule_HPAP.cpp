...
{
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_NODAMAGE))
		return;
	...
	if (!ZGetGame()->m_pMyCharacter->IsObserverTarget() && !ZGetGame()->GetMatch()->IsQuestDrived() && !ZGetGame()->m_pMyCharacter->IsDie() && !ZGetGameClient()->IsDuelTournamentGame())
	{
		...
		{
#ifdef _BOXLEAD
			if (ZGetGameClient()->GetMatchStageSetting()->IsLead() == false || ZGetGameClient()->GetMatchStageSetting()->IsLead() == true && (ZDAMAGETYPE)Type != ZD_BULLET && (ZDAMAGETYPE)Type != ZD_BULLET_HEADSHOT)
#else
