	...
	case MC_MATCH_DUELTOURNAMENT_GAME_INFO:
		{
			...
			if (m_DTGameInfo.bIsRoundEnd)
			{
				...
				if ((m_DTGameInfo.uidPlayer1 == ZGetMyUID()) || (m_DTGameInfo.uidPlayer2 == ZGetMyUID())){
					...
					ZCharacter* cha = (ZCharacter*) ZGetCharacterManager()->Find(m_DTGameInfo.uidPlayer1);
					if (cha != NULL) cha->Revival();

					cha = (ZCharacter*) ZGetCharacterManager()->Find(m_DTGameInfo.uidPlayer2);
					if (cha != NULL) cha->Revival();
				...
				&& (m_DTGameInfo.uidPlayer2 != ZGetGame()->m_pMyCharacter->GetUID())) 
			{
				...
				ZCharacter *pCharacter = (ZCharacter*) ZGetCharacterManager()->Find(ZGetMyUID());
				if (pCharacter)
				{
	...
	case MC_PEER_OPENED:
		{
			...
			ZCharacter* pCharacter = (ZCharacter*) ZGetCharacterManager()->Find(uidChar);
			if (!pCharacter) break;
...
void ZRuleDuelTournament::SetMatchResultInfo(MTD_DuelTournamentMatchResultInfo& DTGameResultInfo)
{
	...
	for(int i=0; i<8; ++i)
	{
		if(m_QuaterFinalPlayer[i].uidPlayer == winnerPlayer.uidPlayer)
		{
			...
			m_QuaterFinalPlayer[i].nVictory = 0;
...
void ZRuleDuelTournament::SetMatchPlayerInfoUI(const MUID& uidPlayer1, const MUID& uidPlayer2)
{
	...
	ZCharacter* pChar1 = (ZCharacter*) ZGetCharacterManager()->Find(uidPlayer1);
	ZCharacter* pChar2 = (ZCharacter*) ZGetCharacterManager()->Find(uidPlayer2);
	if (pChar1) {
	...
	MMatchCharItemParts partsToShow[] = { MMCIP_MELEE, MMCIP_PRIMARY, MMCIP_SECONDARY, MMCIP_CUSTOM1, MMCIP_CUSTOM2, MMCIP_CUSTOM3, MMCIP_LONGBUFF1, MMCIP_LONGBUFF2, MMCIP_END };
...
#ifdef _SRVRPNG
			nPing = pPeerPingDT(pPeer->GetPing(ZGetGame()->GetTickTime()));
#endif
	}
	return nPing;
}
