...
bool ZSoundFMod::Create( HWND hwnd, FSOUND_OUTPUTTYPES type, int maxrate, int minchannels, int maxchannels, int maxsoftwarechannels, unsigned int flag )
{
	...
	for( int i = 0; i < nDriver; ++i )
	{
		...
		else
		{
			...
			if( maxsoftwarechannels < 16 ) return false; // ÇÏµå¿þ¾î·Î »ý¼ºÇß´Âµ¥ ÇÏµå¿þ¾î¸¦ Áö¿øÇÏÁö ¾Ê´Â Ä«µåÀÏ °æ¿ì..
		}
	}
	//#endif
	...
	if (!FSOUND_Init(maxrate, maxsoftwarechannels, flag)) // TODO : ¸¸¾à fx»ç¿îµå »ç¿ëÇÒ °æ¿ì ¸¶Áö¸· ÆÄ¶ó¹ÌÅÍ ¼ÂÆÃ
	{
		mlog("Init: %s\n", FMOD_ErrorString(FSOUND_GetError()));
		return false;
	}
...
//	mzf.ReadAll( Buffer, SoundFileLength );
	...
	FSOUND_SAMPLE* r = FSOUND_Sample_Load(FSOUND_FREE, Buffer,
		((Flag & ~FSOUND_HW3D) | FSOUND_LOADMEMORY | FSOUND_FORCEMONO), 0, SoundFileLength);
	if( r == 0 )
	{
		mlog("LoadWave: %s\n", FMOD_ErrorString(FSOUND_GetError()));
	}
...
void ZSoundFMod::PlayMusic( bool bLoop )
{
	...
	FSOUND_SetPriority( m_iMusicChannel, 255 );	// ¹ÂÁ÷ÀÌ playµÇ´Â Ã¤³ÎÀÇ priority´Â maximumÀÌ´Ù
	FSOUND_SetPaused( m_iMusicChannel, false );
}
