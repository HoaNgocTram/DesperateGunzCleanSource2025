...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pWeaponIcons[MWT_GREAT_SWORD]		= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_sword.elu"));//±◊≥… øÅE± ∞∞¿∫∞Õ ªÁøÅE
	m_pWeaponIcons[MWT_DOUBLE_KATANA]	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_blade.elu"));//±◊≥… øÅE± ∞∞¿∫∞Õ ªÁøÅE
...
void ZScreenEffectManager::DrawGauges()
{
	...
	else
	{
		...
		DrawGauge(80.f / 800.f, 27.f / 600.f, 150.f / 800.f, 13.f / 600.f, 0.f, color); // Capa

		// hp barra
		if (m_fGaugeHP == 1.0f)		color = D3DCOLOR_ARGB(255, 0, 140, 230);
		else if (m_fGaugeHP > 0.7f)	color = D3DCOLOR_ARGB(255, 69, 177, 186);
		else if (m_fGaugeHP > 0.3f)	color = D3DCOLOR_ARGB(255, 231, 220, 24);
		else						color = D3DCOLOR_ARGB(255, 233, 44, 22);
		DrawGauge(80.f / 800.f, 27.f / 600.f, min(1.f, m_fGaugeHP) * 150.f / 800.f, 13.f / 600.f, 0.f, color);

		// ap capa
		color = D3DCOLOR_ARGB(255, 80, 80, 80);
		DrawGauge(80.f / 800.f, 47.f / 600.f, 150.f / 800.f, 13.f / 600.f, 0.f, color); // Capa

		// ap barra
		color = D3DCOLOR_ARGB(255, 68, 193, 62); //Bar: NewAP
		DrawGauge(80.f / 800.f, 47.f / 600.f, min(1.f, m_fGaugeAP) * 150.f / 800.f, 13.f / 600.f, 0.f, color);

		// exp capa
		color = D3DCOLOR_ARGB(255, 80, 80, 80);
		DrawGauge(80.f / 800.f, 65.f / 600.f, 150.f / 800.f, 4.f / 600.f, 0.f, color); // Capa

		// exp
		color = D3DCOLOR_ARGB(255, 255, 255, 255);
		DrawGauge(80.f / 800.f, 65.f / 600.f, min(1.f, m_fGaugeEXP) * 150.f / 800.f, 4.f / 600.f, 0.f, color);
		...
		{
			color = 0x60ef0000;
			float x = min(1.f, m_fGaugeHP) * 150.f / 800.f;
			float w = (m_fCurGaugeHP - m_fGaugeHP) * 150.f / 800.f;
			DrawGauge(80.f / 800.f + x, 27.f / 600.f, w, 13.f / 600.f, 0.f, color);
		}

		if (render_cur_ap)
		{
			color = 0x60ef0000;
			float x = min(1.f, m_fGaugeAP) * 150.f / 800.f;
			float w = (m_fCurGaugeAP - m_fGaugeAP) * 150.f / 800.f;
			DrawGauge(80.f / 800.f + x, 47.f / 600.f, w, 13.f / 600.f, 0.f, color);
		}
...
void ZScreenEffectManager::Draw()
{
	...
	if ( !ZGetGame()->IsReplay() || ZGetGame()->IsShowReplayInfo())
	{
		if(ZGetCombatInterface()->IsShowUI())
			DrawEffects(); // ƒﬁ∫∏ ø°¥œ∏ﬁ¿Ãº« µÂ∑ŒøÅE
	...
	if(ZGetCombatInterface()->IsShowUI())
	{
		DrawQuestEffects(); // ƒ˘Ω∫∆ÆΩ√ K.O ¿ÃπÃ¡ÅE
		DrawDuelEffects();
		DrawTDMEffects();
		DrawCTFEffects();
	}
...
void ZScreenEffectManager::DrawMyBuffImage()
{
	...
	// TodoH(ªÅE - ¿⁄Ω≈¿« πˆ«¡ »ø∞ÅEæ∆¿Ãƒ‹¿ª ±◊∑¡æﬂ «’¥œ¥Ÿ. ∏Æº“Ω∫µµ « ø‰«’¥œ¥Ÿ.
}
...
void ZScreenEffectManager::UpdateEffects()
{
	...
	// Ω∫∆—¿Ã∏ÅEKO∂ÅE»≠ªÅE•¥¬ ±◊∏Æ¡ÅEæ ¥¬¥Ÿ.
	if (ZGetGameInterface()->GetCombatInterface())
	{ // ZGetGameInterface()->GetCombatInterface() <- NULL¿œ ∞ÊøÅE° µÈæ˚€√∂ß∞° ¿÷¿Ω
...
void ZScreenEffectManager::DrawCombo()
{
	...
	// ∞∞¿∫ ≈∏¿Ãπ÷ø° ªÁ∂Û¡ˆµµ∑œ «œ±ÅE¿ß«ÿ æ’¿« º˝¿⁄µÈµµ 
	// ∏∂¡ˆ∏∑ ¿⁄∏Æºˆ¿« «¡∑π¿”¿Ã∂ÅE∞∞¿∫ «¡∑π¿”¿∏∑Œ µπ∑¡≥ı¥¬¥Ÿ
...
void ZScreenEffectManager::SetCombo(int nCombo)
{
	...
	if (nCombo > MAX_COMBO) nCombo = MAX_COMBO;		// 99 ≥—¿∏∏ÅEø°∑Ø≥≠¥Ÿ
	...
	for(int i=0;i<nCount;i++)
	{
		...
		if(combonumbers[i]!=ncurrent || m_pComboEffects[i+2]==NULL)
		{
			...
			// ±◊ ¿⁄∏Æø° º˝¿⁄∞° ¿ÃπÃ ¿÷¿∏∏ÅE«¡∑π¿”¿ª µ⁄¬ ¿∏∑Œ µπ∑¡ ∞ÅEªÁ∂Û¡ˆµµ∑œ ∏∏µÁ¥Ÿ
...
{
#ifdef _BIRDSOUND
	ZGetSoundEngine()->PlaySound("if_score_flyby");		// æ∆¡ÅEµÙ∑π¿Ã ¿˚øÅE»µ«æÅE¿÷¿Ω 
#else
