...
#ifdef _PORTALGUN 1
void ZWeaponMgr::AddPortal(rvector& pos, rvector& dir, ZObject* pC, int nType)
{
	ZWeaponPortal* pWeapon = new ZWeaponPortal;

	RMesh* pMesh = ZGetWeaponMeshMgr()->Get("portal_gun_portal");
	if (!pMesh) return;

	pWeapon->Create(pMesh, pos, dir, pC, nType);
	Add(pWeapon);
}
#endif
...
// SPY MODE.
void ZWeaponMgr::AddSpyFlashBang(rvector& pos, rvector& velocity, ZObject* pC)
{
	ZWeaponSpyFlashBang* pWeapon = new ZWeaponSpyFlashBang;
...
void ZWeaponMgr::AddSpySmokeGrenade(rvector& pos, rvector& velocity, ZObject* pC)
{
	ZWeaponSpySmokeGrenade* pWeapon = new ZWeaponSpySmokeGrenade;
...
void ZWeaponMgr::AddSpyTrap(rvector& pos, rvector& velocity, int nItemId, ZObject* pC)
{
	ZWeaponSpyTrap* pWeapon = new ZWeaponSpyTrap;
...
void ZWeaponMgr::AddStunGrenade(rvector &pos, rvector &velocity, ZObject* pC)
{
	ZWeaponStunGrenade* pWeapon = new ZWeaponStunGrenade;

	RMesh* pMesh = ZGetWeaponMeshMgr()->Get("spy_stungrenade");
	if (!pMesh) return;
...
void ZWeaponMgr::AddNewQuestProjectile(const ZActorActionRangeShot* pRangeShot, const rvector &pos, const rvector &dir, ZObject* pOwner)
{
	ZWeaponNpcBullet* pWeapon = new ZWeaponNpcBullet;

	RMeshMgr* pMeshMgr = ZGetEffectManager()->GetEffectMeshMgr();
	if(!pMeshMgr) return;

	RMesh* pMesh = pMeshMgr->Get( pRangeShot->GetMeshName() );
	if(!pMesh) return;

	pWeapon->Create(pMesh, pRangeShot, pos, dir, 1.f, pOwner);
	Add(pWeapon);
}
...
ZMovingWeapon* ZWeaponMgr::UpdateWorldItem(int nItemID, rvector& pos)
{
	...
	for (node = m_list.begin(); node != m_list.end(); ) {
		...
		if (pMWeapon) {
			// pMWeapon->GetItemUID °ªÀÌ -1ÀÌ¸E»ý¼º¸¸ µÆ°E¾ÆÁE¼­¹ö¿¡¼­ ÇÒ´çµÈ °ª(UID)ÀÌ ¼¼ÆÃÀÌ ¾ÈµÆ±â¶§¹®¿¡
			// ¼¼ÆÃÀÌ ¾ÈµÈ°ÍµEÁß¿¡¼­ °¡ÀE°¡±ûÛE¾ÆÀÌÅÛÀ» Ã£¾Æ ³ªÀÇ°ÍÀ¸·Î ¼¼ÆÃÇØÁØ´Ù
			// ·£´ý°ªÀ» »ý¼ºÇØ ¼­¹ö¿¡ À¯ÀúID¸¦ ÇÔ²²º¸³»°E´Ù½Ã ¼­¹ö·Î ±×°ªÀ» ¹Þ¾Æ ¸ÅÄ¡µÇ´Â°É
			// ¿ùµE¾ÆÀÌÅÛÀ¸·Î ¼¼ÆÃÇØÁà¾ß ÇÏÁö¸¸.......20090218 by kammir
				...
				//			if(pMWeapon->m_nWorldItemID == nItemID) 
			{
				/*
								...
								// °¡ÀE°¡±ûÛE¾ÆÀÌÅÛÀÌ ÀÚ½ÅÀÇ°Í..³ªÁß¿¡ ¼­¹EÆÐÄ¡½Ã.. ID µûÜ» Áà¼­ ½Äº°..
				*/
...
void ZWeaponMgr::DeleteWeaponHasTarget( const MUID& uidTargetChar )
{
	...
	for(node = m_list.begin(); node != m_list.end(); ) {
		pWeapon = (*node);
		// Å¸°ÙÀ» °¡ÁE¼EÀÖ´Â ÅºÀº ÇöÀE¸¶¹ýÅº »ÓÀÌ´Ù
