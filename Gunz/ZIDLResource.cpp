...
ZLadderWarsRankingListBox* ZIDLResource::GetLadderWarsRankingListBox(MXmlElement& element)
{
	MXmlElement childElement;
	char szBuf[4096];
	//	char szAttr[4096];
	...
	ZLadderWarsRankingListBox* pWidget = new ZLadderWarsRankingListBox("", pParentWidget, pParentWidget);
...
ZItemSlotView* ZIDLResource::GetItemSlot(MXmlElement& element)
{
	...
	else if (!stricmp(szParts, "custom3"))		pWidget->SetParts(MMCIP_CUSTOM3);
...
void ZIDLResource::Parse(MXmlElement& element)
{
	...
	else if (!strcmp(szTagName, "LADDERWARSRANKINGLIST"))
	{
		GetLadderWarsRankingListBox(element);
	}
