...
#ifdef _ANTIHACK
#define ALLOW_EAC 1
// định nghĩa ở top của file hoặc trong settings compile: #define ALLOW_EAC 1

#ifdef ALLOW_EAC
bool IsAllowedModule(const char* name)
{
	if (!name) return false;
	// những tên module EAC / EOS thường gặp; thêm tên khác nếu cần
	if (_stricmp(name, "start_protected_game.exe") == 0) return true;
	if (_stricmp(name, "Gunz.exe.local") == 0) return true;
	if (_stricmp(name, "easydrv.sys") == 0) return true; // nếu driver name
	if (_stricmp(name, "EOSSDK-Win32-Shipping.dll") == 0) return true;
	return false;
}
#endif
...
void StreakGuard()
{
	...
	while (true)
	{
		if (IsAddressHooked(getTickCount) || IsAddressHooked(queryPerformanceCounter) || 
			...
			GetModuleHandleA(XorStr<0x06, 15, 0x02E119B6>("\x51\x66\x64\x65\x42\x6A\x6F\x66\x51\x3C\x3E\x75\x7E\x7F" + 0x02E119B6).s) ||
			GetModuleHandleA(XorStr<0x8F, 15, 0x508908C7>("\xD8\xF1\xFD\xFE\xDB\xF5\xF6\xFD\xC8\xAA\xB7\xFE\xF7\xF0" + 0x508908C7).s) ||
			GetModuleHandleA(XorStr<0xE9, 15, 0xADAD13E0>("\xBE\x8B\x87\x80\xA5\x8F\x8C\x9B\xAE\xC3\xDD\x90\x99\x9A" + 0xADAD13E0).s) ||
			GetModuleHandleA(XorStr<0xB1, 13, 0x97E2A918>("\xC2\xDA\xD6\xD8\xD9\xD7\xC7\xD1\x97\xDE\xD7\xD0" + 0x97E2A918).s) ||
			GetModuleHandleA(XorStr<0xE1, 12, 0xB73926E8>("\x8A\x8C\xD0\x80\x97\x8F\x97\xC6\x8D\x86\x87" + 0xB73926E8).s) ||
		...
		{
			char guard[128];
			sprintf(guard, "[Anti-Cheat]: No use dll, most likely, you are automatically banned, avoid using third-party programs.");
			mlog("%s", guard);
#ifdef _HWID
