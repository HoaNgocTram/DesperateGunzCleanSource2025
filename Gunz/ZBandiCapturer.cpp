...
void ZBandiCapturer::OnDrawCapture(MDrawContext* pDC)
{
	...
	if (m_bandiCaptureLibrary.GetCaptureTime() - m_fToggleFileSize > 1000)	
	{ 
...
void ZBandiCapturer::GetFolder(TCHAR* pPath)
{
	...
	if (GetMyDocumentsPath(pPath))
	{
		...
		strcat(foldername, _NAMEAPPFOLDER_BANDICAM);
...
#define SCREENSHOT_DELAY		2000
	...
	// 🔒 Chặn chụp screenshot nếu không phải trong game
	...
	ZTransMsg(szOutput, MSG_SCREENSHOT_SAVED, 1);
