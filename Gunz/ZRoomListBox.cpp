...
//		//Custom: put option to hide non full rooms
//		if (m_pMapInfo[i].nPeople == m_pMapInfo[i].nMaxPeople) 
//		{ 
//			sprintf(szBuf, "Full");
//		}
...
//			sprintf(szBuf, "Solo");
//		}
//		else if (m_pMapInfo[i].nMaxPeople % m_pMapInfo[i].nPeople != 0) //Bug: #1
//		{
//			sprintf(szBuf, "Even");
//		}
//		else 
...
//		}
//#ifdef _SPEC 1
//	if (m_pMapInfo[i].bObserverEnabled == false)
//	{
// /*       #define FN_ROOMLIST_NO_SPEC "spec_mode_off.png"
//		pBitmap = MBitmapManager::Get(FN_ROOMLIST_NO_SPEC);
//
//		if (pBitmap != 0)
//		{
...
//	else
//	{
// /*       #define FN_ROOMLIST_SPEC "spec_mode_on.png"
//		pBitmap = MBitmapManager::Get(FN_ROOMLIST_SPEC);
//
//		if (pBitmap != 0)
//		{
//			r.x = width + m_RoomWidth * 0.82 + pressed_reposition;
			...
			case MWM_LBUTTONDBLCLK:
			{
				for( int i = 0 ; i < NUM_DISPLAY_ROOM; ++i )
				{
					...
					if( rect.InPoint( ClickPos ) )
					{
						...
						ZPOSTCMD2(MC_MATCH_REQUEST_LOBBYSTAGE_INFO, MCmdParamUID(pSelRoomInfo->uidStage), MCmdParamUID(ZGetMyUID()));
#endif
...
#ifdef _SPEC 1
	m_pMapInfo[nIndex].bObserverEnabled = pRoomInfo->bObserverEnabled;
#endif
...
void ZRoomListBox::RequestSelStageJoin()
{
	...
	{
		// Custom: Event Team
		if ( (ZGetMyInfo()->IsAdminGrade() == false) && ZGetMyInfo()->GetUGradeID() != MMUG_GAMEMASTER && (m_pMapInfo[m_Selection].bPrivate) )
		{
