...
bool ZObserver::OnKeyEvent(bool bCtrl, char nKey)
{
	...
	if (bCtrl) {
		...
		ZCharacter* pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(uidTarget);
...
void ZObserver::DrawPlayerDuelHPAPBar(MDrawContext* pDC)
{
	...
	for (ZCharacterManager::iterator itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		{
			strcpy(charName[0], pCharacter->GetCharName());
		...
		{
			strcpy(charName[1], pCharacter->GetCharName());
		...
		{
			strcpy(charName[2], pCharacter->GetCharName());
	...
	pDC->FillRectangleW((193.0f + 163.0f) * fRx - nWidth, fPosy, nWidth, fHeight);

	nWidth = (int)((float)fHP[1] / fMaxHP[1] * fLength);
	pDC->FillRectangleW(444.0f * fRx, fPosy, nWidth, fHeight);


	// AP
	pDC->SetColor(0, 50, 0, 128);
	pDC->FillRectangleW(218.0f * fRx, 37.0f * fRy, 150.0f * fRx, 6.0f * fRy);
	pDC->FillRectangleW(432.0f * fRx, 37.0f * fRy, 150.0f * fRx, 6.0f * fRy);

	pDC->SetColor(0, 180, 0, 200);
	nWidth = (int)((float)fAP[0] / fMaxAP[0] * 150.0f * fRx);
	pDC->FillRectangleW((218.0f + 150.0f) * fRx - nWidth, 37.0f * fRy, nWidth, 5.0f * fRy);
...
void ZObserver::OnDraw(MDrawContext* pDC)
{
	...
	if (ZGetMyInfo()->IsAdminGrade())
	{
		...
		char szName[128];
		sprintf(szName, "%s (HP:%d, AP:%d)", m_pTargetCharacter->GetCharName(), (int)m_pTargetCharacter->GetHP(), (int)m_pTargetCharacter->GetAP());
...
{
	ZCharacter* pCharacter = NULL;
	pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(muid);
	if (pCharacter)
	{
		SetTarget(pCharacter);
	}
