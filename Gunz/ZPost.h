...
#ifdef _ICONCHAT
inline void ZPostPeerChatVoiceIcon(bool bIconVoice)
{
	ZPOSTCMD1(MC_PEER_CHATVOICE_ICON, MCmdParamBool(bIconVoice));
}
#endif
...
#endif
#ifdef _HWID
inline void ZPostLogin(char* szUserID, char* szPassword, unsigned int nChecksumPack, char *szEncryptMD5Value, char* szHwid)
{
	...
	ZPOSTCMD6(MC_MATCH_LOGIN, MCmdParamStr(szUserID), 
		MCmdParamStr(szPassword), 
		MCmdParamInt(MCOMMAND_VERSION), 
		MCmdParamUInt(nChecksumPack), 
		MCommandParameterBlob(pBlob, MGetBlobArraySize(pBlob)), MCmdParamStr(szHwid));
...
#endif
#ifdef _HWID
inline void ZPostBanAccount(char* pszTargetName)
{
	ZPOSTCMD1(MC_MATCH_BANACCOUNT, MCmdParamStr(pszTargetName));
}
#endif
...
#ifdef _CMD_ALL
inline void ZPostAllCmd(const char* CMD1, const char* CMD2, const char* CMD3, MUID uidChar = MUID(0, 0))
{
	ZPOSTCMD4(MC_ALL_CMD, MCmdParamStr(CMD1), MCmdParamStr(CMD2), MCmdParamStr(CMD3), MCmdParamUID(uidChar));
}
#endif
...
#ifdef _NOLEAD
inline void ZPostNoLead(const MUID& uidAttacker, const MUID& uidVictim, rvector& pos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	MCommand* pCmd = new MCommand(ZGetGameClient()->GetCommandManager()->GetCommandDescByID(MC_PEER_NOLEAD), ZGetGameClient()->GetServerUID(), MUID(0, 1));
	...
	MSafeUDP* pSafeUDP = ZGetGameClient()->GetSafeUDP();
	ZGetGameClient()->SendCommandByUDP(pCmd, ZGetGameClient()->GetServerIP(), ZGetGameClient()->GetServerPeerPort());

	delete pCmd;
	//ZPOSTCMD7(MC_PEER_NOLEAD, MCommandParameterUID(uidVictim), MCommandParameterPos(pos.x, pos.y, pos.z), MCommandParameterInt(damageType), MCommandParameterInt(weaponType), MCommandParameterFloat(fDamage), MCommandParameterFloat(fPiercingRatio), MCommandParameterInt(nMeleeType) );
}

inline void ZPostNoLeadBlob(const MUID uidSender, void* pBlobArray)
{
	MCommand* pCmd = new MCommand(ZGetGameClient()->GetCommandManager()->GetCommandDescByID(MC_PEER_NOLEADBLOB), ZGetGameClient()->GetServerUID(), MUID(0, 1));
...
inline void ZPostLadderWars(const MUID& uidAttacker, float fDamage)
{
	ZPOSTCMD2(MC_PEER_LADDERWARS, MCommandParameterUID(uidAttacker), MCommandParameterFloat(fDamage));
}
...
#ifdef _HWID
inline void ZPostBanMe(const WarpHackType nType)
{
	void* pBlobArray = MMakeBlobArray(sizeof(MTD_KeyParams), 0);
	ZPOSTCMD2(MC_MATCH_BANME, MCommandParameterInt(nType), MCommandParameterBlob(pBlobArray, MGetBlobArraySize(pBlobArray)));
	MEraseBlobArray(pBlobArray);
}
inline void ZPostBanMe(const WarpHackType nType, std::list<MTD_KeyParams*> keyList)
{
	void* pBlobArray = MMakeBlobArray(sizeof(MTD_KeyParams), keyList.size());
	int j = 0;
	for (std::list<MTD_KeyParams*>::iterator i = keyList.begin(); i != keyList.end(); i++)
	{
		MTD_KeyParams* pKeyParam = (MTD_KeyParams*)MGetBlobArrayElement(pBlobArray, j);
		CopyMemory(pKeyParam, (*i), sizeof(MTD_KeyParams));
		j++;
	}
...
#endif
inline void ZPostAdminSBTest(const MUID& uidMe, char* szTargetName)
{
	ZPOSTCMD2(MC_ADMIN_SBTEST, MCommandParameterUID(uidMe), MCommandParameterString(szTargetName));
}
...
// ÀÚ»EäÃ» - Á÷Á¢È£ÃâÇÏÁö¸»°EZGameClient::RequestGameSuicide() ¸¦ È£ÃâÇÏµµ·Ï ÇÏÀÚ.
...
#ifdef _PORTALGUN 1
inline void ZPostRequestSpawnGamePortal(const MUID& uidChar, const int nItemID, const rvector& pos, const rvector& dir)
{
	ZPOSTCMD4(MC_MATCH_REQUEST_SPAWN_GAMEPORTAL, MCommandParameterUID(uidChar),
		MCommandParameterInt(nItemID), MCommandParameterPos(pos.x, pos.y, pos.z), MCommandParameterPos(dir.x, dir.y, dir.z));
}
inline void ZPostRequestMoveToGamePortal(const int nItemUID)
{
	ZPOSTCMD1(MC_REQUEST_MOVETO_GAMEPORTAL, MCommandParameterInt(nItemUID));
}
#endif
...
#endif
#ifdef _CW_VOTE
inline void ZPostLadderWarsVote(int Map)
{
	ZPOSTCMD1(MC_MATCH_LADDERWARS_VOTE, MCmdParamInt(Map));
}
#endif // _CW_VOTE
inline void ZPostJoinLadderWars(int Arg1, int Arg2)
{
	ZPOSTCMD2(MC_MATCH_JOIN_LADDERWARS, MCmdParamInt(Arg1), MCmdParamInt(Arg2));
}
...
inline void ZPostInviteClanWar()
{
	ZPOSTCMD0(MC_INVITE_CLANWAR);
}
...
// ´Ù¸¥»ç¶÷ÀÇ µ¿ÀÇ ¿äÃ» - ÀÌÇÔ¼ö´Â Á÷Á¢»ç¿EÏÁö¸»°EZGameClient::RequestProposal¸¦ ÀÌ¿EØ¾ßÇÑ´Ù.
...
// µ¿ÀÇ ÀÀ´E- ÀÌÇÔ¼ö´Â Á÷Á¢»ç¿EÏÁö¸»°EZGameClient::ReplyAgreement¸¦ ÀÌ¿EØ¾ßÇÑ´Ù.
...
// ·¡´õ°ÔÀÓ ½ÅÃ»
inline void ZPostLadderRequestChallenge(char** ppMemberCharNames, const int nMemberCount, unsigned long int nOptions, unsigned long int nAntiLead, unsigned long int nGameType, unsigned long int bDodge, unsigned long int bRobase)
{
	...
	ZPOSTCMD7(MC_MATCH_LADDER_REQUEST_CHALLENGE, 
	MCmdParamInt(nMemberCount), 
	MCmdParamUInt(nOptions),
	MCmdParamBlob(pBlobMembersName, MGetBlobArraySize(pBlobMembersName)), 
	MCmdParamUInt(nAntiLead), 
	MCmdParamUInt(nGameType),
	MCmdParamUInt(bDodge),
	MCmdParamUInt(bRobase));


	MEraseBlobArray(pBlobMembersName);
}

inline void ZPostLadderCancel(bool LadderWar)
{
	ZPOSTCMD1(MC_MATCH_LADDER_REQUEST_CANCEL_CHALLENGE, MCmdParamBool(LadderWar));
}
#ifdef _LADDERWARSPACKETS
inline void ZPostLadderWarsInvite(char* name)
{
	ZPOSTCMD1(MC_MATCH_LADDERWARS_FRIENDINVITE, MCmdParamStr(name));
}
#endif
inline void ZPostLadderWarsAccept()
{
	ZPOSTCMD0(MC_MATCH_LADDERWARS_FRIENDACCEPT);
}

inline void ZPostLadderWarsLeave()
{
	ZPOSTCMD0(MC_MATCH_LADDERWARS_FRIENDLEAVE);
}
...
{
	static char szCharName[MATCHOBJECT_NAME_LENGTH] = { -1, 0, };	// ZGetMyInfo()->GetCharName()°¡ ""¸¦ ¸®ÅÏÇÒ ¼EÀÖÀ¸¹Ç·Î ÀÌº¯¼ö¸¦ ""·Î ÃÊ±âÈ­ÇÏ¸E¿ÀÀÛµ¿ ÀÏ¾ûÏ²
	static DWORD timeLastReqeust = 0;
	DWORD currTime = timeGetTime();

	// ³Ê¹« ÀÚÁÖ ¿äÃ»ÇÏÁE¾Êµµ·Ï Á¦ÇÑ
	// Ä³¸¯ÅÍ º¯°æÀ» ÇÑ °æ¿EÂ ±×³É ¿äÃ»
...
#ifdef _ANTISPAM
inline void ZPostAdminWall(const MUID& uidChar, char* szMsg, const char* szChar)
{
	ZPOSTCMD3(MC_ADMIN_WALL, MCommandParameterUID(uidChar), MCommandParameterString(szMsg), MCommandParameterString(szChar));
}
#endif
