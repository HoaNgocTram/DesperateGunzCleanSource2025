...
bool ZWorldItem::ApplyWorldItem( ZCharacter* pCharacter )
{
	...
	// Custom: [EX] disables mines.
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_EXPLOSION) && m_fAmount < 0)
		return true;
	...
	case WIT_MINE:
	{
		rvector vDir = pCharacter->GetDirection();
		if (ZGetGame()->m_pMyCharacter != NULL)
		{
			if (ZGetGame()->m_pMyCharacter == pCharacter)
			{

				ZGetGame()->m_pMyCharacter->ReserveDashAttacked(ZGetGame()->m_pMyCharacter->GetUID(), ZGetGame()->GetTime(), vDir);
				ZGetGame()->m_pMyCharacter->SetVelocity(0.f, 0.f, 0.f);
			}

			float fRatio = ZItem::GetPiercingRatio(MWT_DYNAMITYE, eq_parts_legs);
			pCharacter->OnDamaged(pCharacter, GetPosition(), ZD_EXPLOSION, MWT_DYNAMITYE, m_fAmount, fRatio);

			ZGetSoundEngine()->PlaySound("we_grenade_explosion", GetPosition());
			ZGetEffectManager()->AddDynamiteEffect(GetPosition(), vDir);
		}
	}
	break;
	...
	/*case WIT_MINE:
	{
		ZGetGame()->OnExplosionDynamite(OwnerUID, m_Position, m_fAmount, 280.f, 1.f, Team);
	...
	case WIT_WEAPON:
	{
		MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(m_nItemID);

		if (pDesc == pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->GetDesc())
		{
			currentBullet = pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->GetBulletCurrMagazine();		// 현재 착용한 탄창안의 총알수
			currentMagazine = pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->GetBulletSpare();				// 현재 착용하지 않은 탄창안의 총알수

			inc = pDesc->m_nMagazine.Ref();		// 총알 증가분
			max = pDesc->m_nMaxBullet.Ref();		// 전체 채워질수 있는 총알의 갯수
			...
			if (currentBullet < inc)
			{
				pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->SetBulletCurrMagazine(maxBullet);				// 현재 탄창의 총알수 채우기			
				--nBullet;
			}

			if (nBullet > 0)
				pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->SetBulletSpare(min((nBullet*inc + currentMagazine), maxMagazine));
		}
		else if (pDesc == pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->GetDesc())
		{
			currentBullet = pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->GetBulletCurrMagazine();		// 현재 착용한 탄창안의 총알수
			currentMagazine = pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->GetBulletSpare();				// 현재 착용하지 않은 탄창안의 총알수
			...
			if (currentBullet < inc)
			{
				pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->SetBulletCurrMagazine(maxBullet);				// 현재 탄창의 총알수 채우기			
				--nBullet;
			}

			if (nBullet > 0)
				pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->SetBulletSpare(min((nBullet*inc + currentMagazine), maxMagazine));
		}
		else
		{
			if (pCharacter->m_Items.GetSelectedWeaponType() == MMCIP_PRIMARY)
			{

				pCharacter->m_Items.EquipItem(MMCIP_PRIMARY, m_nItemID, 1);
				pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->SetBulletCurrMagazine(pDesc->m_nMagazine.Ref());
				pCharacter->m_Items.GetItem(MMCIP_PRIMARY)->SetBulletSpare(pDesc->m_nMaxBullet.Ref() - pDesc->m_nMagazine.Ref());
				pCharacter->ChangeWeapon(MMCIP_MELEE);
				pCharacter->ChangeWeapon(MMCIP_PRIMARY);
			}
			else if (pCharacter->m_Items.GetSelectedWeaponType() == MMCIP_SECONDARY)
			{
				pCharacter->m_Items.EquipItem(MMCIP_SECONDARY, m_nItemID, 1);
				pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->SetBulletCurrMagazine(pDesc->m_nMagazine.Ref());
				pCharacter->m_Items.GetItem(MMCIP_SECONDARY)->SetBulletSpare(pDesc->m_nMaxBullet.Ref() - pDesc->m_nMagazine.Ref());
				pCharacter->ChangeWeapon(MMCIP_MELEE);
...
bool ZWorldItemManager::ApplyWorldItem( WIL_Iterator& iter, ZCharacter* pCharacter )
{
	...
	switch(pWorldItem->GetType())
	{
		...
		case WIT_QUEST :
			...
			break;
		case WIT_AP :
			ZGetEffectManager()->AddRepairEffect( pCharacter->GetPosition(), pCharacter );
			break;
		case WIT_BULLET :
		case WIT_WEAPON :
			...
			break;
		case WIT_MINE:
			// Spy Landmine : just explosion, do nothing.
			break;
	}
...
#ifdef _PORTALGUN
		if (pDesc->m_nItemType == WIT_PORTAL) {
			pWorldItem->CreateVisualMesh();
		}
#endif
...
void ZWorldItemManager::update()
{
	...
	for( WIL_Iterator iter = mItemList.begin(); iter != mItemList.end();)
	{
		...
		{
			if (pItem->GetItemID() == MSPYMODE_WORLDITEM_LANDMINE_ID && pCharacter->GetTeamID() != MMT_ALL) // Custom: Event Channel
			{
				++iter;
				continue;
			}
...
void ZWorldItemManager::OnOptainWorldItem(ZWorldItem* pItem)
{
	...
	{
		ZCharacter* pCharacter = ZGetGame()->m_pMyCharacter;
		if (ZGetGame()->GetMatch() && pCharacter != NULL)
		{
		if (pItem->GetType() == WIT_MINE)
		{
			/*if(pCharacter->GetUID().Low == pItem->OwnerUID.Low) return;
			if (ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_SPY)
			{
				if (ZGetGame()->GetMatch()->IsTeamPlay() && pItem->Team == pCharacter->GetTeamID()) return;
			}*/
		}
#ifdef _MAGICBOX
		if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DROPMAGIC)
		{
			if (pItem->GetType() == WIT_QUEST)
			{
				if (GetAsyncKeyState('E') & 0x8000)
				{
					unsigned int nowTime = timeGetTime();
					if (nowTime > pItem->m_dwBoxObtainTime + 1 * 3 * 1000)
					{
						pItem->m_dwBoxObtainTime = nowTime;
						ZPostDropGunGameRequestItem(pCharacter->GetUID());
					}
					return;
				}
				return;
		...
		{
			if(pCharacter->GetTeamID() == MMT_RED && pItem->GetType() == WIT_FLAG && pItem->GetItemID() == CTF_RED_ITEM_ID)
				return;
		    else if	(pCharacter->GetTeamID() == MMT_BLUE && pItem->GetType() == WIT_FLAG && pItem->GetItemID() == CTF_BLUE_ITEM_ID)
				return;
		}
		else if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CTF && pCharacter->IsTagger())
		{
			unsigned int nowTime = timeGetTime();
				
			// capturing flag + worlditem pickup fix
			if (pCharacter->GetTeamID() == MMT_RED && pItem->GetItemID() == CTF_RED_ITEM_ID || pCharacter->GetTeamID() == MMT_BLUE && pItem->GetItemID() == CTF_BLUE_ITEM_ID)
			{
				if (nowTime > pItem->m_dwFlagObtainTime + 1000)
				{
					pItem->m_dwFlagObtainTime = nowTime;
					ZPostRequestCaptureFlag(pItem->GetItemID());
				}
				return;
			}
		}
...
#ifdef _PORTALGUN
	if (pItem->GetType() == WIT_PORTAL)
	{
		ZMyCharacter* pCharacter = ZGetGame()->m_pMyCharacter;
		if (ZGetGame()->GetMatch() && pCharacter != NULL)
		{
			unsigned int nowTime = timeGetTime();
			if (nowTime > pCharacter->m_fLastPortalTime + 1000)
			{
				pCharacter->m_fLastPortalTime = nowTime;
				ZPostRequestMoveToGamePortal(pItem->GetID());
			}
		}
		return;
	}
	...
	if( !CheckBitSet(pItem->GetSpawnTypeFlags(), WORLD_ITEM_STAND_ALINE) )
	{
		...
		if ((int)ZGetCharacterManager()->size() >= 4 && ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_CTF)
		{
			...
			{
				ZCharacter* pCharacter = (ZCharacter*)ZGetCharacterManager()->Find((*itor).first);
