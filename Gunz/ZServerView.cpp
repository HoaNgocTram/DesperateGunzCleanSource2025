...
: MWidget(szName, pParent, pListener)
{
	...
	m_nSelBoxSizeX = 270;
	m_nSelBoxSizeY = 17;
	// Custom: Extended server list node width since I prefer selecting the server from the end of the bar.
	m_nSelBoxSizeW = 290; //220;

	// Custom: Disable server list ping bars

#if defined(LOCALE_NHNUSA) || defined(_LOGINPING)
...
void ZServerView::PingImageDraw(MDrawContext* pDC, MRECT rectBox, int nImageCount, bool bIsLive)
{
	...
	const int nMoveImage = int(210 * MGetWorkspaceWidth() / 800.f);
	pDC->Draw(rectBox.x + nMoveImage, rectBox.y, 22, 14);
#endif
#ifdef  _DEBUG
	char strPing[16];
	if(0 < m_dAgentPing[nImageCount] && m_dAgentPing[nImageCount] < 1000000 )
	{
		...
		rectBox.w = 230; // Custom: 210 (original)
