...
#ifdef _HITSCOUNT
	nShots = 0, nHits = 0;
	ZGetGame()->m_pMyCharacter->m_nShots = 0;
	ZGetGame()->m_pMyCharacter->m_nHits = 0;
#endif
	ZGetGame()->m_pMyCharacter->m_nLastShot = 0;
#ifdef _FLAHS
	RFlashWindow();
...
void ZMyCharacter::ProcessInput(float fDelta)
{
	...
	//Custom: No movement in the rounds
	...
	ZCharacter* pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(GetUID());
	...
	{	// blast fall ªÛ≈¬¿œ∂ß¥¬ ¡°«¡≈∞ ¿‘∑¬¿Ã ∞°¥…«œ¥Ÿ.
		...
		*/
			...
			!zStatus.m_bSlash && !zStatus.m_bJumpSlash && !zStatus.m_bJumpSlashLanding)
		{
...
#ifdef _PAINTMODE
				if (zStatus.m_bWallJumpQueued && DotProduct(GetVelocity(), m_Direction) > 0 && GetVelocity().z > -10.f && !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_FPS) && !ZGetGame()->GetMatch()->IsPaintBall())
				{
			...
			if (ZGetGame()->GetMatch()->IsTraining() && GetRGMain().TrainingSettings.Ninja)
			{
				bJump2 = true;
			}

			//Custom: Only work in channel event option 2
			if (strstr(ZGetGameClient()->GetChannelName(), "Event"))
			{
				if (ZGetGame()->m_pMyCharacter->GetStatus().Ref().isNinja == 1)
					bJump2 = true;
			}
...
#ifdef _PAINTMODE
						if (!ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_FPS) && !ZGetGame()->GetMatch()->IsPaintBall())
						{
...
void ZMyCharacter::OnShotRocket()
{
	...
	if (pSelectedItem->GetDesc()->m_nWeaponType.Ref() == MWT_ROCKET)
	{
		ZPostShotSp(vWeaponPos, dir, ZC_WEAPON_SP_ROCKET, sel_type);
	}
	if (pSelectedItem->GetDesc()->m_nWeaponType.Ref() == MWT_PORTAL_GUN)
	{
		ZPostShotSp(vWeaponPos, dir, ZC_WEAPON_SP_PORTAL_GUN, sel_type);
	}
}
#endif
/*
...
#ifdef _PORTALGUN 1
void ZMyCharacter::OnGadget_PortalGun()
{
	ZItem* pSelectedItem = GetItems()->GetSelectedWeapon();
	...
	if (pSelectedItem->GetDesc()->m_nWeaponType.Ref() == MWT_PORTAL_GUN)
	{
		ZPostShotSp(vWeaponPos, dir, ZC_WEAPON_SP_PORTAL_GUN_RED, sel_type);
	}
}
#endif
...
#define GUARD_TIME	0.1f
	...
	if (bBothPressed)
	{
		if (!zStatus.m_bGuard && bGuardTime)
		{
			...
			if (strstr(ZGetGameClient()->GetChannelName(), "Newbie"))
			{
				if (zStatus.m_bShot || 
					zStatus.m_bJumpShot || 
					zStatus.m_bSkill)
...
#endif
			if (ZGetGameClient()->GetMatchStageSetting()->GetStageSetting()->bNokStyle == true)
			{
				if (zStatus.m_bShot ||
					zStatus.m_bJumpShot ||
					zStatus.m_bSkill)
...
void ZMyCharacter::ProcessShot()
{
	...
	*/
#ifdef _ROCKETGUIDED
	if (GetItems()->GetSelectedWeapon()->GetDesc()->m_nID == 910051)
	{
		if (!GetItems()->GetSelectedWeapon()->Shot()) 
			return;
...
#ifdef _PAINTMODE
	if (ZGetGame()->GetMatch()->IsPaintBall() && !IsDie() && !ZGetGame()->IsReservedSuicide())
	{
		rvector dir = rvector(GetVelocity().x, GetVelocity().y, 0);
		Normalize(dir);
...
void ZMyCharacter::OnTumble(int nDir)
{
	...
	//TCustom: No movement in the rounds
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SKILLMAP &&
		ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_COUNTDOWN ||
		ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SKILLMAP &&
		ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_COUNTDOWN)
...
void ZMyCharacter::OnDamagedAnimation(ZObject* pAttacker, int type)
{
	...
	if (ZGetGame()->GetMatch()->IsTraining() && GetRGMain().TrainingSettings.NoStuns)
		return;
...
void ZMyCharacter::EnterCharge()
{
	...
	if (ZGetGame()->GetMatch()->IsTraining() && GetRGMain().TrainingSettings.NoMass){
		zStatus.m_bEnterCharge = false;
		return;
	}
