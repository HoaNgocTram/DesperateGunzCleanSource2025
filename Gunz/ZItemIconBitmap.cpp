...
MBitmap* GetItemIconBitmap(MMatchItemDesc* pItemDesc)
{
	...
	case MMIST_CUSTOM:
		{
			...
			//case MWT_ENCHANT_STARSHOOTER: strcpy(szFileName, "slot_icon_en_starshooter"); break;
			...
			case MWT_SPYCASE:			strcpy(szFileName, "icon_spy_spycase");		break;
			case MWT_TRAP_SPY:			strcpy(szFileName, GetItemIconBitmap_Trap(pItemDesc));		break;
			case MWT_STUN_GRENADE_SPY:			strcpy(szFileName, "icon_spy_stungrenade");		break;
			case MWT_FLASH_BANG_SPY:			strcpy(szFileName, "slot_icon_flashbang");		break;
			case MWT_LANDMINE_SPY:			strcpy(szFileName, "icon_spy_landmine");		break;
			case MWT_SMOKE_GRENADE_SPY:			strcpy(szFileName, "slot_icon_flashbang");		break;
	...
	case MMIST_FINGER:
		{
			...
			else if (pItemDesc->IsMedalItem())
			{
				strcpy(szFileName, "slot_icon_ringG");	break;
			}
			else if (pItemDesc->IsEventItem())
			{
				strcpy(szFileName, "slot_icon_ringG");	break;
			}
			else 
...
#ifdef _PORTALGUN 1
				case MWT_PORTAL_GUN:	strcpy(szFileName, "slot_icon_rocket"); break;
#endif
				}
			}
		}
	...
	case MMIST_AVATAR:	strcpy(szFileName, "slot_icon_avatar");	break;
...
#else
	strcat(szFileName, ".tga");	//Custom: tga to png.
#endif
...
MBitmap* GetItemThumbnailBitmap(MMatchItemDesc* pDesc)
{
	...
	sprintf(szFilePath, "interface/default/IconItems/itemicon%d.dds", pDesc->m_nID);
