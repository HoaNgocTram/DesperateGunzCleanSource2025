...
//		fShotTime-=pOwnerCharacter->m_fTimeOffset;
		...
		if (pDesc)
		{
			...
			{
			case 515000:
				ZGetEffectManager()->AddEffectWaveFine(pOwnerCharacter->GetUID(), waveCenter, pOwnerCharacter->GetDirection());
				break;
			case 515001:
				ZGetEffectManager()->AddEffectWaveFireBall(pOwnerCharacter->GetUID(), waveCenter, pOwnerCharacter->GetDirection());
				break;
			case 515002:
				ZGetEffectManager()->AddEffectWaveIceMissile(pOwnerCharacter->GetUID(), waveCenter, pOwnerCharacter->GetDirection());
				break;
			case 515003:
				ZGetEffectManager()->AddEffectWaveMagic(pOwnerCharacter->GetUID(), waveCenter, pOwnerCharacter->GetDirection());
				break;
			default:
#ifdef _MACOLOR
			...
			itor != ZGetObjectManager()->end(); ++itor)
		{
			...
			{
				ZGetEffectManager()->AddPaintEffect(tpos, -fTarDir);
			}
#endif
		...
		itor != ZGetObjectManager()->end(); ++itor)
	{
		...
		if (fDist < 200.0f) {
			...
			if ((pTar) && (pTar != pOwnerCharacter))
			{
				...
				else if (ZGetGame()->GetMatch()->IsQuestDrived() || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
				{
				...
				if( bCheck) {//팀이아닌경우만
					...
					if (fDot>0)
					{
						...
						*/
						...
						// Custom: No Flip Room Mod
						if( !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_FLIP ) && !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_GLAD))
						{
...
void ZGameAction::OnPeerSkill_Dash(ZCharacter *pOwnerCharacter)
{
	...
	// Spy Case : suppress the skill range.
...
//						ZGetEffectManager()->AddSlashEffect( tpos , -fTarDir , cm );
						...
						// Custom: [F] now prevents dagger lunge, damage is still sent though
						if( !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed( MMOD_FLIP ) && !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_GLAD))
						{	
							if (pTar == ZGetGame()->m_pMyCharacter) {
	...
	if(pDesc)
	{
		switch(pOwnerCharacter->GetEnchantType())
		{
			...
			case ZC_ENCHANT_LIGHTNING : 
				...
				break;
	/*		case ZC_ENCHANT_STARSHOOTER:
				ApplyStartShooterEnchantDamage(pTarget, pOwnerCharacter, pDesc->m_nDamage.Ref(), pDesc->m_nDelay.Ref());
				break;*/
		};
	}
