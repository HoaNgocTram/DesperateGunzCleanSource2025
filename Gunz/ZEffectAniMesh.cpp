...
bool ZEffectAniMesh::Draw(u64 nTime)
{
	...
	if (m_bDelay && !m_isCheck) {
		...
		if (pCObj) {
			...
			/*
			int cm = g_pGame->SelectSlashEffectMotion(pObj);//³²³à Ä® ÈÖµÎ¸£´Â ¹æÇâ
			if(cm==SEM_ManSlash5) {
				_vdir.z = 0.f;
				_vp += _vdir * 120.f;
				_vp.z -= 100.f;
	...
	if (m_nLifeTime != -1 && m_bLoopType) {// ¼Ò¸ê ½Ã°£ÀÌ Á¤ÇØÁ® ÀÖ°í..¿¡´Ï¸ÞÀÌ¼ÇÀÌ ·çÇÁ Å¸ÀÙÀÎ °æ¿ì..
...
ZEffectBerserkerIconKILoop::ZEffectBerserkerIconKILoop(RMesh* pMesh, ZObject* pObj)
	: ZEffectIcon(pMesh, pObj)
{
	m_nElapsedTime = 0;
	AniFrameInfo* pInfo = m_VMesh.GetFrameInfo(ani_mode_lower);
...
bool ZEffectBerserkerIconKILoop::Draw(unsigned long int nTime)
{
	ZObject* pObj = ZGetObjectManager()->GetObject(m_uid);
	ZCharacter* pChar = MDynamicCast(ZCharacter, pObj);
