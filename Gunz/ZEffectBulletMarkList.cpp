...
#else
void ZEffectBulletMarkList::Update(float fElapsed) {
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_PAINT)) {
#ifdef _PAINTMODE
...
bool ZEffectBulletMarkList::Draw()
{
	...
	while(dwRemainNum)
	{
		...
		DWORD dwThisNum = min( dwRemainNum , BULLETMARK_FLUSH_COUNT );			// °¹¼ö°¡ BULLETMARK_FLUSH_COUNT ¸¦ ³Ñ¾î°¡¸é BULLETMARK_FLUSH_COUNT ¾¿ Âï´Â´Ù

		dwThisNum = min( dwThisNum , EFFECTBASE_DISCARD_COUNT - m_dwBase );	// ¹öÆÛÀÇ Å©±â¸¦ ³Ñ¾î°¡¸é °³¼ö¸¦ ÁÙ¿©¼­ Å©±â¸¸Å­¸¸ Âï´Â´Ù
