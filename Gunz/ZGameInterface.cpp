...
// Custom: Shop list anti-serialize (no scroll)
bool g_bReceivedShopItemList = false;
//GunzState g_nCurrentShopState = GUNZ_LOBBY;
...
{
	// szSubDir(´E¼°æ·Î)¿¡¼­ ¸ÕÀEÆÄÀÏÀ» Ã£°E¾øÀ¸¸EszDir¿¡¼­ ·ÎµùÇÑ´Ù
	// (´E¼°æ·Î¿¡´Â »ç¿EÚ°¡ ¿É¼Ç¿¡¼­ ¼±ÅÃÇÑ ¾ð¾ûÛEºñÆ®¸ÊÀ» ³Ö¾ûÑÐ´Ù)
	mlog("start log bipmap\n");
	// Custom: Allow jpg images to be loaded for the interface
	const char *loadExts[] = { ".png", ".bmp", ".tga",".jpg",".dds"};
...
#endif
					// ´E¼ °æ·Î¿¡ °°Àº ÀÌ¸§ÀÇ ÆÄÀÏÀÌ Á¸ÀçÇÏ¸E±×°ÍÀ» ·ÎµE ¾øÀ¸¸EµðÆúÆ® °æ·Î¿¡¼­ ·ÎµE
...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	SetBandiCaptureConfig(ZGetConfiguration()->GetMovingPicture()->iResolution);	// µ¿¿µ»EÄ¸ÃÄ ÇØ»óµµ ¼¼ÆÃ
	SetBandiCaptureFileSize(ZGetConfiguration()->GetMovingPicture()->iFileSize);		// µ¿¿µ»EÄ¸ÃÄ ÆÄÀÏÅ©±E¿E®Á¦ÇÑ)

	bool bRet = true;
	char szPath[256];
	char szSubPath[256] = "interface/grm/default/";	// ¾ð¾ûÖ¯°æÀ» À§ÇÑ ´E¼ °æ·Î
	char szFileName[256];
...
//		pShopEquipListBox->SetOnDropCallback(CharacterEquipmentItemListBoxOnDrop);
	...
	// ¾ÆÀÌÅÛ ¼³¸EtextareaÀÇ Ä¿½ºÅÒ ·èÀ» ÁöÁ¤
...
#ifdef _FASTDEBUG
	//m_IDLResource.FindWidget("LoginPassword")->SetText("1111"); // fix?
...
bool ZGameInterface::InitInterfaceListener()
{
	...
	SetListenerWidget("VoiceVolumeSlider", ZGetVoiceVolumeSizeSliderListener());
	...
	//SetListenerWidget("WindowChangeRestart", ZGetWindowChangeRestartListener());
	//SetListenerWidget("WindowChangeCancel", ZGetWindowChangeCancelListener());
	...
	// Custom: Added Login Option Frame Listener
	SetListenerWidget("LoginOptionFrame", ZGetOptionFrameButtonListener());
	// Custom: Added Lobby Replay Frame Listener
	SetListenerWidget("LoginReplayFrame", ZGetReplayCallerButtonListener());
	// Custom: Added Login Register Frame Listener
	SetListenerWidget("RegisterFrame", ZGetRegisterListener());
...
//	SetListenerWidget("StagePlayerListTabFriend", ZGetListenerGamePlayerListTabFriend());
...
#ifdef _LADDERUPDATE 
	SetListenerWidget("CharUpdateCaller", ZGetCharSelectionCallerButtonListener());
#endif

	SetListenerWidget("RejoinCW", ZGetLadderRejoinConfirmListener());

	SetListenerWidget("LadderWarsDialogOk", ZGetLadderWarsDialogOkListener());
	SetListenerWidget("LadderWarsDialogClose", ZGetLadderWarsDialogCloseListener());

#ifdef _CW_VOTE
	SetListenerWidget("ArrangedLadderWars", ZGetArrangedLadderWarsListener());
	SetListenerWidget("PlayerVoteMapButton0", ZGetLadderWarsVote0());
	SetListenerWidget("PlayerVoteMapButton1", ZGetLadderWarsVote1());
	SetListenerWidget("PlayerVoteMapButton2", ZGetLadderWarsVote2());
#endif // _CW_VOTE
	...
	SetListenerWidget("InviteCW", ZGetInviteWarListener());
	...
	SetListenerWidget("ChannelList_LadderCustom", ZGetChannelList());
...
#ifdef _BOXLEAD
	SetListenerWidget("Lead", ZGetStageSettingChangedSettingListener());
#endif
	...
	SetListenerWidget("StageMaxPlayer", ZGetStageSettingChangedComboboxListener());			// µ¿È¯ÀÌ°¡ Ãß°¡ : ÃÖ´EÎ¿E
	SetListenerWidget("StageRoundCount", ZGetStageSettingChangedComboboxListener());		// µ¿È¯ÀÌ°¡ Ãß°¡ : °æ±âÈ½¼E
	SetListenerWidget("StageLimitTime", ZGetStageSettingChangedComboboxListener());			// µ¿È¯ÀÌ°¡ Ãß°¡ : Á¦ÇÑ½Ã°£
	SetListenerWidget("StageIntrude", ZGetStageSettingChangedComboboxListener());			// µ¿È¯ÀÌ°¡ Ãß°¡ : ³­ÀÔÇã¿E
	...
	SetListenerWidget("StageObserverBtn", ZGetStageObserverBtnListener());					// µ¿È¯ÀÌ°¡ Ãß°¡ : °EEÃ¼Å©
...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	SetListenerWidget("MedalShopCaller", ZGetMedalShopCallerButtonListener());
	...
	SetListenerWidget("Shop_to_MedalShop", ZGetMedalShopCallerButtonListener() );
	SetListenerWidget("Shop_to_EventShop", ZGetEventShopCallerButtonListener());

	// Custom: Sub-Filter
	SetListenerWidget("Shop_AllEquipmentSubFilter", ZGetShopListSubFilterListener());
	SetListenerWidget("Equip_AllEquipmentSubFilter", ZGetEquipListSubFilterListener());
	

	// Custom: Shop Search
	SetListenerWidget("Shop_Search", ZGetShopSearchListener());
	SetListenerWidget("Equip_Search", ZGetEquipSearchListener());
	...
	SetListenerWidget("Shop_MiscEquipmentTab", ZShopMiscEquipmentTabOpen());
	...
	SetListenerWidget("Equipment_to_MedalShop", ZGetEquipmentShopCallerButtonListener());		
	SetListenerWidget("Equipment_to_EventShop", ZGetEquipmentShopCallerButtonListener());
	...
	SetListenerWidget("Equip_MiscEquipmentTab", ZEquipMiscEquipmentTabOpen());
	...
	SetListenerWidget("CE_Sex", ZChangeCreateCharInfoListener());
	SetListenerWidget("CE_Hair", ZChangeCreateCharInfoListener());
	SetListenerWidget("CE_Face", ZChangeCreateCharInfoListener());
	...
	SetListenerWidget("KickPlayer_ConfirmOKButton", ZGetKickPlayerOKButtonListener());
	SetListenerWidget("KickPlayer_ConfirmCancelButton", ZGetKickPlayerCancelButtonListener());
	...
	SetListenerWidget("Replay_Delete",				ZGetReplayDeleteButtonListener());
	...
	// ¼±ÅÃµÈ Ä³¸¯ÅÍ Ä«¸Þ¶ó·Î µ¹¸®¸Eº¸±E

	// ¸®½º³Ê ¾÷µ¥ÀÌÆ®µÈ ÅÇÄÜÆ®·Ñ °»½ÅÇØÁÖ±E
	MTabCtrl *pTab = (MTabCtrl*)m_IDLResource.FindWidget("PlayerListControl");
	if( pTab ) pTab->UpdateListeners();

	GetRGMain().SetListeners(); //Custom: Training SetListerns
...
// ÆÄÃ÷±³È¯°E¹«±âÀåºñÀÇ ÀÓ½Ã µ¥ÀÌÅÍ µE...

static bool g_parts[10];	// Ä³¸¯ÅÍ ÀåºEºÎÂEÅ×½ºÆ®¿EÀÓ½Ã º¯¼E
...
/// OnCommand¸¦ ZGameInterface_OnCommand.cpp·Î ¿È±E 
...
bool ZGameInterface::OnGameCreate(void)
{
	...
	/*if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_BLITZKRIEG)
	{
		...
		{
		case (0):
			strcpy(szFileName, "Interface/default/loading/loading_Blue01.jpg");
			break;
		case (1):
			strcpy(szFileName, "Interface/default/loading/loading_Blue02.jpg");
			break;
		case (2):
			strcpy(szFileName, "Interface/default/loading/loading_Red01.jpg");
			break;
		case (3):
			strcpy(szFileName, "Interface/default/loading/loading_Red02.jpg");
			break;
		}
	}*/
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_SPY)
	{
		strcpy(szFileName, "Interface/default/loading/loading_SpyMode.jpg");
	}
	else if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST)
	{
		strcpy(szFileName, "Interface/default/loading/loading_1.jpg");
	}
	else if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		strcpy(szFileName, "Interface/default/loading/loading_teen.jpg");
	}
	else
	{
		switch (rand() % 9)
	...
	*/
	...
	if ((ZApplication::GetInstance()->GetLaunchMode() == ZApplication::ZLAUNCH_MODE_STANDALONE_REPLAY) ||
		(ZGetGameClient()->IsCWChannel()) || // Custom: Exploit change weapons in equipament
		...
		ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SKILLMAP)
	{
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_OPTION, true);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_CLOSE, true);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_BATTLE_EXIT, false);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_STAGE_EXIT, true);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_PROG_EXIT, true);
	}
...
{
	mlog( "game interface destroy begin.\n" ); // Custom: Fixed spelling mistake cause Stephan complained. '-' Maiet doe.
	MPicture* pPicture;
...
void ZGameInterface::OnGreeterCreate(void)
{
	...
	if (m_pBackground)
	{
		if (ZGetConfiguration()->GetEtc()->bInstantLogin)
...
void ZGameInterface::OnLoginCreate(void)
{
	...
	// Custom: Don't initialize m_pLoginBG
	m_pLoginBG = NULL; //new MBitmapR2;
	bool bRead = false;

	// ¿ÜºÎ ÆÄÀÏÀ» ÀÐ´Â´Ù.
#ifdef _DEBUG
	// Custom: Don't load loginbg.png
	//bRead = m_pLoginBG->Create( "loginbg.png", RGetDevice(), "wallpaper.jpg", false);
#else
	MZFile::SetReadMode(MZIPREADFLAG_ZIP | MZIPREADFLAG_MRS | MZIPREADFLAG_MRS2 | MZIPREADFLAG_FILE);
	// Custom: Don't load loginbg.png
	...
	// ÀÐ¾ûÛÂ ºñÆ®¸Ê ÀÌ¹ÌÁEÆ÷ÀÎÅÍ¸¦ ÇØ´EÀ§Á¬¿¡ ³Ñ°ÜÁà¼­ Ç¥½ÃÇÑ´Ù
	//if ( bRead && m_pLoginBG)
	//{
	//	MPicture* pPicture = (MPicture*)m_IDLResource.FindWidget( "Login_BackgrdImg");
	//	// Custom: Don't set login BG (use Map instead)
	//	if ( pPicture)
	//		pPicture->SetBitmap( m_pLoginBG->GetSourceBitmap());
	...
	// Custom: Force set Login_BackgrdImg to NULL
	MPicture* pPicture = (MPicture*)m_IDLResource.FindWidget("Login_BackgrdImg");
	if (pPicture)
...
#if	defined(_DEVELOPER) || defined(_DEBUG)//
		ShowWidget("LabelServerIP", true);
		ShowWidget("ServerAddress", true);
		ShowWidget("ServerPort", true);

		pServerList->AddServer("Debug Server", "localhost", 6000, 7, 0, 1000, true);			// Debug server
#endif
	...
	// ¾ÕÀ¸·Î ¸ðµEÆÛºúÔ®¼Å´Â ¼­¹ö¸®½ºÆ® È­¸éÀ» º¸¿©ÁØ´Ù´Â °¡Á¤À¸·Î ÁÖ¼®Ã³¸®
	//#ifdef LOCALE_NHNUSA
	//	m_IDLResource.FindWidget( "LoginID")->SetText( "GUNZ");
	//	m_IDLResource.FindWidget( "LoginPassword")->SetText( "1234567890");
	//#elif LOCALE_JAPAN
	...
	*/
	...
	// IJJI	
		// ·Î±×ÀÎ ¹öÆ°À» È°¼ºÈ­ ÇÏ±âÀ§ÇÑ ¶«»§¿EID, PassWord
	//	m_IDLResource.FindWidget( "LoginID")->SetText( "GUNZ");
	//	m_IDLResource.FindWidget( "LoginPassword")->SetText( "1234567890");
#endif
//#endif
...
void ZGameInterface::OnLoginDestroy(void)
{
	...
	if(pWidget)
	{
		// ·Î±äÀÌ ¼º°øÇÏ¸Ewrite ÇØ¾ß ÇÏ³ª.. Áö±Ý check out °Eè·Î ¿©±â´Ù -_-;
	...
	if ( m_pLoginBG != NULL)
	{
		// ¹è°EÀÌ¹ÌÁö¸¦ º¸¿©ÁÖ´Â À§Á¬ÀÇ ºñÆ®¸Ê ÀÌ¹ÌÁEÆ÷ÀÎÅÍ¸¦ ¸®¼ÂÇÑ´Ù
		MPicture* pPicture = (MPicture*)m_IDLResource.FindWidget( "Login_BackgrdImg");
		if ( pPicture)
			pPicture->SetBitmap( NULL);
	...
	if ( m_pLoginPanel != NULL)
	{
		// ÆÐ³Î ÀÌ¹ÌÁö¸¦ º¸¿©ÁÖ´Â À§Á¬ÀÇ ºñÆ®¸Ê ÀÌ¹ÌÁEÆ÷ÀÎÅÍ¸¦ ¸®¼ÂÇÑ´Ù
		MPicture* pPicture = (MPicture*)m_IDLResource.FindWidget( "Login_Panel");
		if ( pPicture)
			pPicture->SetBitmap( NULL);
...
void ZGameInterface::OnLobbyCreate(void)
{
	...
	// ¸®ÇÃ·¹ÀÌ ÈÄ¿¡ ¹Ù²ELevelPercent°ªÀ» ¿ø·¡´EÎ º¹¿øÇÑ´Ù
	if ( m_bOnEndOfReplay)
	{
		m_bOnEndOfReplay = false;
	...
	ZGetGameClient()->RequestOnLobbyCreated();
	 ...
	 //Custom: Staff no join Competitive mode.
	if (ZGetMyInfo()->IsAdminGrade())
		ShowWidget("ArrangedLadderWars", false);
	else
		ShowWidget("ArrangedLadderWars", true);

	if (ZGetMyInfo()->IsAdminGrade())
		ShowWidget("ArrangedTeamGame", false);
	else
		ShowWidget("ArrangedTeamGame", true);
	...
	//if (ZGetMyInfo()->IsAdminGrade())
	//	ShowWidget("ExtraStageSettingFrame", true);
	//else
	//	ShowWidget("ExtraStageSettingFrame", false);
	...
	*/
	...
	((MBitmapR2*)m_pClanInfoBg)->Create("claninfo_panel.png", RGetDevice(), "interface/loadable/claninfo_panel.png");
	if (m_pClanInfoBg != NULL)
	{
	...
	((MBitmapR2*)m_pDuelTournamentInfoBg)->Create("dueltournamentinfo_panel.png", RGetDevice(), "interface/loadable/dueltournamentinfo_panel.png");
	if (m_pDuelTournamentInfoBg != NULL)
	{
	...
	ZLadderWarsRankingListBox* pBZRankingListBox = (ZLadderWarsRankingListBox*)m_IDLResource.FindWidget("Lobby_LadderWarsRankingList");
	if (pBZRankingListBox) pBZRankingListBox->LoadInterfaceImgs();
#else
	MPicture* pPicture = 0;
	pPicture = (MPicture*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("Lobby_StripBottom");
	if (pPicture != NULL)	pPicture->SetAnimation(0, 1000.0f);
...
//	InitLadderUI();
	...
	pWidget = m_IDLResource.FindWidget("RejoinCW");
	if (pWidget)		pWidget->Enable(bEnable);
	...
	//	{
	//		std::vector<string> MeshesCustom(20);
	//		ZMyItemList* Itemz = ZGetMyInfo()->GetItemList();
	//		string FilePath = "";
	...
	//		{
	//			if (Itemz->GetEquipedItem((MMatchCharItemParts)(j)) != nullptr)
	//			{
	//				const unsigned long itemID = Itemz->GetEquipedItem((MMatchCharItemParts)j)->GetItemID();
	//				MMatchItemDesc* item = MGetMatchItemDescMgr()->GetItemDesc(itemID);
	//				if (item != nullptr)
	//				{
	//					if (IsArmor((MMatchCharItemParts)j))
	//					{
	//						MeshesCustom.push_back(FilePath + item->GetDyn());
	//					}
	//				}
	//			}
	//		}

	//		RMesh* Male = ZGetMeshMgr()->Get("heroman1");
	//		if (Male != nullptr)
	//		{
	//			for (auto& mesh : Male->m_parts_mgr->m_list)
	//			{
	//				if (mesh->GetIsModelAutoLoad() == false)
	//				{
	//					if (std::find(MeshesCustom.begin(), MeshesCustom.end(), mesh->GetFileName()) == MeshesCustom.end())
	//						Male->m_parts_mgr->Del(mesh);
	//				}
	//			}
	//		}
	//		MeshesCustom.clear();

	//		RMesh* Female = ZGetMeshMgr()->Get("herowoman1");
	//		if (Female != nullptr)
	//		{
	//			for (auto& mesh : Female->m_parts_mgr->m_list)
	//			{
	//				if (mesh->GetIsModelAutoLoad() == false)
	//				{
	//					if (std::find(MeshesCustom.begin(), MeshesCustom.end(), mesh->GetFileName()) == MeshesCustom.end())
	//						Female->m_parts_mgr->Del(mesh);
	//				}
	//			}
	//		}
	//	}
...
#ifdef _SYSINTERNEW
void ZGameInterface::BackGroundButtom()
{
	...
	bool bLadderWarsUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDERWAR);
	...
	if (bClanBattleUI)
	{
		...
		pWidget = m_IDLResource.FindWidget("Lobby_BottomBG");
		if (pWidget) pWidget->Show(true);

		pWidget = m_IDLResource.FindWidget("Lobby_ChannelNameBG");
		if (pWidget) pWidget->Show(true);

		pWidget = m_IDLResource.FindWidget("Lobby_LobbyListTabPanel");
		if (pWidget) pWidget->Show(true);
	...
	else if (bLadderWarsUI)
	{
		...
		pWidget = m_IDLResource.FindWidget("Lobby_PagePanel");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("Lobby_PanelShadow");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("Lobby_CenterPanel");
		if (pWidget) pWidget->Show(false);
		...
		/*pWidget = m_IDLResource.FindWidget("Lobby_ChannelName");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("ShopCaller");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("EventShopCaller");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("EquipmentCaller");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("LobbyOptionFrame");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("ChannelChattingInput");
		if (pWidget) pWidget->Show(false);

		pWidget = m_IDLResource.FindWidget("ChannelChattingOutput");
		if (pWidget) pWidget->Show(false);*/
...
void ZGameInterface::SelectBackground(int i)
{
	...
	case 0:
		...
		if (pWidget) pWidget->Show(false);
		pWidget = m_IDLResource.FindWidget("Lobby_MainLadderWarsBG");
		if (pWidget) pWidget->Show(false);
		pWidget = m_IDLResource.FindWidget("Lobby_MainClanBG");
		if (pWidget) pWidget->Show(false);
		pWidget = m_IDLResource.FindWidget("Lobby_MainBattleBG");
		if (pWidget) pWidget->Show(false);
		break;
	case 1:
		pWidget = m_IDLResource.FindWidget("Lobby_MainNormalBG");
...
void ZGameInterface::ClanChannelType()
{
	bool bClanBattleUI = ((ZGetGameClient()->GetServerMode() == MSM_CLAN) && (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_CLAN));
	...
	if (bClanBattleUI)
	{
		...
		ZGetGameInterface()->InitLadderWarsUI(false);
		ZGetGameInterface()->InitClanLobbyUI(true);
	}
}
void ZGameInterface::DuelChannelType()
{
	bool bDuelTournamentUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_DUELTOURNAMENT);

	if (bDuelTournamentUI == true)
		SelectBackground(2);
...
void ZGameInterface::PlayerChannelType()
{
	bool bLadderWarsUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDERWAR);

	if (bLadderWarsUI == true)
		SelectBackground(3);
	...
	if (bLadderWarsUI)
	{
		ZGetGameInterface()->InitDuelTournamentLobbyUI(false);
		ZGetGameInterface()->InitClanLobbyUI(false);
		ZGetGameInterface()->InitLadderWarsUI(true);
	}
}
void ZGameInterface::NormalChannelType()
{
	bool bNormal = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_USER ||
		ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_PRIVATE ||
		ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_PRESET);
...
void ZGameInterface::InitLadderWarsUI(bool bLadderWars)
{
	BackGroundButtom();
	...
	if (pWidget) pWidget->Show(!bLadderWars);
	pWidget = m_IDLResource.FindWidget("StageAfterBtn");
	if (pWidget) pWidget->Show(!bLadderWars);
	bool bCheck = !bLadderWars;
	for (int i = 1; i <= 6; i++)
	{
		char szBuffer[64];
	...
	m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_BATTLE_EXIT, !bLadderWars);
	...
	pWidget = m_IDLResource.FindWidget("ArrangedLadderWars");
	if (pWidget) pWidget->Show(bLadderWars);
#ifdef _LADDERUPDATE 
	pWidget = m_IDLResource.FindWidget("CharUpdateCaller");
	if (pWidget) pWidget->Show(bLadderWars);
#endif
	pWidget = m_IDLResource.FindWidget("Lobby_LadderWarsRankingList");
	if (pWidget)
	{
		pWidget->Show(bLadderWars);
	}
}
void ZGameInterface::InitClanLobbyUI(bool bClanBattleEnable)
{
	...
	pWidget = m_IDLResource.FindWidget("InviteCW");
	if (pWidget) pWidget->Show(bClanBattleEnable);
...
void ZGameInterface::OnLobbyDestroy(void)
{
	...
	ZLadderWarsRankingListBox* pBZRankingListBox = (ZLadderWarsRankingListBox*)m_IDLResource.FindWidget("Lobby_DuelTournamentRankingList");
	if (pBZRankingListBox) {
		pBZRankingListBox->UnloadInterfaceImgs();
		pBZRankingListBox->SetVisible(false);
	}
	MWidget* pWidget = m_IDLResource.FindWidget("StageName");
...
void ZGameInterface::OnStageCreate(void)
{
	...
	*/
	/*
		// ¹æÀåÀÌ¸EÇöÀE°ÔÀÓ ¼³Á¤À» ´Ù½Ã ¼³Á¤ÇÑ´Ù.
...
char* GetItemSlotName( const char* szName, int nItem)
{
	...
	switch (nItem)
	{
		...
		case MMCIP_CUSTOM3 :		strcat( szTemp, "_EquipmentSlot_Custom3");		break;
...
bool ZGameInterface::OnCreate(ZLoadingProgress *pLoadingProgress)
{
	...
	if( !InitLocatorList(ZApplication::GetFileSystem(), "system/ZLocatorList.xml") )
	{
		...
		// ·ÎÄÉÀÌÅÍ¸¦ ¸EÀÐ¾ûÑµ °ÔÀÓÀº ÁøÇàÀ» ÇÔ. 
		// return false;
	}
	*/
...
//	SetState(m_nInitialState);
	...
	for(int i = 0;  i < MMCIP_END;  i++)
	{
		...
		if ( itemSlot)
		{
			...
			switch ( itemSlot->GetParts())
			{
				...
				case MMCIP_CUSTOM3:		itemSlot->SetText( ZMsg( MSG_WORD_ITEM3));				break;
	...
	ZGetGameTypeManager()->SetGameTypeStr( MMATCH_GAMETYPE_QUEST_CHALLENGE, ZMsg( MSG_MT_QUEST_CHALLENGE));
	...
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_SPY, ZMsg(MSG_MT_SPYMODE));
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_TURBO, ZMsg(MSG_MT_TURBO));
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_SKILLMAP, ZMsg(MSG_MT_SKILL));
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_GHOST, ZMsg(MSG_MT_GHOST));			// Custom: Ghost Mode - Desperate
#ifdef _MAGICBOX
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_DROPMAGIC, ZMsg(MSG_MT_MAGICBOX));
#endif

#ifdef _PAINTMODE
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_PAINTBALL_SOLO, ZMsg(MSG_MT_PAINT_SOLO));
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_PAINTBALL_TEAM, ZMsg(MSG_MT_PAINT_TEAM));
#endif
...
#ifndef _QUEST_ITEM// Äù½ºÆ® ¾ÆÀÌÅÛ Ä«Å×°úÔ®¸¦ Á¦°ÅÇÑ´Ù!
	...
	// µà¾óÅä³Ê¸ÕÆ® ·ÎºEÀEûUI¿¡ ¼³¸EÅøÆÁ Ãß°¡
	char szTooltip[256];
...
#ifdef _DEVMODE
	Gunz_Console->DebugConsole();
	mlog("Debug console run success.\n");
#endif
...
void ZGameInterface::OnDestroy()
{
	...
	SetState(GUNZ_NA);	// ÇöÀEGunzState Á¾·E
	...
	SAFE_DELETE(m_pLoginBG);			// ÇØÁ¦ ¾ÈµÇ´Â °æ¿E¡ ÀÖ±æ·¡ -_-;
...
bool ZGameInterface::SetState(GunzState nState)
{
	...
	// °ÔÀÓÀ» ½ÃÀÛ(GUNZ_GAME)ÇÏ±EÀE¡ Äù½ºÆ® ¸ðµE½ÃÀÛ ¹«ºñ¸¦ º¸¿©ÁÙ ÇÊ¿ä°¡ ÀÖ´ÂÁE°Ë»E
	if ( nState == GUNZ_GAME)
	{
	...
	else if (nState == GUNZ_CHARSELECTION) OnCharSelectionCreate();
#ifdef _BIRDTEST
   ...
   */
	...
	if (pConnectingLabel)
	{
		...
		for (int i = 0; i < nCount; i++)
			szMsg[i] = ' ';
		sprintf(szMsg, "%s %s ", szMsg, "Verifying your account");
		for (int i = 0; i < nCount; i++)
			strcat(szMsg, ".");
	...
	if (m_bLoginTimeout && (m_dwLoginTimeout <= dwCurrTime))
	{
		...
		if (pLabel) // Custom: Message Dialog Password Incorrect and Problem Server
		{
			char szText[128] = { 0, };
	...
	// Custom: Play music
	if (!ZApplication::GetSoundEngine()->IsPlaying())
		ZApplication::GetSoundEngine()->PlayMusic(true);
	...
	if (dwCurrTime - tmAgentPingTestCheckTimer > CHECK_PING_TIME)
	{
		...
		if (pServerList)
		{
			...
			for (SERVERLIST::iterator itr = cServerList.begin(); itr != cServerList.end(); itr++)
			{
				...
				// Custom: UDP Port changed from 7778 to 5101
...
*/
	...
	if (m_nLoginState == LOGINSTATE_FADEIN)
	{
		...
		// Custom: Changed login frame logic
		/*
		if ( dwCurrTime >= m_dwLoginTimer)
		{
			int nOpacity = pPicture->GetOpacity() + 3;
		...
		*/
		...
		{
			if (timeGetTime() > m_dwLoginTimer + 8000)
			{
				m_dwLoginTimer = dwCurrTime + 1000;
				m_nLoginState = LOGINSTATE_SHOWLOGINFRAME;
			}
...
#else
		pWidget->Show(false);
		m_dwLoginTimer = dwCurrTime + 1500;
		m_nLoginState = LOGINSTATE_SHOWLOGINFRAME;
#endif
	}
	// Show login frame
	...
	{
		m_bLoginTimeout = false;
		pWidget->Show(false); // Custom: Added this
	...
	{
		// Custom: Changed logic for the login map in background
		m_bLoginTimeout = false;
		pWidget->Show(false);
	...
	//#ifdef LOCALE_NHNUSA
#if defined(LOCALE_NHNUSA) || defined(_DEBUG) || defined(_LOGINPING)
#define CHECK_PING_TIME	(1 * 3 * 1000)	// 3 sec
	static unsigned long tmAgentPingTestCheckTimer = dwCurrTime;
	if (dwCurrTime - tmAgentPingTestCheckTimer > CHECK_PING_TIME)
	{
	...
	// Custom: Draw login map background
	if (m_pBackground)
	{
		m_pBackground->LoadMesh();
		m_pBackground->Draw();
...
void ZGameInterface::OnDrawStateLobbyNStage(MDrawContext* pDC)
{
	...
	{
		// TODO : ÀÌµéÀ» draw Å¸ÀÓ¿¡ °è¼ÓÇØ¼­ ÇÒ ÇÊ¿ä´Â ¾ø¾ûÖ¸ÀÎ´Ù. Àû´çÇÑ °÷À¸·Î ¿Å±âÀÚ
		...
		if (pLabel) pLabel->SetText(buf);
		// MD
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecMD");
		sprintf(buf, "%s: %d", ZMsg(MSG_CHARINFO_MEDAL), ZGetMyInfo()->GetLC());
		if (pLabel) pLabel->SetText(buf);

		// EV
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecEV");
		sprintf(buf, "%s: %d", ZMsg(MSG_CHARINFO_EVENT), ZGetMyInfo()->GetEV());
		if (pLabel) pLabel->SetText(buf);

		// HP/AP
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecHPAP");
		sprintf( buf, "%s/%s: %d/%d", ZMsg(MSG_CHARINFO_HP), ZMsg(MSG_CHARINFO_AP), ZGetMyInfo()->GetHP(), ZGetMyInfo()->GetAP());
		if (pLabel) pLabel->SetText(buf);

		// Kill/Death
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecKillDeath");
		sprintf(buf, "%s/%s: %d/%d", ZMsg(MSG_CHARINFO_KILL), ZMsg(MSG_CHARINFO_DEATH), ZGetMyInfo()->GetKill(), ZGetMyInfo()->GetDeath());
		if (pLabel) pLabel->SetText(buf);
		...
		sprintf( buf, "%s > %s", ZMsg( MSG_WORD_LOBBY), ZGetGameClient()->GetChannelName());
		if (pLabel) 
			pLabel->SetText(buf);
	}
...
void ZGameInterface::OnDraw(MDrawContext *pDC)
{
	...
	if (timeGetTime() >= m_dwTimeCount)
	{
		...
		// Custom: Translated play-time hour message from Korean
		char szText[256];
		if (m_dwHourCount > 3)
			sprintf(szText, "You have been online for 1 hours. Please take a break.", m_dwHourCount);
		else
			sprintf(szText, "You have been online for 1 hours, we recommend that you rest well, please.", m_dwHourCount);
		ZChatOutput(MCOLOR(ZCOLOR_CHAT_SYSTEM), szText);

		ZChatOutput(MCOLOR(ZCOLOR_CHAT_SYSTEM), "This game is available only to users who are older than 15 years old.");
	}
#endif
	__EP(11);
}
...
#ifndef _PUBLISH
	// µðÀÚÀÌ³Ê¿E.. È¥ÀÚ ¿Ê°¥¾Æ ÀÔ´Â Å×½ºÆ® ÇÒ¶§ »ç¿EÑ´Ù...
...
void ZGameInterface::RespawnMyCharacter()	// È¥ÀÚÅ×½ºÆ®ÇÒ¶§ Å¬¸¯ÇÏ¸EµÇ»EÆ³­´Ù.
{
...
bool ZGameInterface::OnGlobalEvent(MEvent* pEvent)
{
	...
	switch(pEvent->nMessage){
		...
		case MWM_KEYDOWN:
			{
				// Ctrl+R UI ¸®¼Ò½º ¸®·ÎµE- ¹®Á¦°¡ ¸¹´Ù. ±Ùµ¥ ±×³É UI µðÀÚÀÎ¿E±ÞÁ¶ ±â´ÉÀÌ´Ï±E±×³É ¾¹½Ã´E.
				// (800*600ÀÌ¿Ü¿¡¼­´Â È­¸éÀÌ ÀÌ»óÇØÁE Á¾·á½Ã ¸Þ¸ð¸®¸¯ ¾öÃ» ³², °ÔÀÓÁß¿£ ¸®·ÎµEÀÛµ¿¾ÈµÊ)
...
void ZGameInterface::ChangeParts(int key)
{
	...
	/*
	int mode = 0;
	int type = 0;		// ³ªÁß¿¡ ¿©·¯ ¹úÀÌ¶ó¸EÀåºñÇÒ id ÁöÁ¤...
	...
	g_parts[mode] = !g_parts[mode];	// Ä³¸¯ÅÍ Å¬·¡½º·Î ¿Å±â±E
	//	g_parts_change = true;
...
void ZGameInterface::ChangeWeapon(ZChangeWeaponType nType)
{
	...
	if (m_pGame->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_GHOST && pChar->GetTeamID() == MMT_RED) return;		// Custom: Ghost Mode - Desperate
	...
	if ((nType == ZCWT_PREV) || (nType == ZCWT_NEXT))
	{
		...
		// Custom: add new slot custom by Desperate Fix
		for (int i = MMCIP_MELEE; i < MMCIP_CUSTOM3; ++i)
		{
			if (!IsWeaponCharItemParts((MMatchCharItemParts)i))
				continue;

			if (ZGetConfiguration()->GetEtc()->bWeaponRotation && (i == MMCIP_CUSTOM1 || i == MMCIP_CUSTOM2 || i == MMCIP_CUSTOM3))
				continue;
		...
		if (nPos < 0) return;
		//Custom: Fixed Avatar bug scroll in game
		if (nType == ZCWT_PREV)
		{
			if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_SNIPERMODE)
				return;
			...
			while (nNewPos != nPos)
			{
				...
				if (!ZGetConfiguration()->GetEtc()->bWeaponRotation && ((nPart == MMCIP_CUSTOM1) || (nPart == MMCIP_CUSTOM2) || (nPart == MMCIP_CUSTOM3)))
				{
	...
	{
		if( (!pChar->m_bInfected && nParts == MMCIP_MELEE) || nParts == MMCIP_CUSTOM1 || nParts == MMCIP_CUSTOM2 || nParts == MMCIP_CUSTOM3)
			return;
	}
...
bool ZGameInterface::Update(float fElapsed)
{
	...
	{
		if (m_dwAfkTimer + 900000 <= dwCurrTime)
		{
			ZPostStageLeave(ZGetMyUID());
			...
			ZChatOutput(MCOLOR(255, 128, 36), "You were kicked for being inactive for 15 minutes.");
		}
	}
#endif
	// º¸ÀÌ½º »ç¿ûÑE¾÷µ¥ÀÌÆ®
	...
	// Å×½ºÆ®·Î ¸®ÇÃ·¹ÀÌ´Â ´Ù½Ã Ã³À½ºÎÅÍ ½ÃÀÛÇÏ±EÀ§ÇØ ÀÌ·¸°Ô ¸¸µé¾ú´Ù.
	if (GetState() == GUNZ_LOBBY)
	{
	...
	// ·Îºñ°¡ ¾Æ´Ñµ¥ Åä³Ê¸ÕÆ® ÂE¡Ã¢ÀÌ ¶°ÀÖÀ¸¸EÁ¢¼Ó ²÷°Ü¼­ ·Î±×ÀÎ È­¸éÀ¸·Î µ¹¾Æ°£ °æ¿EµûÜÌ¹Ç·Î ÂE¡Ã¢À» ´Ý¾ÆÁØ´Ù
...
void ZGameInterface::UpdateCursorEnable()
{
	...
	else
	{
		...
		{
			SetCursorEnable( true);
			return; // Custom: Enhanced Kick System
		}
		else
			SetCursorEnable( false);

		// Custom: Enhanced Kick System
		pWidget = m_IDLResource.FindWidget("KickPlayer");
		if ( pWidget && pWidget->IsVisible())
			SetCursorEnable( true);
...
{
	// 'µà¾óÅä³Ê¸ÕÆ® ÂE¡Áß' ´ÙÀÌ¾ó·Î±× ¸Þ½ÃÁöÀÇ ... ±æÀÌ¸¦ ½Ã°£¿¡ µû¶EÁ¶Á¤ÇÑ´Ù
	...
	// ±âÁ¸¿¡ ºÙ¾ûÜÖ´E...À» Á¦¿ÜÇÑ ¿øº» ¹®ÀÚ¿­ÀÇ ±æÀÌ¸¦ ¾Ë¾Æ³½´Ù
...
void ZGameInterface::ShowMessage(const char* szText, MListener* pCustomListenter, int nMessageID)
{
	...
	// nMessageID°¡ 0ÀÌ ¾Æ´Ï¸E¸Þ¼¼ÁEµÚ¿¡ ¸Þ¼¼ÁE¹øÈ£µµ ÇÔ²² Ãâ·ÂÇØÁØ´Ù.(´Ù¸¥ ³ª¶E¸»ÀÏ¶§ È®ÀÎÇÏ±EÀ§ÇÔ)
	if (nMessageID != 0)
	{
...
void ZGameInterface::ChangeSelectedChar(int nNum)
{
	...
	// ¸¸¾EÄ³¸¯ÅÍ Á¤º¸¸¦ ¾È¹Þ¾Ò¾úÀ¸¸E¼­¹ö¿¡ ´Þ¶ó°E¿äÃ»ÇÑ´Ù.
...
void ZGameInterface::UpdateBlueRedTeam(void)
{
	...
	// Custom: Infected game mode room list
	...
	else
	{
		...
		if (pListBox)
		{
			...
			/*pNumLabel = (ZBmNumLabel*)pRes->FindWidget("Stage_ChallengeTime");
			if (pNumLabel)
				pNumLabel->Show(false);*/
			...
			if (pLabel)
			{
				...
				{
					// Custom: Display best time from db (haven't set an actual value yet, this is just a placeholder value).
					sprintf(szGoodTimeTest, "Max Record: %d:%02d",atoi(ZGetGameClient()->GetMatchStageSetting()->GetStageBestTime())/60, atoi(ZGetGameClient()->GetMatchStageSetting()->GetStageBestTime())%60);
					pLabel->SetText(szGoodTimeTest);
				}
			}*/
		}
	}
...
{
	// Custom: Prevent scrolling after purchase (EXCLUDE -- (but reserialize if state changed))
	//if( g_bReceivedShopItemList && g_nCurrentShopState != ZGetGameInterface()->GetState() )
	//{
	//	g_bReceivedShopItemList = false;
	//	g_nCurrentShopState = ZGetGameInterface()->GetState();
	//}

	if( !g_bReceivedShopItemList )
	{
		g_bReceivedShopItemList = true;
												 ...
												 , int nGItemCount )
{
	...
	// »óÁ¡¿¡¼­ Ä³¸¯ÅÍ UI Text Á¤º¸¸¦ ¾÷µ¥ÀÌÆ® ÇØÁØ´Ù (·¹º§, ¹Ù¿ûâ¼, Ã¼·Â, ¹æ¾E ¹«°Ô)
...
{
	// Äù½ºÆ® ¸ðµE..  ¸ÓÇÏ´Â ÄÚµåÀÎÁE¸ð¸£°ÚÀ½...  -_-;
	// ³ªÁß¿¡ ÇÊ¿ä¾ø°Ô µÇ¸EÀÌ ÇÔ¼EÀÚÃ¼¸¦ »èÁ¦ÇÏ°E¹Ù·Î OnStageInterfaceSettupÀ¸·Î ³Ñ¾ûÌ¡°Ô...
/*
#ifdef _QUEST
	...
	{
		// ³ªÁß¿¡ ·£´ý¸Ê ±¸ÇöÇÒ¶§±ûÝE±×³É ¸Ê ÇÏ³ª¸¸ »ç¿E
...
// ÃÖ´E200 À¸·ÎÀâ°E-14 ±ûÝEÃ¼Å© ±ÛÀÚ°¡ Àß¸®ÁE¾Êµµ·Ï..

 // zmaplistbox.cpp ¿¡µµ ¼±¾ðµÇ¾ûÜÖ´Ù..
...
//		str += " ";
	...
	if(pItemDesc->m_nResLevel.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,":%d",pItemDesc->m_nResLevel.Ref());
	...
	if(pItemDesc->m_nWeight.Ref()){
		if(bAdd) str += " / ";
		sprintf(temp,"%d",pItemDesc->m_nWeight.Ref());
	...
	if(pItemDesc->m_nMaxBullet.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,": %d",pItemDesc->m_nMaxBullet.Ref());
		// sprintf(temp,"ÃÖ´Eº¼E: %d",pItemDesc->m_nMaxBullet.Ref());
		nLen = (int)strlen(temp);
		if((int)str.size() + nLen > (nLine+1) * MAX_TOOLTIP_LINE_STRING+3) { str += "\n"; nLine++; }
	...
	if(pItemDesc->m_nMagazine.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,": %d",pItemDesc->m_nMagazine.Ref());
		// sprintf(temp,"ÅºÃ¢ : %d",pItemDesc->m_nMagazine.Ref());
		nLen = (int)strlen(temp);
		if((int)str.size() + nLen > (nLine+1) * MAX_TOOLTIP_LINE_STRING+3) { str += "\n"; nLine++; }
	...
	if(pItemDesc->m_nDelay.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,": %d",pItemDesc->m_nDelay.Ref());
		// sprintf(temp,"µô·¹ÀÌ : %d",pItemDesc->m_nDelay.Ref());
		nLen = (int)strlen(temp);
		if((int)str.size() + nLen > (nLine+1) * MAX_TOOLTIP_LINE_STRING+3) { str += "\n"; nLine++; }
	...
	if(pItemDesc->m_nReloadTime.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,": %d",pItemDesc->m_nReloadTime.Ref());
		// sprintf(temp,"ÀåÀEÃ°£ : %d",pItemDesc->m_nReloadTime.Ref());
		nLen = (int)strlen(temp);
		if((int)str.size() + nLen > (nLine+1) * MAX_TOOLTIP_LINE_STRING+3) { str += "\n"; nLine++; }
	...
	if(pItemDesc->m_nMaxWT.Ref()) {
		if(bAdd) str += " / ";
		sprintf(temp,": %d",pItemDesc->m_nMaxWT.Ref());
		// sprintf(temp,"+ÃÖ´E«°Ô : %d",pItemDesc->m_nMaxWT.Ref());
		nLen = (int)strlen(temp);
		if((int)str.size() + nLen > (nLine+1) * MAX_TOOLTIP_LINE_STRING+3) { str += "\n"; nLine++; }
	...
	{	// ½ºÅ×ÀÌÁE¾È¿¡¼­ ÀåºE¹öÆ° ¼±ÅÃ
		...
		if ( pButton && pLabel)
		{
			...
			if ( ZApplication::GetGameInterface()->GetState() == GUNZ_STAGE)
			{
				...
				pButton->Show( true );
				//pButton->Show( false);
				sprintf( buf, "%s > %s", ZMsg( MSG_WORD_STAGE), ZMsg( MSG_WORD_EQUIPMENT));
			...
			{
				pButton->Show( true);
				sprintf( buf, "%s > %s", ZMsg( MSG_WORD_LOBBY), ZMsg( MSG_WORD_EQUIPMENT));
				pLabel->SetText( buf);
			}
		}
...
//		ZPostRequestGetCharQuestItemInfo( ZGetGameClient()->GetPlayerUID());
	...
	{	// ½ºÅ×ÀÌÁE¾È Àåºñºä¾ûÛ¡¼­ ³ª°¡±E¹öÆ° ¼±ÅÃ
...
void ZGameInterface::ShowShopDialog(bool bShow)
{
	...
	switch(ZShopTypes)
	{
		case ZSHOP_NORMAL:
		{
			MButton * pWidget = (MButton*)pResource->FindWidget("ShopCaller");
			if (pWidget) 
				pWidget->Show(true);
		}
		break;
	...
	{	// »óÁ¡ ¼±ÅÃ
		...
		// Custom: Show shop in stage
		if ( ZApplication::GetGameInterface()->GetState() == GUNZ_STAGE )
			ZPostStageState( ZGetGameClient()->GetPlayerUID(), ZGetGameClient()->GetStageUID(), MOSS_EQUIPMENT); // MOSS_SHOP isn't handled + no pic..
		...
		if ( ZApplication::GetGameInterface()->GetState() == GUNZ_LOBBY )
			sprintf( buf, "%s > %s", ZMsg( MSG_WORD_LOBBY), ZMsg( MSG_WORD_SHOP));
		else
			sprintf( buf, "%s > %s", ZMsg( MSG_WORD_STAGE), ZMsg( MSG_WORD_SHOP));
...
void ZGameInterface::EnableLobbyInterface(bool bEnable)
{
	...
	EnableWidget("Lobby_Charviewer_info", bEnable);		// Á¤º¸º¸±E¹öÆ°
	EnableWidget("StageJoin", bEnable);					// °ÔÀÓÂE¡ ¹öÆ°
	EnableWidget("StageCreateFrameCaller", bEnable);	// °ÔÀÓ»ý¼º ¹öÆ°
	EnableWidget("ShopCaller", bEnable);				// »óÁ¡ ¹öÆ°
	EnableWidget("MedalShopCaller", bEnable);				// »óÁ¡ ¹öÆ°
	EnableWidget("EventShopCaller", bEnable);
	EnableWidget("EquipmentCaller", bEnable);			// ÀåºE¹öÆ°
	EnableWidget("ReplayCaller", bEnable);				// ¸®ÇÃ·¹ÀÌ ¹öÆ°
	EnableWidget("CharSelectionCaller", bEnable);		// Ä³¸¯ÅÍ º¯°E¹öÆ°
	EnableWidget("Logout", bEnable);					// ·Î±×¾Æ¿E¹öÆ°
	EnableWidget("QuickJoin", bEnable);					// ÄE¶ÀÎ ¹öÆ°
	EnableWidget("QuickJoin2", bEnable);				// ÄE¶ÀÎ ¹öÆ°
	EnableWidget("ChannelListFrameCaller", bEnable);	// Ã¤³Îº¯°E¹öÆ°
...
#ifdef _SCREENSHOT_FILTER_CHAT
	EnableWidget("ScreenShotCaller", bEnable);
#endif
	EnableWidget("MonsterBookCaller", bEnable);
	if (bEnable)
	{
...
void ZGameInterface::EnableStageInterface(bool bEnable)
{
	...
	EnableWidget("ShopCaller", bEnable);				// »óÁ¡ ¹öÆ°
	EnableWidget("MedalShopCaller", bEnable);			// ÀåºE¹öÆ°
	EnableWidget("EventShopCaller", bEnable);
	EnableWidget("EquipmentCaller", bEnable);			// ÀåºE¹öÆ°
	EnableWidget("StageSettingCaller", bEnable);		// ¹æ¼³Á¤ ¹öÆ°
	EnableWidget("StageObserverBtn", bEnable);			// °EEÃ¼Å© ¹öÆ°
	EnableWidget("Lobby_StageExit", bEnable);			// ³ª°¡±E¹öÆ°
	...
	EnableWidget("StageMaxPlayer", bEnable);			// ÃÖ´EÎ¿EÄÞº¸¹Ú½º
	EnableWidget("StageRoundCount", bEnable);			// °æ±âÈ½¼EÄÞº¸¹Ú½º

	EnableWidget("StagePlayerList_", bEnable);
#ifdef _BOXLEAD
	EnableWidget("Lead", bEnable);
#endif
	//Banderas
#ifdef _COUNTRYFLAG
...
{
	//If tries to leave, return
	/*if (ZGetGameClient()->IsVoteInProgress())
	{
		if (ZGetGame()->m_pMyCharacter != NULL)
		{
			if (stricmp(ZGetGame()->m_pMyCharacter->GetUserName(), GetCombatInterface()->GetVoteInterface()->GetTargetName()) == 0)
				return;
		}
	}*/

	// Custom: Stop recording when you leave a battle to prevent corrupt replays
	ZGetGame()->StopRecording();

	// Custom: Force unscope.
...
void ZGameInterface::ReserveLeaveBattle()
{
	...
	// Á×ÀÚ¸¶ÀÚ ½ºÅ×ÀÌÁö·Î ³ª°£ÈÄ ¹Ù·Î ¹èÆ²·Î µé¾ûÛÃ°æ¿E6.5ÃÊ¾È¿¡ ÀçÇöµÉ°æ¿E
	// ¼­¹ö¿¡¼­ Ä³¸¯ÅÍ¸¦ ½ºÆùÇØÁÖÁE¾Ê´Â´Ù....±×°ÍÀ¸·Î ÀÎÇØ ¹ö±× ¹ß»ý
	// Á×¾ûØ­ ½ºÆùµÇ±â¸¦ ±â´Ù¸±¶§´Â
...
// Custom: Enhanced Kick System
void ZGameInterface::ShowKickPlayerDialog(bool bShow)
{
	MWidget* pWidget = m_IDLResource.FindWidget( "KickPlayer");
	if ( !pWidget)
		return;
	...
	MComboBox* pCombo = (MComboBox*)m_IDLResource.FindWidget( "KickPlayer_ConfirmID");
	...
	if (m_nState == GUNZ_GAME)
	{
		for ( ZCharacterManager::iterator itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); itor++)
		{
			...
			if (pCharacter && !pCharacter->IsAdminHide())
				pCombo->Add(pCharacter->GetUserName());
		}
	}
...
void ZGameInterface::SetupPlayerListButton(int index)
{
	...
	// Å¬·£ÀÎµ¥ Å¬·£¿¡ °¡ÀÔÀÌ ¾ÈµÇ¾ûÜÖÀ¸¸E»ý¼º Ã¢À» º¸ÀÎ´Ù
...
#endif // _CW_VOTE
{
	...
	if (isvote == true)
	{
		...
		pWidget = m_IDLResource.FindWidget("MapSelector");
		if (pWidget)
		{
			pWidget->Show(bFinding);
			if (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_CLAN || ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDERWAR)
				pWidget->SetText("System Vote Map for Clan War.");
			else
				pWidget->SetText("System Vote Map for Ladder.");
		}
	}
	else
	{
...
//	SAFE_ENABLE("LobbyPlayerListTabClanCreateButton");
	...
	SAFE_ENABLE("MedalShopCaller", bFinding);
	SAFE_ENABLE("EventShopCaller", bFinding);
	...
	SAFE_ENABLE("MonsterBookCaller", bFinding);
	SAFE_ENABLE("ArrangedLadderWars", bFinding); // Custom: Player Wars invisible button
	SAFE_ENABLE("CharUpdateCaller", bFinding);
	SAFE_ENABLE("ScreenShotCaller", bFinding);
	m_bWaitingArrangedGame = bFinding;
}

// µà¾óÅä³Ê¸ÕÆ® ½ÅÃ»ÇßÀ» ¶§ È­¸é»óÀÇ ¹öÆ°µéÀ» ºñÈ°¼ºÈ­ÇÑ´Ù( È¤Àº ±×¹Ý´E
void ZGameInterface::OnDuelTournamentGameUI(bool bWaiting)
{
	...
	SAFE_ENABLE("MedalShopCaller", bWaiting);
	SAFE_ENABLE("EventShopCaller", bWaiting);
	...
	SAFE_ENABLE("MonsterBookCaller", bWaiting);
	SAFE_ENABLE("ArrangedLadderWars", bWaiting); // Custom: Player Wars invisible button
#ifdef _LADDERUPDATE 
	SAFE_ENABLE("CharUpdateCaller", bWaiting);
#endif
	SAFE_ENABLE("ScreenShotCaller", bWaiting);
}
...
bool ZGameInterface::IsReadyToPropose()
{
	...
	// TODO : ¾Õ¿¡ modal Ã¢ÀÌ ¶°ÀÖÀ¸¸Ereturn false ÇÏÀÚ..
	return true;
}
...
// ¸®ÇÃ·¹ÀÌ °EÃ ÇÔ¼öµEµ¿È¯ÀÌ°¡ Ãß°¡)

/***********************************************************************
  ShowReplayDialog : public
  
  desc : ¸®ÇÃ·¹ÀÌ È­¸Eº¸ÀÌ±E
...
void ZGameInterface::LoadCustomImages()
{
	...
	{
		mlog("[Open ScreenShot] Failed due to finding first file - %d\n", GetLastError());
	}
	do
	{
...
void ZGameInterface::ShowReplayDialog( bool bShow)
{
	if ( bShow)			// º¸ÀÌ±âÀÌ¸E..
	{
		// Custom: Fix for replay box showing in stage rooms, glitching the stage
		if (GetState() != GUNZ_LOBBY && GetState() != GUNZ_LOGIN)
			return;
		...
		// Custom: Delete Replay
		pWidget = (MWidget*)m_IDLResource.FindWidget( "Replay_Delete");
		if ( pWidget)
			pWidget->Enable( false);
		...
		if ( pListBox)
		{
			...
			// Get path name
			TCHAR szPath[ MAX_PATH];
			if ( GetMyDocumentsPath( szPath))		// ³»¹®¼­ Æú´E°æ·Î
			{
				strcat( szPath, GUNZ_FOLDER);		// Gunz Æú´E°æ·Î
				strcat( szPath, REPLAY_FOLDER);		// Replay Æú´E°æ·Î
				CreatePath( szPath );
			}
			TCHAR szFullPath[ MAX_PATH];
			strcpy( szFullPath, szPath);

			strcat( szPath, "/*.gzr");			// È®Àå¸EÇÊÅÍ
...
void ZGameInterface::ViewReplay( void)
{
	...
	if ( GetMyDocumentsPath( szName))				// ³»¹®¼­ Æú´E°æ·Î
	{
		strcat( szName, GUNZ_FOLDER);				// Gunz Æú´E°æ·Î
		strcat( szName, REPLAY_FOLDER);				// Replay Æú´E°æ·Î
		strcat( szName, "/");
...
// Custom: Delete Replay
void ZGameInterface::DeleteReplay( void)
{
	MListBox* pListBox = (MListBox*)m_IDLResource.FindWidget( "Replay_FileList");
	if ( !pListBox)
		return;
	...
	if ( remove( szName ) != 0 )
	{
		ZApplication::GetGameInterface()->ShowMessage( "Replay File could not be deleted." );
		return;
	}

	pListBox->Remove(pListBox->GetSelIndex());

	ZApplication::GetGameInterface()->ShowMessage( "Replay File deleted successfully." );
}
...
void ZGameInterface::OnResponseBuyQuestItem( const int nResult, const int nBP )
{
	...
	{
		// Á¤ÀÇµÇ¾ûÜÖÁE¾ÊÀ½.
		mlog( "ZGameInterface::OnCommand::MC_MATCH_RESPONSE_BUY_QUEST_ITEM - Á¤ÀÇµÇÁE¾ÊÀº °á°úÃ³¸®.\n" );
		ASSERT( 0 );				
	}
...
// Äù½ºÆ®¿E¾ÆÀÌÅÛ ¾ÆÀÌÄÜ È­ÀÏ¸EÇÏµEÄÚµEµ¢¾ûÔ®~  -_-;)
// ³õÀ»µ¥°¡ ¾ø¾ûØ­ ÀÏ´Ü ¿©±â´Ù...  -_-;
MBitmap* ZGameInterface::GetQuestItemIcon( int nItemID, bool bSmallIcon)
{
	...
	{
		// Page
		case 200001 :		// Âõ°ÜÁE13ÆäÀÌÁE
		case 200002 :		// Âõ°ÜÁE25ÆäÀÌÁE
		case 200003 :		// Âõ°ÜÁE41ÆäÀÌÁE
		case 200004 :		// Âõ°ÜÁE65ÆäÀÌÁE
			strcpy( szFileName, "slot_icon_page");
			break;

		// Skull
		case 200005 :		// ÀÛÀºµÎ°E
		case 200006 :		// Å«µÎ°E
		case 200007 :		// ¼ö¼ö²²±âÀÇ µÎ°E
		case 200008 :		// °úÖúÔ°ÀÇ µÎ°E
		case 200009 :		// °úÖúÔ° Å·ÀÇ µÎ°E
		case 200010 :		// °Å´EÑ À¯°E
		...
		// Doll
		case 200019 :		// ½ºÄÌ·¹ÅEÀÎÇE
		case 200020 :		// ÄÚº¼µEÀÎÇE
		case 200021 :		// °úÖúÔ° ÀÎÇE
		case 200022 :		// ÆÊÆ÷¿EÀÎÇE
		case 200023 :		// Åä³¢ÀÎÇE
		case 200024 :		// °õÀÎÇE
		case 200025 :		// ÀúÁÖ ¹ÞÀº °õÀÎÇE
		case 200026 :		// ÆÊÆ÷¿E¾ÆÀÌµ¹
		case 200027 :		// ÀúÁÖ ¹ÞÀº ÆÊÆ÷¿E¾ÆÀÌµ¹
			strcpy( szFileName, "slot_icon_doll");
			break;

		// Book
		case 200028 :		// ¾Ç¸¶ÀÇ »çÀE
		case 200029 :		// ½ºÅ©¶óÀÌ´õÀÇ ¸úÖÎ »óÆE
		case 200030 :		// ½ºÅ©¶óÀÌ´õÀÇ ¸úÖÎ ÇÏÆE
		// Custom: School Quest Items
		case 200050 :
		case 200051 :
		case 200052 :
		...
		// Sword & 
		// Custom: iLevel Items 
		case 200038 :		// °úÖúÔ° Å·ÀÇ µµ³¢
		case 200039 :		// ÄÚº¼µE¿ÕÀÇ µµ³¢
		case 200040 :		// °Å´E½ºÄÌ·¹ÅæÀÇ Ä®
		case 200041 :		// ÀúÁÖ¹ÞÀº ½Ã½ÅÀÇ Ä®
		case 200042 :		// ¸®Ä¡ÀÇ ²¿¸®
		case 200043 :		// ÆÊÆ÷¿EÇ ¾óÀ½ Ä®
...
//		pServerList->AddServer( "", "", 0, 0, 0, 1000, false);						// Null
	...
	// Custom: Auto-set server once the server-list is received.
	int nLiveIndex = 0;

	if( (0 < nListCount) && (0 != pBlob) )
	{
		/*
		 * ÀEÛµÇ´Â ServerList´Â ÇöÁ¦ È°¼ºÈ­µÇ¾ûÜÖ´Â DBÀÇ Á¤º¸¸¸ º¸³»ÁÜ.
		 *  ÀEÛµÇÁE¾ÊÀº ServerID´Â Server°¡ ÇöÁ¦ µ¿ÀÛÁßÀÌÁE¾Ê´Â°ÍÀÓ.
		 */
		...
		for( int i = 0; i < nListCount; ++i )
		{
			...
			// pss->m_nCurPlayer;	// ÇöÁ¦ Á¢¼ÓÀ¯ÀE¼E
			// pss->m_nMaxPlayer;	// ¼­¹EÃÖ´EÁ¢¼Ó °¡´É ¼E
			// pss->m_nPort;		// ¼­¹EPort.
			// pss->m_nServerID;	// ¼­¹EID.
			// pss->m_nType;		// ¼­¹EÅ¸ÀÔ.
			// pss->m_bIsLive;		// ÇöÁ¦ ¼­¹ö°¡ µ¿ÀÛÇÏ°EÀÖ´Â°¡.
...
//					_snprintf( szServName, 127, "server%d(dead)", pss->m_nServerID );
			...
			if ( pss->m_nType == 1)					// Normal server (¿ø·¡ "type==1"ÀÌ Debug server¿´À¸³ª ³Ý¸¶ºúÛ¡¼­ ÀÏ¹Ý¼­¹ö·Î »ç¿EÏ°EÀÖ¾ûØ­ º¯°E
			...
			// ¼­¹EÀÌ¸§À» ¹Þ¾Æ¼­ Á÷Á¢ Ç¥½ÃÇØÁÜ
			sprintf( szServName, "%s", pss->m_szServerName);

			if ( (pss->m_nServerID % 50) >= 25)
				nRow = 1;

#ifdef LOCALE_NHNUSADIS) || defined(LOCALE_NHNUSA) || defined(_LOGINPING)
			//	¹Ì±¹ ¼­¹E¸®½ºÆ® ¹è¿­
			//	Å¬·£¼­¹E (Type 2) ¼­¹öID: ¼­ºÎÅ¬·£50~74,		µ¿ºÎÅ¬·£75~99		(Å¬·£¼­¹ö´Â 50~99 »çÀÌ)
			//	¸ÅÄ¡¼­¹E (Type 3) ¼­¹öID: ¼­ºÎ¸ÅÄ¡100~124,		µ¿ºÎ¸ÅÄ¡125~149		(¸ÅÄ¡¼­¹ö´Â 100~149 »çÀÌ)
			//	Äù½ºÆ®¼­¹EType 4) ¼­¹öID: ¼­ºÎÄù½ºÆ®150~174,	µ¿ºÎÄù½ºÆ®175~199	(Äù½ºÆ®¼­¹ö´Â 150~200 »çÀÌ)
			//	Å¬·£¼­¹E (Type 2) ¼­¹öID: XXXXÅ¬·£250~74,		À¯·´ºÎÅ¬·£275~299		(Å¬·£¼­¹ö´Â 250~299 »çÀÌ)
			//	¸ÅÄ¡¼­¹E (Type 3) ¼­¹öID: XXXX¸ÅÄ¡300~324,		À¯·´¸ÅÄ¡325~349		(¸ÅÄ¡¼­¹ö´Â 300~349 »çÀÌ)
			//	Äù½ºÆ®¼­¹EType 4) ¼­¹öID: XXXXºÎÄù½ºÆ®350~374,	À¯·´Äù½ºÆ®375~399	(Äù½ºÆ®¼­¹ö´Â 350~400 »çÀÌ)
			//	ÀÌ¼­¹öµéÀ» ÁÂ¿EÎ ³ª´²ÁÖ°úÙÍÀ¸¸E 25±âÁØÀ¸·Î  25º¸´Ù ÀÛÀ¸¸E¿ÞÂÊ, Å©¸E¿À¸¥ÂÊ
			//	ÀÌ¼­¹öµéÀ» »óÇÏ·Î ³ª´²ÁÖ°úÙÍÀ¸¸E200±âÁØÀ¸·Î 200º¸´Ù ÀÛÀ¸¸EÀ§ÂÊ, Å©¸E¾Æ·¡ÂÊ

			// À¯·´¼­¹ö¸¦ ÇÑÄ­ ¹ØÀ¸·Î ÇØÁØ´Ù. 
			// ³ªÁß¿¡ 4°³·Î ³ª´©°Ô µÈ´Ù¸E¿©±â¼­ Ã³¸®ÇØÁÖ¸éµÈ´Ù.
			//		-------------------------
			//		|	¼­ºÎ	|	µ¿ºÎ	|
			//		|	nRow=0	|	nRow=1	|
			//		|	nCol=0 	|	nCol=0	|
			...
			//nPeerPort = pss->m_nPeerPort;

#elif defined(LOCALE_NHNUSA) || defined(_DEBUG)
			//		-------------------------
			//		|			|	Å¬·£ÀE|
			//		|	ÀÏ¹Ý¼­¹ö|	nRow=1	|
			//		|	nRow=0 	|	nCol=0	|
			...
			//		-------------------------
			// º¸±âÆúãÏ°Ô ÇÏµåÄÚµù~~
			// ÀÏ¹Ý¼­¹E1~50, ·¹º§¼­¹E51~100, Å¬·£ÀE101~150, Äù½ºÆ®¼­¹E151~200, ÀÚÀ¯¼­¹E201~250
			...
			//pServerList->AddServer( "Billcap", "127.0.0.1", 6000, 4, 0, 1000, true, "127.0.0.1");

			// Custom: Auto-set server once the server-list is received.
			if( nCurrSel == -1 && pss->m_bIsLive )
				nCurrSel = nLiveIndex;
			else
				++nLiveIndex;
...
#ifdef _DEBUG
			// ¼­¹E·Î±× ³²±E
			    mlog( "ServerList - ID:%d, Name:%s, IP:%d, Port:%d, Type:%d, (%d/%d)\n",
...
void ZGameInterface::OnResponseBlockCountryCodeIP( const char* pszBlockCountryCode, const char* pszRoutingURL )
{
	...
	{
		// ºúÓ°µÈ IPÀÇ ±¹°¡¸¦ Å¬¶óÀÌ¾ðÆ®¿¡ ÅE¸ÇØÁà¾ß ÇÔ.
...
*/
	...
	if (ZApplication::GetInstance()->IsLaunchTest())				// Å×½ºÆ® ·±ÃÄÀÏ °æ¿E
...
void ZGameInterface::OnDisconnectMsg( const DWORD dwMsgID )
{
	...
	if( 0 != pListBox )
	{
		// ÇØÅ· °EÃ ¸Þ½ÃÁöÀÏ °æ¿E£ Á¾·E¿¹¾àÀ» ÇÑ´Ù.
		if ( (dwMsgID == MERR_FIND_HACKER) || (dwMsgID == MERR_BLOCK_HACKER) 
			...
			|| (dwMsgID == MERR_COMMAND_INVALID_VERSION)) // Custom: Added invalid version
		{
...
void ZGameInterface::PlayVoiceSound( char* pszSoundName, DWORD time)
{
	...
	// ÇÃ·¹ÀÌ ÁßÀÎ »ç¿ûÑå°¡ ¾øÀ¸¸E¹Ù·Î ÇÃ·¹ÀÌÇÑ´Ù.
...
void ZGameInterface::UpdateLadderWarsMyCharInfo()
{
	ZIDLResource* pRes = ZApplication::GetGameInterface()->GetIDLResource();
	if (!pRes) return;

	const ZGameClient::LADDERWARSCHARINFO* pCharInfo = GetGameClient()->GetMyLadderWarsCharInfo();
	if (!pCharInfo) return;
	...
	MLabel* pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsRanking");
	pLabel->SetText(szOutput);
	ZBmNumLabel* pBmNumLabelRankin = (ZBmNumLabel*)m_IDLResource.FindWidget("Lobby_LadderWarsRanking");
	if (pBmNumLabelRankin)
		pBmNumLabelRankin->SetCharMargin(nMargin);

	// Wins
	sprintf(szOutput, "%d", pCharInfo->Wins);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsWins");
	pLabel->SetText(szOutput);
	ZBmNumLabel* pBmNumLabelWins = (ZBmNumLabel*)m_IDLResource.FindWidget("Lobby_LadderWarsWins");
	if (pBmNumLabelWins)
		pBmNumLabelWins->SetCharMargin(nMargin);

	// Losses
	sprintf(szOutput, "%d", pCharInfo->Losses);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsLosses");
	pLabel->SetText(szOutput);
	ZBmNumLabel* pBmNumLabelLosses = (ZBmNumLabel*)m_IDLResource.FindWidget("Lobby_LadderWarsLosses");
	if (pBmNumLabelLosses)
		pBmNumLabelLosses->SetCharMargin(nMargin);

	// Draw  //Elo
	if (pCharInfo->Score >= 0 && pCharInfo->Score <= 10)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 10 ) / 2 );
	}
	else if (pCharInfo->Score >= 11 && pCharInfo->Score <= 30)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 30) / 2);
	}
	else if (pCharInfo->Score >= 31 && pCharInfo->Score <= 50)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 50) / 2);
	}
	else if (pCharInfo->Score >= 51 && pCharInfo->Score <= 80)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 80) / 2);
	}
	else if (pCharInfo->Score >= 81 && pCharInfo->Score <= 120)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 120) / 2);
	}
	else if (pCharInfo->Score >= 121 && pCharInfo->Score <= 170)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 170) / 2);
	}
	else if (pCharInfo->Score >= 171 && pCharInfo->Score <= 230)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 230) / 2);
	}
	else if (pCharInfo->Score >= 231)
	{
		sprintf(szOutput, "%d", (pCharInfo->Wins - pCharInfo->Losses + 300) / 2);
	}
	pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsDraws");
	pLabel->SetText(szOutput);
	ZBmNumLabel* pBmNumLabelDraws = (ZBmNumLabel*)m_IDLResource.FindWidget("Lobby_LadderWarsDraws");
	if (pBmNumLabelDraws)
		pBmNumLabelDraws->SetCharMargin(nMargin);

	// Score
	sprintf(szOutput, "%d", pCharInfo->Score);
	pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsScore");
	pLabel->SetText(szOutput);
	ZBmNumLabel* pBmNumLabelScore = (ZBmNumLabel*)m_IDLResource.FindWidget("Lobby_LadderWarsScore");
	if (pBmNumLabelScore)
		pBmNumLabelScore->SetCharMargin(nMargin);

	// Name
	sprintf(szOutput, "%s", ZGetMyInfo()->GetCharName());
	pLabel = (MLabel*)pRes->FindWidget("Lobby_LadderWarsName");
	pLabel->SetText(szOutput);

	// My Rank Emblem
	GetTierIconFile(szOutput, pCharInfo->Score);
	MPicture* pPicture = (MPicture*)pRes->FindWidget("Lobby_LadderWarsEmblem");
	if (pPicture) {
		pPicture->SetBitmap(MBitmapManager::Get(szOutput));

		char sz[32];
		char szTooltip[256];
		sprintf(sz, "%d", pCharInfo->Score);
		ZTransMsg(szTooltip, MSG_LOBBY_DUELTOURNAMENT_RECORD_TOOPTIP_GRADE, 1, sz);
		SetWidgetToolTipText("Lobby_LadderWarsEmblem", szTooltip, MAM_VCENTER);
	}
...
void ZGameInterface::UpdateDuelTournamantMyCharInfoPreviousUI()
{
	...
	if (pNumLabel)
	{
		pNumLabel->SetIndexOffset(16);		// »ö´Ù¸¥ ±Û¾¾·Î ÂEÂ´Ù
		pNumLabel->SetText(szOutput);
	}
	sprintf(szOutput,"%d",pCharInfo->tournamentPoint);
...
{
	m_Weapons.push_back(new Emoji('\xbd', 'B', "suicidio.png"));
...
int MDrawContext::TextMultiLine(MRECT& r, const char* szText, int nLineGap, bool bAutoNextLine, int nIndentation, int nSkipLine, MPOINT* pPositions)
{
	...
	do {
		...
		if (ZGetGame() && ZGetConfiguration()->GetEtc()->bChatBackground && !ZGetConfiguration()->GetEtc()->bFeed)
		{
				...
				for (int i = 0; i < nCharCount; i++)
				{
					...
					for (Emojis::iterator emoji = m_Weapons.begin(); emoji != m_Weapons.end(); emoji++) {
						...
						if (szCurrent[i] == (*emoji)->A && szCurrent[i + 1] == (*emoji)->B) {
							...
							SetBitmap(MBitmapManager::Get((*emoji)->name));
							int nSize = 24;
							//Draw((r.x + nX) + ArmasxRight, y - 15, 200, 60);
