...
public:
	...
	bool InitAccountInfo(const char* szAccountID, MMatchUserGradeID nUGradeID, MMatchPremiumGradeID nPGradeID, int nCountryFlag, int nCash, int nEvent);
	...
	int                 GetEV() { return m_nEvent; }
	int					GetCash() { return m_nCash; }
	int					GetHP();
	int					GetAP();
	int                 GetKill() { return m_nTotalKillCount; }
	int                 GetDeath() { return m_nTotalDeathCount; }
...
#ifdef _NEWGRADE
			(GetUGradeID() == MMUG_HIDE_ADMIN) ||
#endif
			(GetUGradeID() == MMUG_EVENTTEAM) ||
			(GetUGradeID() == MMUG_GAMEMASTER) ||
	...
	bool				IsEventGrade()
	{
		if ((GetUGradeID() == MMUG_EVENT1 ||
			...
			GetUGradeID() == MMUG_BOOSTER ||
			GetUGradeID() == MMUG_WINTOUR ||
	...
	void SetKill(int nKillCount) { m_nLC = m_nTotalKillCount; }
	void SetDeath(int nDeathCount) { m_nLC = m_nTotalDeathCount; }

	void SetLC(int nLC)                         { m_nLC = nLC; }
	void SetCash(int nCash)						{ m_nCash = nCash; }
	void SetEV(int nEvent)					    { m_nEvent = nEvent; }
