...
float __forceinline ZObject::OnDamagedCalc(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	ZModule_HPAP* pModule = (ZModule_HPAP*)GetModule(ZMID_HPAP);
	...
	return pModule->OnDamageCalc(pAttacker ? pAttacker->GetUID() : MUID(0, 0), fDamage, fPiercingRatio);
}
...
void ZObject::ShiftFugitiveValues()
{
	...
	if (m_pVMesh)
	{
		// Custom: Disable m_pVMesh antihack
		/*
		// AniFramInfo
		{
