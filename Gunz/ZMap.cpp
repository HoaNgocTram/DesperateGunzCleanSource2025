...
bool InitMaps(MWidget *pWidget)
{
...
#ifdef _MAGICBOX
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_DROPMAGIC)
	{

		for (auto& MagicMap : MGetMagicMap()->GetMagicMaps())
		{
			pCombo->Add(MagicMap.mapName);
		}
		return true;
	}
#endif
	...
	{
		for (auto& PaintMap : MGetPaintMap()->GetPaintMaps())
		{
			pCombo->Add(PaintMap.mapName);
		}
		return true;
	}

	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_PAINTBALL_TEAM)
	{
	  for (auto& PaintMap : MGetPaintMap()->GetPaintMaps())
		{
			pCombo->Add(PaintMap.mapName);
...
//#endif
		...
		if (bLGHMode)
		{
			if (pRule->CheckLightMap(MGetMapDescMgr()->GetMapName(i)))
			{
				if (strcmp(MMATCH_MAPNAME_RELAYMAP, MGetMapDescMgr()->GetMapName(i)) != 0)
					pCombo->Add(MGetMapDescMgr()->GetMapName(i));
			}
...
#ifdef _LIGHTURNOFF
		if ( pRule->CheckMap( MGetMapDescMgr()->GetMapName(i), bDuelMode) && !bCTFMode && !bLGHMode)
#else
