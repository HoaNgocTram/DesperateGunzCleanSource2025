...
public:
	enum ZLAUNCH_MODE {
		...
		ZLAUNCH_MODE_STANDALONE_DEVELOP,		// °³¹ßÀÚ ¸ðµå		// °ð »ç¶óÁú°ÍÀÓ
...
private:
	...
	GunzState				m_nInitialState;			///< ÃÊ±â »óÅÂ
	GunzState				m_nNextState;				///< reset an toàn
	ZStageInterface*		m_pStageInterface;
	ZOptionInterface*		m_pOptionInterface;
	ZLAUNCH_MODE			m_nLaunchMode;
    char					m_szFileName[_MAX_PATH];	// ¸®ÇÃ·¹ÀÌ È¤Àº standalone ¸ÊÀÌ¸§
	char					m_szCmdLine[ 4096 ];			// ÃÊ±â Ä¿¸Çµå¶óÀÎ ÀúÀå
	UINT					m_nTimerRes;
	MDataChecker			m_GlobalDataChecker;
	bool					m_bLaunchDevelop;			///< °³¹ßÀÚ¹öÀü ¿É¼Ç
	bool					m_bLaunchTest;				///< Å×½ºÆ®¹öÀü ¿É¼Ç
...
protected:
	...
	static ZSkillManager	m_SkillManager;				///< ½ºÅ³ ¸Å´ÏÀú

	//¾Ö´Ï¸ÞÀÌ¼Ç ÀÌº¥Æ® ¸Þ´ÏÀú...
...
public:
	...
	void SetNextState(GunzState s) { m_nNextState = s; }
	GunzState GetNextState() const { return m_nNextState; }
	bool GetSystemValue(const char* szField, char* szData);		///< ·¹Áö½ºÆ®¸®ÀÇ °ªÀ» ÀÐ¾î¿È
	void SetSystemValue(const char* szField, const char* szData);		///< ·¹Áö½ºÆ®¸®¿¡ ÀúÀå
	...
	void OnInvalidate();
	void OnRestore();
	static void ResetTimer();	// Ã¹ update ¸¦ ºÎ¸£±â Àü¿¡ ºÒ·¯ÁØ´Ù.
	static void Exit();
	void RestartGame();
	/// Singleton Instance
