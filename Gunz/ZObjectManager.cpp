...
bool ZObjectManager::DrawObject(ZObject* pObject)
{
	if ((pObject->GetInitialized()) && (pObject->IsVisible()))
	{
		...
		{
			{
				ZCharacterObject* pObj = dynamic_cast<ZCharacterObject*>(ZGetObjectManager()->GetObject(pObject->GetUID()));
				if (pObj && !pObj->IsDie())
				{
					pObj->DrawShadow();
				}
...
void ZObjectManager::Draw()
{
	...
	ZObject* pMyCharacter = (ZObject*)ZGetGame()->m_pMyCharacter;
	...
	{ 
		...
		if(pVMesh) {
			...
			if(pVMesh->GetVisibility() != 1.f) {
				...
				pVMesh->SetSpRenderMode(PASS_DEPTH_ONLY);

				DrawObject( pMyCharacter );

				pVMesh->SetSpRenderMode(PASS_COLOR);
			}

			pMyCharacter->m_bRendered = DrawObject( pMyCharacter );

			pVMesh->SetSpRenderMode(PASS_NORMAL);//일반 모드로 바꿔줘야한다..꼬옥..
		}
...
ZObject* ZObjectManager::Pick( rvector& pos, float Radius )
{
	...
	for( iterator iter = begin(); iter != end(); ++iter )
	{
		...
		float temp =  D3DXVec3Length(&(pObject->GetPosition() - pos));

		if( D3DXVec3Length(&(pObject->GetPosition() - pos)) < best_dist )
		{
			pPickObject	= pObject;
		}
	}
