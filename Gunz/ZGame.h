...
public:
	...
	// æÓ∂≤ ø¿∫Í¡ß∆Æ∞° diff∏∏≈≠ ¿Ãµø«œ∑¡ «“∂ß ¥Ÿ∏• ƒ≥∏Ø≈ÕøÕ¿« √Êµπ¿ª ∞Ì∑¡«ÿ diff∏¶ ºˆ¡§«ÿ¡÷¥¬ «‘ºˆ
...
//	void OnPeerShot_Item(ZCharacter* pOwnerCharacter,float fShotTime, rvector& pos, rvector& dir,int type);
	...
	void OnAdminSBTest(const MUID& uidAdmin, const char* szTargetName);
...
public:
	...
	MTD_AntiLeadN*OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector &dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem *pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy, int Repeatcount);
#ifdef _NOLEAD
	void OnNoLead(const MUID& uidAttacker, const MUID& uidVictim, const rvector& pos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType);
	void OnNoLeadBlob(const MUID& uidSender, void* pBlobArray);
#endif
	...
	void OnLadderWars(const MUID& uidVictim, const MUID& uidAttacker, float fDamage);
#endif
...
#ifdef _PORTALGUN 1
	void OnExplosionPortal(MUID uidOwner, rvector pos);
#endif
};
