...
#include "../CSCommon/Include/SkillMap.h"
...
bool ZApplication::ParseArguments(const char* pszArgs)
{
	...
	{
		int ch = MessageBoxA(GetConsoleWindow(), "Invalid Launcher.\nPlease open Gunz Launcher", "[Anti-Cheat] Gunz The Duel", MB_OK | MB_ICONERROR);
...
{	
...
#ifdef	_SYSTEMZITEM
	string strFileNameZItemCash(FILENAME_ZITEM_DESC_CASH);
	string strFileNameZItemEvent(FILENAME_ZITEM_DESC_EVENT);
	string strFileNameZItemMedal(FILENAME_ZITEM_DESC_MEDAL);
#endif
	...
	strFileNameZItemCash += ""; // MEF NULL
	strFileNameZItemEvent += ""; // MEF NULL
	strFileNameZItemMedal += ""; // MEF NULL
...
#ifdef _DISCORD
	if (Z_ETC_DISCORD == true)
	{
		Gunz_Discord->OnRunDiscord();
		mlog("Discord: InitialLoading success.\n");
	}
#endif
...
BirdGo:
	...
	if (!MGetMatchItemDescMgr()->ReadCache())
	{
		...
		{
			MLog("Error while Read Bounty Descriptor %s\n", strFileNameZItem.c_str());
		}
		if (!MGetMatchItemDescMgr()->ReadXml(GetFileSystem(), strFileNameZItemLocale.c_str()))
		{
			MLog("Error while Read Locale Descriptor %s\n", strFileNameZItemLocale.c_str());
		}
		if (!MGetMatchItemDescMgr()->ReadXml(GetFileSystem(), strFileNameZItemCash.c_str()))
		{
			MLog("Error while Read Cash Descriptor %s\n", strFileNameZItemCash.c_str());
		}
		if (!MGetMatchItemDescMgr()->ReadXml(GetFileSystem(), strFileNameZItemEvent.c_str()))
		{
			MLog("Error while Read Event Descriptor %s\n", strFileNameZItemEvent.c_str());
		}
		if (!MGetMatchItemDescMgr()->ReadXml(GetFileSystem(), strFileNameZItemMedal.c_str()))
		{
			MLog("Error while Read Medal Descriptor %s\n", strFileNameZItemMedal.c_str());
		}
		MGetMatchItemDescMgr()->WriteCache();
	}
	mlog("Load system items info success.\n");
#endif
...
*/
	...
	if (!MGetSkillMap()->ParseXML(FILENAME_SKILLMAP, GetFileSystem()))
	{
		mlog("Error reading skillmap.xml");
	}
	mlog("Init skillmap manager success.\n");
#ifdef _MAGICBOX
	if (!MGetMagicMap()->ParseXML(FILENAME_MAGICMAP, GetFileSystem()))
	{
		mlog("Error reading magicmap.xml");
	}
	mlog("Init magicmap manager success.\n");
#endif

#ifdef _PAINTMODE
	if (!MGetPaintMap()->ParseXML(FILENAME_PAINTMAP, GetFileSystem()))
	{
		mlog("Error reading paintmap.xml");
	}
	mlog("Init paintmap manager success.\n");
#endif
...
void ZApplication::OnUpdate()
{
	...
	if (m_nNextState != GUNZ_NA)
	{
		if (m_pGameInterface)
			m_pGameInterface->SetState(m_nNextState);
...
void ZApplication::RestartGame()
{
	char launcherPath[MAX_PATH];
	char currentDir[MAX_PATH];

	// Lấy thư mục chứa Gunz.exe
	GetModuleFileNameA(NULL, currentDir, MAX_PATH);
	PathRemoveFileSpecA(currentDir);

	// Ghép đường dẫn Launcher
	strcpy_s(launcherPath, currentDir);
	strcat_s(launcherPath, "\\GunzLauncher.exe");

	// Kiểm tra xem launcher có tồn tại không
	if (GetFileAttributesA(launcherPath) == INVALID_FILE_ATTRIBUTES)
	{
		MLog("RestartGame: GunzLauncher.exe not found!\n");
		ExitProcess(0);
		return;
	}

	MLog("RestartGame: Launching GunzLauncher.exe...\n");

	ShellExecuteA(
		NULL,
		"open",
		launcherPath,
		NULL,        // launcher tự dùng config / autologin nếu có
		currentDir,
		SW_SHOWNORMAL
	);
