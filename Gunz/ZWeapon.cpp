...
#define ROCKET_VELOCITY_GUIDED	2700.f
...
#ifdef _KILLFEED
	ZPostLastWeaponUsed(MWT_ROCKET);
#endif
...
#ifdef _KILLFEED
	ZPostLastWeaponUsed(MWT_FRAGMENTATION);
#endif
...
#ifdef _PORTALGUN 1
MImplementRTTI(ZWeaponPortal, ZMovingWeapon);

ZWeaponPortal::ZWeaponPortal() :ZMovingWeapon() {
	m_bSendMsg = false;
	m_nTypePortal = 0;
}

ZWeaponPortal::~ZWeaponPortal() {
}

void ZWeaponPortal::Create(RMesh* pMesh, rvector& pos, rvector& dir, ZObject* pOwner, int nType) {
	ZWeapon::Create(pMesh);
...
bool ZWeaponPortal::Update(float fElapsedTime)
{
	rvector oldPos = m_Position;
...
void ZWeaponPortal::Render()
{
	ZWeapon::Render();
}

void ZWeaponPortal::Explosion()
{
	rvector v = m_Position;

	rvector dir = -RealSpace2::RCameraDirection;
	if (m_uidOwner == ZGetGameClient()->GetPlayerUID() && !m_bSendMsg && m_nTypePortal == 0) {
		ZPostRequestSpawnGamePortal(m_uidOwner, PORTAL_BLUE_ID, v, dir);
		m_bSendMsg = true;
	}

	if (m_uidOwner == ZGetGameClient()->GetPlayerUID() && !m_bSendMsg && m_nTypePortal == 1) {
		ZPostRequestSpawnGamePortal(m_uidOwner, PORTAL_RED_ID, v, dir);
		m_bSendMsg = true;
	}
}
#endif
