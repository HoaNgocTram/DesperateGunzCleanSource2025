...
//Custom: LadderWar SPawn Defines
#define ZTOK_SPAWN_TEAM1 "spawn_team1"
#define ZTOK_SPAWN_TEAM2 "spawn_team2"
...
bool ZMapSpawnManager::Add(ZMapSpawnData* pMapSpawnData)
{
	...
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_TEAM, strlen(ZTOK_SPAWN_TEAM)))
	{
		if (strlen(pMapSpawnData->m_szSpawnName) > 11)
		{
			...
			else if (pMapSpawnData->m_szSpawnName[11] == 'r')
			{
				pMapSpawnData->m_nType = ZMST_FLAGRED;
				m_SpawnArray[ZMST_FLAGRED].push_back(pMapSpawnData);
			}
			else if (pMapSpawnData->m_szSpawnName[11] == 'b')
			{
				pMapSpawnData->m_nType = ZMST_FLAGBLUE;
				m_SpawnArray[ZMST_FLAGBLUE].push_back(pMapSpawnData);
			}
			else if (pMapSpawnData->m_szSpawnName[10] == 'f')
			{
				pMapSpawnData->m_nType = ZMST_FLAGRED;
				m_SpawnArray[ZMST_FLAGRED].push_back(pMapSpawnData);
			}
	...
	{
		// spawn_npc_xxxx 처럼 스폰노드 이름을 임의로 정하는 것들을 이름별로 리스트에 넣는다
...
ZMapSpawnData* ZMapSpawnManager::GetTeamData(int nTeamIndex, int nDataIndex)
{
	...
	{
		// Custom: Remove this annoying _ASSERT
		//_ASSERT(0);
		return NULL;
	}
...
{
	// nCustomSpawnIndex: 커스텀스폰 지점들을 이름으로 정렬한 목록에서의 인덱스
	// nIndex: 해성 커스텀스폰 명을 가진 지점들의 배열 인덱스
