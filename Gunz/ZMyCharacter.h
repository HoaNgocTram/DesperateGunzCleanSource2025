...
#ifdef _PORTALGUN 1
	float	m_fLastPortalTime;
#endif

	MProtectValue<float>	m_fStunEndTime;				///< stun 풀리는 시간, 5번타입을 위함

	DWORD	m_dwLastDamageTime;			// Custom: Shield mode
	DWORD	m_dwLastSprintTime;			// Custom: FPS mode
	bool	m_bSprint;					// Custom: FPS mode
	int		m_nSprintFactor;			// Custom: FPS mode
...
public:
	...
	bool IsDirLocked()
	{
		...
		return (zStatus.m_bSkill || uStatus.m_bWallJump || uStatus.m_bWallJump2 || zStatus.m_bWallHang ||
