...
void ZShop::Serialize()
{
	...
	for (int i=0; i<GetItemCount(); i++)
	{
		...
		if (MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(m_vShopItem[i]->nItemID))
		{
			...
			{
				if (pDesc->itemClass() == ITEM_MEDAL || pDesc->itemClass() == ITEM_CASH || pDesc->itemClass() == ITEM_EVENT) continue;
			}
			else if (GetShopType() == ZSHOP_LADDER)
			{
				if (pDesc->itemClass() == ITEM_CASH || pDesc->itemClass() == ITEM_BOUNTY || pDesc->itemClass() == ITEM_EVENT) continue;
			}
			else if (GetShopType() == ZSHOP_EVENT)
			{
				if (pDesc->itemClass() == ITEM_MEDAL || pDesc->itemClass() == ITEM_CASH || pDesc->itemClass() == ITEM_BOUNTY) continue;
			}
		...
		else if (MQuestItemDesc* pDesc = GetQuestItemDescMgr().FindQItemDesc( m_vShopItem[i]->nItemID))
		{
			...
			if (ZGetShop()->GetShopType() == ZSHOP_LADDER) continue;
			if (ZGetShop()->GetShopType() == ZSHOP_EVENT) continue;
	...
	for (int i=0; i<GetItemCount(); i++)
	{
		...
		if (MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(m_vShopItem[i]->nItemID))
		{
			...
			if (CheckTypeWithListSubFilter(pDesc->m_nWeaponType.Ref()) == false) continue;
			if (pDesc->m_nResSex.Ref() != -1 && pDesc->m_nResSex.Ref() != int(ZGetMyInfo()->GetSex())) continue;
			if (pDesc->itemClass() == ITEM_MEDAL || pDesc->itemClass() == ITEM_BOUNTY || pDesc->itemClass() == ITEM_EVENT) continue;
