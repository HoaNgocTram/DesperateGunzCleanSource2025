...
public:
	enum {
		...
		FSMFUNC_SPEED_ACCEL,
		FSMFUNC_TURN_ORBITAL_DIRECTION,
		FSMFUNC_CLEAR_WAYPOINTS,
		FSMFUNC_SPAWN_NPC,
		FSMFUNC_FIND_TARGET_IN_DIST,
		FSMFUNC_RUN_WAYPOINTS_ALONG_ROUTE,
...
protected:
	...
	virtual void Func_FindTargetInHeight(int nHeight) { _ASSERT(0); }
	virtual void Func_RunAlongTargetOrbital(float fFrameDelta,int nArg1) { _ASSERT(0); }
	virtual void Func_SpeedAccel(int nArg1) { _ASSERT(0); }
	virtual void Func_TurnOrbitalDir() { _ASSERT(0); }
	virtual void Func_ClearWayPoints() { _ASSERT(0); }
	virtual void Func_FindTargetInDist(int nArg1) { _ASSERT(0); }
	virtual void Func_RunWayPointsAlongRoute(float fFrameDelta) { _ASSERT(0); }
