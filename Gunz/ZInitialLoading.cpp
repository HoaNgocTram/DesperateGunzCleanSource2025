...
#ifdef _DEBUG // È¤½Ã µÇµ¹¾Æ°¡´Â°æ¿EÎÁEÃ¼Å©
	_ASSERT(m_fLastProgress <= fTotalProgress);
	m_fLastProgress = fTotalProgress;
#endif

	ZGetInitialLoading()->SetLoadingStr(m_strName);
...
void	ZInitialLoading::Draw(LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_)
{
	...
	if (!m_bGradeFadeOutStart && timeGetTime() - m_Grade_Time > GRADE_TIMER - 2)	// 3ÃÊ µ¿¾È¸¸ º¸¿©ÁÜ
	{	// 3ÃÊÈÄ¿¡ 15¼¼ µî±Þ ÀÌ¹ÌÁö ÆäÀÌµå ¾Æ¿ôÃ³¸® ½ÃÀÛÇØ ÁÜ
		numDrawing = FADEOUT_NUM_DRAWING;
	...
	for (int i = 0; i < numDrawing; ++i)
	{
		...
		//	if( mode_ == MODE_FADEOUT ) //Loading Bar
		{
			/*DrawBar((float)(RGetScreenWidth() * 0.1162109375), (float)(RGetScreenHeight() * 0.6653645833333333f),
				(float)(RGetScreenWidth() * 0.768554688f), (float)(RGetScreenHeight() * 0.0065104166666667f), mPercentage * 0.01f);*/
		...
		//if (m_strLoadingStr.length() > 0 && mpDC) {
		//	char buffer[256];
		//	// Custom: memset buffer
		//	memset(buffer, 0, sizeof(buffer));

		//	sprintf(buffer, "Loading %s ...", m_strLoadingStr.c_str());
		//	//sprintf(buffer, "Loading Game...", m_strLoadingStr.c_str());
		...
		if (mpDC) {
			...
			std::string safeCopy = m_strLoadingStr; // ✅ bản copy an toàn

			if (safeCopy.empty())
				sprintf_s(buffer, sizeof(buffer), "Loading...");
			else
				sprintf_s(buffer, sizeof(buffer), "Loading %s...", safeCopy.c_str());

			int nWidth = mpDC->m_Font.GetTextWidth(buffer);
			int x = (int)(RGetScreenWidth() * 0.5f) - nWidth / 2;
			int y = (int)(RGetScreenHeight() * 0.9f);
			mpDC->m_Font.DrawText(x, y, buffer, 0xFFCCCCCC);
		}
#endif
...
{
	if (!mbUseEnable) return;	// È°¼ºÈ­ µÇ¾EÀÖÁE¾ÊÀ¸¸Eskip
...
void	ZInitialLoading::DrawTips()
{
	...
	if ((g_pDC != 0) && (pFont != 0))
	{
		...
		else
		{
			...
			// ·ÎµEÈ­¸é¿¡ ¸ÂÃç¼­ ÅØ½ºÆ®¸¦ ¼³Á¤ÇÑ´Ù.(ÇÏµåÄÚµE.. -_-;)
			int nPosx, nPosy;
...
ZLoadingProgress::ZLoadingProgress(string strName, ZLoadingProgress* pParent, float fRatio)
{
	m_strName = strName;
	m_pParent = pParent;
