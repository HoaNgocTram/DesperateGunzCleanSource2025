...
#ifdef _DYNAMIC
bool ZCharacterItem::EquipItem(MMatchCharItemParts parts, int nItemDescID, int nItemCount, MMatchSex playerSex, bool bValidationCheck)
{
	if (nItemDescID == 0) {
...
void ZCharacterItem::SelectWeapon(MMatchCharItemParts parts)
{
	_ASSERT( (parts == MMCIP_MELEE)   || (parts == MMCIP_PRIMARY) || (parts == MMCIP_SECONDARY) 
			|| (parts == MMCIP_CUSTOM1) || (parts == MMCIP_CUSTOM2) || (parts == MMCIP_CUSTOM3));
...
ZItem* ZCharacterItem::GetSelectedWeapon()
{
	...
	if(m_nSelectedWeapon < MMCIP_MELEE || m_nSelectedWeapon > MMCIP_CUSTOM3) return NULL;
...
bool ZCharacterItem::IsWeaponItem(MMatchCharItemParts parts)
{
	if ((parts == MMCIP_MELEE)		||
		...
		(parts == MMCIP_CUSTOM1 )	||
		(parts == MMCIP_CUSTOM2)    ||
		(parts == MMCIP_CUSTOM3 ) ) return true;
...
bool ZCharacterItem::Load(ZFile *file, int nReplayVersion)
{
	...
	struct Converter
	{
		static int v0_to_v6(int v0)
		{
			...
			case MMCIP_CUSTOM3_V0:		return MMCIP_CUSTOM3;
			}
			return MMCIP_END;
		}
