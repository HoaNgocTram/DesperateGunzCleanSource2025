...
//	pObj->SetFriendInfo(pJob->GetFriendInfo());	// Save Async Result
	...
	//Ladder--------------------------------------------------------------------------------
	SendLadderWarsCharInfoToPlayer(pJob->GetUID());
	SendLadderWarsSideRankingToPlayer(pJob->GetUID());
...
void MBMatchServer::OnAsyncBuyBountyItem( MAsyncJob* pJobResult )
{
	...
	{
	case SHOP_CASH:
		pAccountInfo->m_nCash -= dwPrice;
		break;
	case SHOP_LADDER:
		pCharInfo->m_nLC -= dwPrice;
		break;
	case SHOP_EVENT:
		pAccountInfo->m_nEvent -= dwPrice;
		break;
	default:
...
void MBMatchServer::OnAsyncBringAccountItem(MAsyncJob* pJobResult)
{
	...
	pObj->GetCharInfo()->m_ItemList.CreateItem(MMatchItemMap::UseUID(), noutCIID, noutItemID, 
		boutIsRentItem, noutRentMinutePeriodRemainder, woutRentHourPeriod, 1);
