...
void MBMatchServer::OnRequestBuyItem(const MUID& uidPlayer, const unsigned long int nItemID, const int nItemCount)
{
	...
	MSHOPMODE nShopMode = SHOP_NORMAL;
	...
	if( MGetMatchShop()->IsSellItem(nItemID) )
	{
		...
		//nItemType = pItemDesc->m_nCurrency.Ref();
		nShopMode = (MSHOPMODE)pItemDesc->m_nCurrency.Ref();
		//LOG(LOG_PROG, "SHOP VALUE %i \n", nShopMode);
		//nItemType = (MSHOPMODE)nShopMode;
	}
	else
	{
	...
	if( !CheckUserCanBuyItem(pObj, nItemID, nItemCount, dwPrice, nShopMode) ) { return; }

	ResponseBuyItem( pObj, nItemID, nItemCount, bIsGambleItem, dwPrice, nRentHourPeriod, bIsSpendableItem, nShopMode);
}

bool MBMatchServer::ResponseBuyItem( MMatchObject* pObj, const unsigned long int nItemID, const int nItemCount
									, const bool bIsGambleItem, const DWORD dwPrice, const WORD wRentHourPeriod, const bool bIsSpendableItem, MSHOPMODE nShopMode )
{
	MMatchCharInfo* pCharInfo = pObj->GetCharInfo();
...
const bool MBMatchServer::CheckUserCanBuyItem( MMatchObject* pObj, const int nItemID, const int nItemCnt, const DWORD dwPrice, MSHOPMODE nShopMode )
{
...
#ifdef _SHOPFIXCANT
	if (nShopMode == SHOP_NORMAL)// 1
	{
		if (static_cast<DWORD>(pCharInfo->m_nBP) < dwPrice) //1
		{
			MCommand* pNew = CreateCommand(MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID());
	...
	if (static_cast<DWORD>(pCharInfo->m_nBP) < dwPrice) //1
	{
		LOG(LOG_PROG, "passing by if (nShopMode == SHOP_NORMAL)");
		MCommand* pNew = CreateCommand(MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID());
		pNew->AddParameter(new MCmdParamInt(MERR_TOO_EXPENSIVE_BOUNTY));
		PostSafeQueue(pNew);
...
#endif
	else if (nShopMode == SHOP_CASH)// 2
	{
		if (static_cast<DWORD>(pAccountInfo->m_nCash) < dwPrice)
		{
			MCommand* pNew = CreateCommand( MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID() );
	...
	else if (nShopMode == SHOP_LADDER)// 3
	{
		if (static_cast<DWORD>(pCharInfo->m_nLC) < dwPrice)
		{
			MCommand* pNew = CreateCommand(MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID());
			pNew->AddParameter(new MCmdParamInt(MERR_TOO_EXPENSIVE_MEDAL));
			PostSafeQueue(pNew);

			return false;
		}
	}
	else if (nShopMode == SHOP_EVENT)// 4
	{
		if (static_cast<DWORD>(pAccountInfo->m_nEvent) < dwPrice)
		{
			MCommand* pNew = CreateCommand(MC_MATCH_RESPONSE_BUY_ITEM, pObj->GetUID());
...
bool MBMatchServer::ResponseCharacterItemList(const MUID& uidPlayer)
{
	...
	pNew->AddParameter(new MCommandParameterInt(pObj->GetCharInfo()->m_nLC));
	pNew->AddParameter(new MCommandParameterInt(pObj->GetAccountInfo()->m_nCash));
	pNew->AddParameter(new MCommandParameterInt(pObj->GetAccountInfo()->m_nEvent));
...
void MBMatchServer::ResponseTakeoffItem(const MUID& uidPlayer, const MMatchCharItemParts parts)
{
	...
	nMaxWeight = pCharInfo->m_nMaxWeight + nMaxWeight;

	nWeight -= pItemDesc->m_nWeight.Ref();

	if (nWeight > nMaxWeight)
		nResult = MERR_CANNOT_TAKEOFF_ITEM_BY_WEIGHT;
