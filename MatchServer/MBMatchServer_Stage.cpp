...
//	
//	// Custom: no killcount/deathcount gain in event channel.
//	MMatchChannel* pChannel = FindChannel(pStage->GetOwnerChannel());
//	if (pChannel && pChannel->IsUseEvent()) return;
//
//	// 캐릭터 XP 업데이트
...
void MBMatchServer::OnStageJoin(const MUID& uidChar, const MUID& uidStage)
{
	...
	{
		// Custom: Event Team
		if (pObj->GetAccountInfo()->m_nUGrade == MMUG_GAMEMASTER)
		{
			MMatchChannel* pOwnerChannel = FindChannel(pStage->GetOwnerChannel());
			// ugly way of checking
			if (pOwnerChannel && !pOwnerChannel->IsUseEvent() || !pOwnerChannel)
			{
				...
				pNew->AddParameter(new MCommandParameterString(pStage->GetName()));
...
int MBMatchServer::ValidateStageJoin(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	{
		if (pStage->GetStageType() == MST_LADDERWARS && pStage->LadderWarsTeam(pObj->GetCharInfo()->m_nCID) == MMT_ALL)
		{
			return MERR_CANNOT_JOIN_STAGE;
		}
		if (pStage->GetStageSetting()->GetMaxPlayers() <= pStage->GetCountableObjCount())
		{
			// Custom: VIP/Premium members can access full rooms, with a maximum of 2, MMPG_PREMIUM_IP
			if(pObj->GetAccountInfo()->m_nPGrade == MMPG_PREMIUM_IP && pStage->GetStageSetting()->GetMaxPlayers() + 2 < pStage->GetCountableObjCount() + 1)
				return MERR_CANNOT_JOIN_STAGE_BY_MAXPLAYERS;
			
			// Gametype check, MMPG_PREMIUM_IP
			if (pObj->GetAccountInfo()->m_nPGrade != MMPG_PREMIUM_IP || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_DEATHMATCH_SOLO || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_GLADIATOR_SOLO)
				return MERR_CANNOT_JOIN_STAGE_BY_MAXPLAYERS;
		}
		...
		// Custom: if AID = banned, don't allow player to join.
		if (pStage->CheckBanList(pObj->GetAccountInfo()->m_nAID))
			return MERR_CANNOT_JOIN_STAGE_BY_BAN;
	}
