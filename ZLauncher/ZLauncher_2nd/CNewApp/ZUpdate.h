...
public:
	ZUpdatePatchNode(const char* pszName, unsigned long nSize, string sChecksum);
...
public:
	...
	const char* GetSpeedDownload(int nSpeed)
	{
		string shit;
		size_t result;

		result = nSpeed;
		shit = convertSize(result);

		//PutLog2(shit.c_str());

		char * writable = new char[shit.size() + 1];
		std::copy(shit.begin(), shit.end(), writable);
		writable[shit.size()] = '\0'; // don't forget the terminating 0
	...
	const char* GetFileSize()
	{
		...
		result = (float)GetCurrDownloadSize();
		shit = convertSize(result);
	...
	const char* GetFileSizeComplete()
	{
		string shit;
		size_t result;

		result = (float)GetCurrPatchedFileSize();
		shit = convertSize(result);
	...
	const char* GetFileRest()
	{
		float _fPercent = (float)GetTotalPatchFileSize() - (float)GetCurrPatchedFileSize();
	...
	const char* GetTotalFiles()
	{
		string shit;
		size_t result;

		result = (float)GetTotalPatchFileSize();
		shit = convertSize(result);
