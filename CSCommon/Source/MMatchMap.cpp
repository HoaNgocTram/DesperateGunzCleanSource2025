...
#ifdef _LIGHTURNOFF
#define MMAP_ISLIGHTMAP		"bIsLightMaps"
#endif
...
bool MMapDesc::Initialize(const char* szFileName, MZFileSystem* pFileSystem)
{
...
#ifdef _LIGHTURNOFF
			aChild.GetAttribute(&m_MapVectors[num].bLightMapTurn, MMAP_ISLIGHTMAP);
#endif
...
#ifdef _LIGHTURNOFF
bool MMapDesc::IsLightMap(const int nMapID)
{
	if (MIsCorrectMap(nMapID))
		return m_MapVectors[nMapID].bLightMapTurn;
	else
		return 0;
}
#endif