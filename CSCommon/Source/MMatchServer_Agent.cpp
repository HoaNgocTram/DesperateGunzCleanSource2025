...
void MMatchServer::ReserveAgent(MMatchStage* pStage)
{
	...
	if (pFreeAgent == NULL) {
		...
		mlog("MMatchServer::ReserveAgent could not find free agent, (%d, %d)", pStage->GetUID().High, pStage->GetUID().Low);
		return;
	}
...
//}
#ifdef _AGENTPORT
void MMatchServer::OnRegisterAgent(const MUID& uidComm, char* szIP, char* AgentIpConfig, int nTCPPort, int nUDPPort, bool AgentBlock)
{
	if (!strstr(szIP, AgentIpConfig) && AgentBlock == true)
	{
		return;
	}
	LOG(LOG_PROG, "Start agent Register (CommUID %u:%u) IP:%s, TCPPort:%d, UDPPort:%d\n ",
...
{
	mlog("MMatchServer::OnPeerReady( (%d, %d) -> (%d, %d) )", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);
	MMatchObject* pChar = GetObject(uidChar);
	if (pChar == NULL) {
		mlog("MMatchServer::OnPeerReady could not find char, (%d, %d) -> (%d, %d)", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);
		return;
	}

	MMatchStage* pStage = FindStage(pChar->GetStageUID());
	if (pStage == NULL) {
		mlog("MMatchServer::OnPeerReady could not find stage, (%d, %d) -> (%d, %d)", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);
		return;
	}
	...
	//if( 10 > nCount++ )
	{
		...
		mlog("MMatchServer::OnPeerReady done, %s -> (%d, %d)", pChar->GetName(), uidPeer.High, uidPeer.Low);
	}
}
...
{
	mlog("MMatchServer::OnRequestRelayPeer( (%d, %d), (%d, %d) )", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);

	MMatchObject* pChar = GetObject(uidChar);
	if (pChar == NULL) {
		mlog("MMatchServer::OnRequestRelayPeer could not find char, (%d, %d) -> (%d, %d)", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);
		return;
	}

	MMatchObject* pPeer = GetObject(uidPeer);
	if (pPeer == NULL) {
		mlog("MMatchServer::OnRequestRelayPeer could not find peer, (%d, %d) -> (%d, %d)", uidChar.High, uidChar.Low, uidPeer.High, uidPeer.Low);
		return;
	}
	...
	MMatchStage* pStage = FindStage(pChar->GetStageUID());
	if (pStage == NULL) {
		mlog("MMatchServer::OnRequestRelayPeer could not find stage, %s -> %s", pChar->GetName(), pPeer->GetName());
		return;
	}
	...
	if (pAgent == NULL)
	{
		...
		if (pAgent == NULL) 
		{
			mlog("MMatchServer::OnRequestRelayPeer FindFreeAgent() == NULL, %s -> %s", pChar->GetName(), pPeer->GetName());
	...
	mlog("MMatchServer::OnRequestRelayPeer done, %s -> %s", pChar->GetName(), pPeer->GetName());
