...
bool MDropGunGameXMLManager::ReadXml(MZFileSystem* pFileSystem, const char* szFileName)
{
	...
	if (pFileSystem)
	{
		if (!mzf.Open(szFileName, pFileSystem))
		{
			...
			{
				mlog("DropGunGame Can't Open File in MZFileSystem (Battle/dropgungame.xml)\n");
				xmlIniData.Destroy();
				return false;
			}
		}
	...
	else
	{
		if (!mzf.Open(szFileName))
		{
			mlog("DropGunGame Can't Open File (Battle/dropgungame.xml)\n");
			xmlIniData.Destroy();
			return false;
		}
	}
	...
	{
		mlog("DropGunGame Can't Load (Battle/dropgungame.xml) From Memory\n");
		xmlIniData.Destroy();
		return false;
	}
