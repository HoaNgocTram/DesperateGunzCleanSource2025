...
#ifdef _CW_VOTE
int MLadderGameStrategy::GetLadderWarsRandomMap(int nTeamMember)
{
	int nVecIndex = 0;
	int nMaxSize = 0;
	switch (nTeamMember)
	{
#ifdef _LADDERGAMETEST // Custom: Fix LadderWar dead matchserver
	case 1:
		nVecIndex = MLADDERTYPE_NORMAL_1VS1; // Custom: 1 vs 1
		break;
#endif // _LADDERGAME
	case 2:
...
{
#ifdef _LADDERGAMETEST // Custom: Fix LadderWar dead matchserver
	for (int i = MLADDERTYPE_NORMAL_1VS1; i <= MLADDERTYPE_NORMAL_2VS2; i++) // Custom: 1 vs 1
	{
...
#ifdef _BOXLEAD
	poutRedLadderInfo->bIsLead = pRedGroup->GetLead();
	poutBlueLadderInfo->bIsLead = pBlueGroup->GetLead();
#endif

#ifdef _ROTATION
	poutRedLadderInfo->bIsRoBase = pRedGroup->GetRoBase();
	poutBlueLadderInfo->bIsRoBase = pBlueGroup->GetRoBase();
#endif
}
		                               ...
		                               MMatchLadderTeamInfo* pRedLadderInfo, MMatchLadderTeamInfo* pBlueLadderInfo, int GameType)
{
	...
	for (MUIDRefCache::iterator itor=pStage->GetObjBegin(); itor!=pStage->GetObjEnd(); itor++) 
	{
		...
		if (IsEnabledObject(pObj))
		{
			...
			{
				// Custom: Check if winner members are part of clan (stage hacking)
			...
			{
				// Custom: Check if loser members are part of clan (stage hacking)
	...
	MMatchServer::GetInstance()->SaveClanPoint(pWinnerClan, pLoserClan, bIsDrawGame,
												nRoundWins, nRoundLosses, nMapID, GameType,
...
#ifdef _CW_VOTE
int MClanGameStrategy::GetLadderWarsRandomMap(int nTeamMember)
{
	int nVecIndex = 0;
	int nMaxSize = 0;
	switch (nTeamMember)
...
//	MLADDERTYPE_NORMAL_3VS3,
//	MLADDERTYPE_NORMAL_4VS4,
//	MLADDERTYPE_NORMAL_5VS5,		// Custom: 5vs5 clan wars
...
//// 각 래더타입별 필요한 인원수
//// Custom: 5vs5 clan wars
//const int g_nNeedLadderMemberCount[MLADDERTYPE_MAX] = { 2, 3, 4, 5/*, 8*/
//#ifdef _DEBUG
//, 1
//#endif
//};
