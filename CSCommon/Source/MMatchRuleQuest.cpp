														...
														m_nCombatStartTime(0), m_nQuestCompleteTime(0), m_nPlayerCount( 0 )
{
	...
	//Custom: iLevel Rarity Scaling default values.
	m_StageGameInfo.nScaling = 100; //100%
	m_StageGameInfo.bScaled = false; //Not scaled - a silly flag to not apply loot code, probably not needed but kept here for posterity's sake
	m_StageGameInfo.nHighestItemPerRound = 0;  //The highest iLevel appearing in the quest across all players
...
void MMatchRuleQuest::RouteGameInfo()
{
	...
	if (m_pQuestLevel)
	{
		...
		pGameInfoNode->nSectorIndexID = m_pQuestLevel->GetCurrSectorIndex();
	}
...
void MMatchRuleQuest::RouteCompleted()
{
	...
	{
		MQuestPlayerInfo* pPlayerInfo = (*itor).second;
		MMatchObject* pPlayer = MMatchServer::GetInstance()->GetObject((*itor).first);
		if( !IsEnabledObject(pPlayer) ) continue;
		pPlayerInfo->bLateJoined = false;
	}
...
void MMatchRuleQuest::RouteSectorBonus(const MUID& uidPlayer, unsigned long int nEXPValue, unsigned long int nBP, unsigned long int nPct)
{
	MMatchObject* pPlayer = MMatchServer::GetInstance()->GetObject(uidPlayer);	
	if ((!IsEnabledObject(pPlayer)) || (!pPlayer->CheckAlive())) return;
	...
	pNewCmd->AddParameter(new MCmdParamUInt(nPct));	// BP용이지만 퀘스트에서는 쓰지 않음
	...
	case MQUEST_COMBAT_COMPLETED:
		{
			...
			{
				OnCompleted();
				//Custom: Reset the highest item per round on each round ending.
				m_StageGameInfo.nHighestItemPerRound = 0;
			}
			else if( !CheckPlayersAlive() )
			{
				// 게임이 중간에 끝남.
				OnFailed();
				//Custom: Reset the highest item per round on failure; this is for future quests
				m_StageGameInfo.nHighestItemPerRound = 0;
			}
			else
			{
				OnSectorCompleted();
				//Custom: Reset the highest item per round on completion, this is for future quests
				m_StageGameInfo.nHighestItemPerRound = 0;
			}
		}
		break;
...
void MMatchRuleQuest::OnSectorCompleted()
{
	...
	if (pScenario)
	{
		...
		//Custom: Must be +/- 15 iLevel within quest to gain exp. Commented out for the time being
		//signed int nTotal =  (signed int)( (signed int)(m_StageGameInfo.nHighestItemPerRound + (signed int)100) - (signed int)m_StageGameInfo.nScaling);
		if (((nSectorXP > 0) || (nSectorBP > 0))  /*&& nTotal >= -15 && nTotal <= 15*/)
		{
			for (MQuestPlayerManager::iterator itor = m_PlayerManager.begin(); itor != m_PlayerManager.end(); ++itor)
			{
				...
				if ((!IsEnabledObject(pPlayer)) || (!pPlayer->CheckAlive()) || (itor->second && itor->second->bLateJoined)) continue;
				...
				//Custom: Bonus experience based on quest iLevel. 1% per iLevel.
				nAddedSectorXP =  (nAddedSectorXP * m_StageGameInfo.nScaling) / 100;
				...
				RouteSectorBonus(pPlayer->GetUID(), nAddedSectorXP, nAddedSectorBP, nExpPercent);
			}
		}
	}
...
void MMatchRuleQuest::OnCompleted()
{
	...
	//Custom: Clear iLevel Sacrifice scaling on completion of the quest (ie; all rounds)
	ClearScaling();
...
void MMatchRuleQuest::DistributeReward()
{
	...
	{
		MQuestPlayerInfo* pPlayerInfo = (*itor).second;
		//Custom: Under no circumstances should a player that is late joining be allowed to gain any rewards.

		pPlayer = MMatchServer::GetInstance()->GetObject((*itor).first);
		if( !IsEnabledObject(pPlayer) ) continue;

		if(!pPlayerInfo) continue;

		if(pPlayerInfo->bLateJoined == true) continue;
		if (pPlayer->GetCharInfo()->m_ItemList.GetCount() >= MAX_ITEM_COUNT)
		{
			char sztext[48];
			sprintf_s(sztext, 48, "You have too many items to receive a reward");
			MGetMatchServer()->Announce(pPlayer, sztext);
			continue;
		}
...
void MMatchRuleQuest::DistributeXPnBP( MQuestPlayerInfo* pPlayerInfo, const int nRewardXP, const int nRewardBP, const int nScenarioQL )
{
	...
	if (IsEnabledObject(pPlayerInfo->pObject))
	{
		...
		//Custom: Bonus experience based on quest iLevel. 1% per iLevel.
		pPlayerInfo->nXP =  (pPlayerInfo->nXP * m_StageGameInfo.nScaling) / 100;

		MMatchServer::GetInstance()->ProcessPlayerXPBP(m_pStage, pPlayerInfo->pObject, pPlayerInfo->nXP, pPlayerInfo->nBP);


		// 라우팅
		int nExpPercent = MMatchFormula::GetLevelPercent(pPlayerInfo->pObject->GetCharInfo()->m_nXP,
			pPlayerInfo->pObject->GetCharInfo()->m_nLevel);
		RouteSectorBonus(pPlayerInfo->pObject->GetUID(), pPlayerInfo->nXP, pPlayerInfo->nBP, nExpPercent);
	}
}
...
const bool MMatchRuleQuest::PostNPCInfo()
{
	...
	for( itNL = NPCList.begin(); endNL != itNL; ++ itNL )
	{
		...
		if(m_StageGameInfo.bScaled)
		{
			//Custom: Bonus NPC HP based on exp rate.
			float nHPBonusHP = pMTD_QuestNPCInfo->m_nMaxHP * (m_StageGameInfo.nScaling * 0.015);
			float nAPBonusAP = pMTD_QuestNPCInfo->m_nMaxAP * (m_StageGameInfo.nScaling * 0.015);

			pMTD_QuestNPCInfo->m_nMaxHP = (pMTD_QuestNPCInfo->m_nMaxHP * this->m_StageGameInfo.nScaling / 100) + nHPBonusHP; //Custom: NPC Scaling based on items sacrificed.
			pMTD_QuestNPCInfo->m_nMaxAP = (pMTD_QuestNPCInfo->m_nMaxAP * this->m_StageGameInfo.nScaling / 100) + nAPBonusAP; //Custom: NPC Scaling based on items sacrificed.
		}
	}
