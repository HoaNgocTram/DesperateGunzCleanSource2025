...
{
	MMatchRule* pRule = m_pMatchStage->GetRule();
	if (pRule->GetGameType() != MMATCH_GAMETYPE_CTF)
		return;
...
void MMatchWorldItemManager::SpawnInfoInit()
{
	...
	for (int i = 0; i < m_nSpawnItemCount; i++)
	{
		...
		if (strMapName.find("[wiw]") != string::npos && m_pMatchStage->GetStageSetting()->GetGameType() !=
			MMATCH_GAMETYPE_SPY && m_pMatchStage->GetStageSetting()->GetGameType() != MMATCH_GAMETYPE_GUNGAME &&
			m_pMatchStage->GetStageSetting()->GetGameType() != MMATCH_GAMETYPE_INFECTED && m_pMatchStage->GetStageSetting()->GetGameType() != MMATCH_GAMETYPE_TURBO)
		{
			MMatchWorldItemDesc* wi = MGetMatchWorldItemDescMgr()->GetItemDesc(m_SpawnInfos[i].nItemID);
		...
		if (m_pMatchStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_SKILLMAP)
		{
			MMatchWorldItemDesc* wi = MGetMatchWorldItemDescMgr()->GetItemDesc(m_SpawnInfos[i].nItemID);
			if (wi)
			{
				if (m_SpawnInfos[i].nItemID == 52 || m_SpawnInfos[i].nItemID == 53 || m_SpawnInfos[i].nItemID == 54)
					m_SpawnInfos[i].bUsed = true;
				else
					m_SpawnInfos[i].bUsed = false;
			}
...
#ifdef _PORTALGUN 1
int MMatchWorldItemManager::AddItem(const unsigned short nItemID, short nSpawnIndex,
...
void MMatchWorldItemManager::OnStageBegin(MMatchStageSetting* pStageSetting)
{
	...
	if (MGetGameTypeMgr()->IsWorldItemSpawnEnable(pStageSetting->GetGameType()))
	{
		...
		if (bIsTeamPlay)
		{
			...
			{
				// Custom: Vector fix
				//m_SpawnInfos.resize(nSpawnCount); //m_SpawnInfos.reserve(nSpawnCount);
			
				for (int i = 0; i < nSpawnCount; i++)
				{
					MMatchWorldItemSpawnInfo SpawnInfo;
					SpawnInfo.x = pSpawnInfoSet->TeamSpawnInfo[i].x;
					SpawnInfo.y = pSpawnInfoSet->TeamSpawnInfo[i].y;
					SpawnInfo.z = pSpawnInfoSet->TeamSpawnInfo[i].z;
					SpawnInfo.nCoolTime = pSpawnInfoSet->TeamSpawnInfo[i].nCoolTime;
					SpawnInfo.nItemID = pSpawnInfoSet->TeamSpawnInfo[i].nItemID;
		...
		else
		{
			...
			if (nSpawnCount > 0)
			{
				...
				{
					MMatchWorldItemSpawnInfo SpawnInfo;
					SpawnInfo.x = pSpawnInfoSet->SoloSpawnInfo[i].x;
					SpawnInfo.y = pSpawnInfoSet->SoloSpawnInfo[i].y;
					SpawnInfo.z = pSpawnInfoSet->SoloSpawnInfo[i].z;
					SpawnInfo.nCoolTime = pSpawnInfoSet->SoloSpawnInfo[i].nCoolTime;
					SpawnInfo.nItemID = pSpawnInfoSet->SoloSpawnInfo[i].nItemID;
...
#ifdef _PORTALGUN 1
bool MMatchWorldItemManager::ObtainGamePortal(MMatchObject* pObj, short nItemUID, int* poutItemID, int* poutExtraValues)
{
	if (m_pMatchStage == NULL) return false;
	...
	if (itor != m_ItemMap.end())
	{
		...
		if (pWorldItem)
		{
			for (MMatchWorldItemMap::iterator itor2 = m_ItemMap.begin(); itor2 != m_ItemMap.end(); ++itor2)
			{
				MMatchWorldItem* pWorldItemCheck = (*itor2).second;

				if (pWorldItemCheck)
				{
					if (pWorldItemCheck->nExtraValue[0] != 0 && pWorldItem->nExtraValue[0] == pWorldItemCheck->nExtraValue[0] && pWorldItem != pWorldItemCheck)
					{
						MCommand* pCmd = MMatchServer::GetInstance()->CreateCommand(MC_RESPONSE_MOVETO_GAMEPORTAL, MUID(0, 0));
						pCmd->AddParameter(new MCommandParameterPos(pWorldItemCheck->x, pWorldItemCheck->y, pWorldItemCheck->z));
						MMatchServer::GetInstance()->RouteToListener(pObj, pCmd);
					}
				}
			}
	...
	int nLifeTime, int* pnExtraValues)
{
	...
	int ObjUID = pObj->GetUID().Low;

	if (IsEnabledObject(pObj) && pnExtraValues != NULL && (nItemID == PORTAL_BLUE_ID || nItemID == PORTAL_RED_ID))
		pnExtraValues[0] = ObjUID;

	int nReturnedID = 0;
	if (nItemID == PORTAL_BLUE_ID)
	{
		nReturnedID = AddItem(nItemID, -1, x, y, z, nLifeTime, pnExtraValues);
		int nItemUIDBlue = pObj->m_UIDItemBlue;
		pObj->m_UIDItemBlue = nReturnedID;
		if (nItemUIDBlue != 0)
		{
			RouteRemoveWorldItem(nItemUIDBlue);
			DelItem(nItemUIDBlue);
		}
	}
	else if (nItemID == PORTAL_RED_ID)
	{
		nReturnedID = AddItem(301, 0, x, y, z, nLifeTime, pnExtraValues);
		int nItemUIDRed = pObj->m_UIDItemRed;
		pObj->m_UIDItemRed = nReturnedID;
		if (nItemUIDRed != 0)
		{
			RouteRemoveWorldItem(nItemUIDRed);
			DelItem(nItemUIDRed);
		}
	}
	...
	{
		nReturnedID = AddItem(nItemID, 0, x, y, z, nLifeTime, pnExtraValues);
	}
}
#endif
...
void MMatchWorldItemManager::RouteSpawnWorldItem(MMatchWorldItem* pWorldItem)
{
	...
	if (pRule->GetGameType() == MMATCH_GAMETYPE_DUEL && pWorldItem->nItemID < 100)
		return;
	if (pRule->GetGameType() == MMATCH_GAMETYPE_SKILLMAP) {
		if (pWorldItem->nItemID < 52 || pWorldItem->nItemID > 55)
			return;
	}
...
void MMatchWorldItemManager::CleanUpCQItems()
{

	if (m_pMatchStage == NULL) return;

	if (m_pMatchStage->GetStageSetting() && m_pMatchStage->GetStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST_CHALLENGE)
		return;
	...
	for (MMatchWorldItemMap::iterator itor = m_ItemMap.begin(); itor != m_ItemMap.end(); )
	{
		...
		if (pWorldItem)
		{
			...
			if ((pWorldItem->nStaticSpawnIndex < 0) && (pWorldItem->nLifeTime > 0))
			{
				MMatchRuleQuestChallenge* pRuleQuestChallenge = (MMatchRuleQuestChallenge*)m_pMatchStage->GetRule();

				if (pRuleQuestChallenge && pRuleQuestChallenge->OnObtainWorldItemRandom(pWorldItem->nItemID, pWorldItem->nExtraValue))
				{
					RouteRemoveWorldItem(pWorldItem->nUID);
					delete pWorldItem;
					itor = m_ItemMap.erase(itor);
