...
#ifdef _HWID
				if (pCommand->GetParameter(szHwid, 5, MPT_STR, 128) == false) break;
#endif
...
//#endif
		...
		case MC_MATCH_REQUEST_PEER_RELAY:
			{
				mlog("MC_MATCH_REQUEST_PEER_RELAY");
				MUID  uidPeer;
		...
		case MC_MATCH_REQUEST_CREATE_CHAR:
			{
				...
				if (StringCheckName(szCharName))
				{
					...
					if (!pObj) break;
					mlog("they tried to create a character name using color command. %s - %s", pObj->GetIPString(), pObj->GetAccountName());
					break;
				}
				if (StringCheckFormat(szCharName))
				{
					MMatchObject* pObj = GetObjectA(pCommand->m_Sender);
					if (!pObj) break;
					LOG(7, "they tried to corrupt this stack: %s - %s", pObj->GetIPString(), pObj->GetAccountName());
					break;
				}
#endif
		...
		*/
		...
		case MC_MATCH_LADDER_REQUEST_CHALLENGE:
		{
			...
			unsigned long int nGameType;
			unsigned long int nDodge;
			unsigned long int nRobase;
			...
			pCommand->GetParameter(&nDodge, 5, MPT_UINT);
			pCommand->GetParameter(&nRobase, 6, MPT_UINT);
			void* pMemberNamesBlob = pMemberNamesBlobParam->GetPointer();
			if (NULL == pMemberNamesBlob)
				break;
			OnLadderRequestChallenge(pCommand->GetSenderUID(), pMemberNamesBlob, nOptions, nAntiLead, nGameType, nDodge, nRobase);
		}
		break;
		case MC_MATCH_LADDER_REQUEST_CANCEL_CHALLENGE:
		{
			...
			if (LadderWar)
			{
				GetLadderWarsMgr()->RemovePlayer(pCommand->GetSenderUID());
			}
			else
...
//				pCommand->GetParameter(&nTimeStamp, 0, MPT_UINT);
		...
		case MC_MATCH_LADDERWARS_VOTE:
		{
			...
			if (pObj)
			{
				...
				if (channel && channel->GetChannelType() == MCHANNEL_TYPE_LADDERWAR)
					GetLadderWarsMgr()->UpdatePlayerVote(Map, pObj);
				else
					GetLadderMgr()->UpdatePlayerVote(Map, pObj);
			}
		}
		break;
			case MC_MATCH_LADDERWARS_FRIENDACCEPT:
				{
					MMatchObject* pObj = (MMatchObject*)GetObject(pCommand->GetSenderUID());
					if (pObj)
					{
						MMatchObject* pFObj = (MMatchObject*)GetObject(pObj->LadderWarsFriend);
						if(pFObj && pObj->GetUID() == pFObj->LadderWarsFriend)
						{
							pFObj->LadderState = MMatchObject::Inviter;
							pObj->LadderState = MMatchObject::Invited;
				...
				break;
			case MC_MATCH_LADDERWARS_FRIENDLEAVE:
				{
					MMatchObject* pObj = (MMatchObject*)GetObject(pCommand->GetSenderUID());
					if (pObj)
					{
						...
						if(pFObj && pObj->GetUID() == pFObj->LadderWarsFriend)
						{
							pFObj->LadderState = MMatchObject::NotInvited;
							pFObj->LadderWarsFriend = MUID(0, 0);
							pObj->LadderState = MMatchObject::NotInvited;
				...
				break;
			case MC_MATCH_LADDERWARS_FRIENDINVITE:
				{
					MMatchObject* pObj = (MMatchObject*)GetObject(pCommand->GetSenderUID());
					if (pObj)
					{
						...
						if (pTargetObj->LadderState == MMatchObject::NotInvited)
						{
							pObj->LadderWarsFriend = pTargetObj->GetUID();
							pTargetObj->LadderWarsFriend = pObj->GetUID();
							char mbuf[256], tbuf[256];
							sprintf(tbuf, "%s has invited you as a LadderWars partner.", pObj->GetCharInfo()->m_szName);
							sprintf(mbuf, "%s has been invited to be your LadderWars partner.", pTargetObj->GetCharInfo()->m_szName);
			...
			case MC_MATCH_JOIN_LADDERWARS:
			{
				...
				if (pObj)
				{
					...
					if (Arg2 <= -1 || Arg2 >= MPLADDERWARSTYPE_MAX) break;
					bool Arg3 = false;
					if (Arg1 == 1) Arg3 = true;
                     bool DoesExist = false;
						for (int i = 0; i < MPLADDERWARSTYPE_MAX; i++)
							if (GetLadderWarsMgr()->GetWaitingGames((LadderWar)i)->IsPlayerAlreadyQueued(pObj->GetUID()) == true)
								DoesExist = true;
						if (DoesExist == true)
							return true;
						else
						{
							GetLadderWarsMgr()->GetWaitingGames((LadderWar)Arg2)->AddPlayer(pObj->GetUID());
							if (pObj->LadderState == MMatchObject::Inviter)
							{
								MMatchObject* pFObj = (MMatchObject*)GetObject(pObj->LadderWarsFriend);
								if (!pFObj)
								{
								...
								if (chan && chan->GetChannelType() == MCHANNEL_TYPE_LADDERWAR)
								{
									bool DoesExist = false;
									for (int i = 0; i < MPLADDERWARSTYPE_MAX; i++)
										if (GetLadderWarsMgr()->GetWaitingGames((LadderWar)i)->IsPlayerAlreadyQueued(pFObj->GetUID()) == true)
											DoesExist = true;
									if (DoesExist == false)
									{
										MCommand* pCommand = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_LADDERWARS_INVITED, MUID(0, 0));
										MMatchServer::GetInstance()->RouteToListener(pFObj, pCommand);
										char buf[256];
										sprintf(buf, "%s has invited you.", pObj->GetCharInfo()->m_szName);
										Announce(pFObj, buf);
										GetLadderWarsMgr()->GetWaitingGames((LadderWar)Arg2)->AddPlayer(pFObj->GetUID());
									}
								}
							}
						}
		...
		case MC_MATCH_BANME:
		{
			...
			MCommandParameter* pBlobParam = pCommand->GetParameter(1);
			if (pBlobParam->GetType() != MPT_BLOB) break;

			void* pStackBlob = pBlobParam->GetPointer();
			if (NULL == pStackBlob)
				break;

			OnMatchBanMe(pCommand->GetSenderUID(), (WarpHackType)nType, pStackBlob);
		}
		break;

		case MC_MATCH_BANACCOUNT:
		{
			char szTargetName[128] = "";
			pCommand->GetParameter(szTargetName, 0, MPT_STR, sizeof(szTargetName));
			BanAccount(pCommand->GetSenderUID(), szTargetName);
		}
		break;
		...
		case MC_ADMIN_SBTEST:
		{
			...
			OnAdminSBTest(pCommand->GetSenderUID(), szName);
		}
		break;
		...
		case MC_INVITE_CLANWAR:
		{
			MMatchObject* pObj = GetPlayerByCommUID(pCommand->GetSenderUID());
			...
			//Custom: Only channel own clan.
			if (pChannel->GetChannelType() != MCHANNEL_TYPE_CLAN) 
			{
				Announce(pObj, "You are not your clan channel.");
				break;
			}
			//Custom: Time message...
			if ((timeGetTime() - pObj->GetAccountInfo()->m_nInviteWar) < 1000 * 300) 
			{
				Announce(pObj, "[Notice]: You have to wait some minutes to make another invitation.");
				return false;
			}

			pObj->GetAccountInfo()->m_nInviteWar = timeGetTime();
			//Custom: message announce custom...
			AnnounceMsg("You are Inviting a clan war!", pObj->GetCharInfo()->m_ClanInfo.m_szClanName);
		}
		break;
