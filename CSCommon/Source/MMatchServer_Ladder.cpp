...
#ifdef _BOXLEAD
	bool Lead = pGroupA->GetLead();
#endif
#ifdef _ROTATION
	bool Rotation = pGroupA->GetRoBase();
#endif
	switch (nGameType)
	{
	case GLADIATOR:
...
#ifdef _ROTATION
	pSetting->SetRoBase(pGroupA->GetRoBase());
#endif
#ifdef _BOXLEAD
	pSetting->SetLead(pGroupA->GetLead());
#endif
...
void MMatchServer::OnLadderRequestChallenge(const MUID& uidRequestMember, void* pMemberNamesBlob, unsigned long int nOptions, unsigned long int nAntiLead, unsigned long int nGameType, unsigned long int nDodge, unsigned long int nRobase)
{
	if ((MGetServerConfig()->GetServerMode() != MSM_LADDER) && 
		(MGetServerConfig()->GetServerMode() != MSM_CLAN)) return;
	...
	if (nGameType <= TDM && nGameType != 0 || nGameType == NONE) { LOG(LOG_PROG, "Invalid ClanWar GameType: %d\n", nGameType); return; }
	...
	if (nDodge == 1)
	{
		pGroup->SetDodge(true);
	}
	else
	{
		pGroup->SetDodge(false);
	}
#ifdef _ROTATION
	if (nRobase == 1)
	{
		pGroup->SetRoBase(true);
	}
	else
	{
		pGroup->SetRoBase(false);
	}
#endif
	if (pTeamGameStrategy)
	{
