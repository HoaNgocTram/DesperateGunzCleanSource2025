...
bool MMatchConfig::Create()
{
	...
	if (!stricmp(szServerMode, SERVER_CONFIG_SERVERMODE_NORMAL)) m_nServerMode = MSM_NORMAL1;
	...
	char szBlockAgent[2] = "";
	GetPrivateProfileString("SERVER", "AGENTBLOCK", "1", szBlockAgent, 2, SERVER_CONFIG_FILENAME);
	if (1 == atoi(szBlockAgent))
		SetUseAgentBlock(true);
	else
	{
		SetUseAgentBlock(false);
	}
#ifdef _AGENTPORT
	GetPrivateProfileString("SERVER", "AGENTIP", "", m_AgentIP, 64, SERVER_CONFIG_FILENAME);
#endif
	// 일본 넷마블 전용
