...
void CopyChannelPlayerListNodeForTrans(MTD_ChannelPlayerListNode* pDest, MMatchObject* pSrcObject)
{
	...
	LadderWarsCharInfo* pCharInfo = pSrcObject->GetLadderWarsInfo();
	pDest->nPWPoints = pCharInfo ? pCharInfo->Score : 0;
...
bool MMatchServer::ChannelAdd(const char* pszChannelName, const char* pszRuleName, MUID* pAllocUID, MCHANNEL_TYPE nType, int nMaxPlayers, int nLevelMin, int nLevelMax,
							  const bool bIsTicketChannel, const DWORD dwTicketItemID, const bool bIsUseTicket, const bool bIsUseEvent, const char* pszChannelNameStrResId)
{
	return m_ChannelMap.Add(pszChannelName, pszRuleName, pAllocUID, nType, nMaxPlayers, nLevelMin, nLevelMax, bIsTicketChannel, dwTicketItemID, bIsUseTicket, bIsUseEvent, pszChannelNameStrResId);
}
...
bool MMatchServer::ChannelJoin(const MUID& uidPlayer, const MCHANNEL_TYPE nChannelType, const char* pszChannelName)
{
	...
	if (pChannel == NULL)
	{
		...
		if( nChannelType == MCHANNEL_TYPE_DUELTOURNAMENT )
			return false;
		if (nChannelType == MCHANNEL_TYPE_LADDERWAR)
			return false;
...
bool MMatchServer::ChannelJoin(const MUID& uidPlayer, const MUID& uidChannel)
{
	...
	if ( MGetServerConfig()->IsUseTicket())
	{
		...
		if ( MGetServerConfig()->GetServerMode() == MSM_NORMAL1)	{ // 일반 서버일때
...
*/
	...
	// Custom: Added functionality for developers to post to channel chat while being in a room
	MUID uidStage = pObj->GetStageUID();
	if( uidStage != MUID(0, 0) && !IsAdminGrade(pObj) && pObj->GetAccountInfo()->m_nUGrade != MMUG_GAMEMASTER)
	{
...
#ifdef _ANTISPAM
	if (IsAdminGrade(pObj) == false && pObj->SetLastChatTime(timeGetTime()) == false)
	{
		char nub[120];
		if (pObj->GetChatBlockCount() >= 6)
		{
//#define IN_MINUTE	(__int64)10000000*60
#define IN_MINUTE (10 * 60 * 1000)
			SYSTEMTIME sysTime;
			FILETIME ft;
			LARGE_INTEGER lgInt;
			GetLocalTime(&sysTime);
			SystemTimeToFileTime(&sysTime, &ft);
			memcpy(&lgInt, &ft, sizeof(FILETIME));
			int nchatblockmins = pObj->GetChatBlockMins();
			lgInt.QuadPart += IN_MINUTE * nchatblockmins;
			memcpy(&ft, &lgInt, sizeof(FILETIME));
			FileTimeToSystemTime(&ft, &sysTime);
			pObj->GetAccountPenaltyInfo()->SetPenaltyInfo(MPC_CHAT_BLOCK, sysTime);
			sprintf(nub, "Chat Blocked For %d Minutes.", nchatblockmins);
			pObj->SetChatBlockCount(0);
		}
		else
		{
			sprintf(nub, "Warning %d/5(Leave 2 Seconds Inbetween Msg's).", pObj->GetChatBlockCount());
		}
		MCommand* pmyCmd = CreateCommand(MC_ADMIN_WALL, MUID(0, 0));
		pmyCmd->AddParameter(new MCmdParamUID(pObj->GetUID()));
		pmyCmd->AddParameter(new MCmdParamStr(nub));
		pmyCmd->AddParameter(new MCmdParamStr("Chat Limiter"));
		RouteToListener(pObj, pmyCmd);
		return false;
	}
#endif
...
#define MAX_CHANNEL_LIST_NODE		100
	...
	for (map<MUID, MMatchChannel*>::iterator itor=m_ChannelMap.GetTypesChannelMapBegin(nChannelType); 
		...
		pNode->nNo = nIndex;
		// Custom: Changed data type for player count (byte -> short) to accommodate more than 255 players
		pNode->nPlayers = pChannel->GetObjCount(); //(unsigned char)pChannel->GetObjCount();
		pNode->nMaxPlayers = pChannel->GetMaxPlayers();
		pNode->nChannelType = pChannel->GetChannelType();
		//Custom: Player Online
		pNode->nPlayerOn = (int)m_Objects.size();
