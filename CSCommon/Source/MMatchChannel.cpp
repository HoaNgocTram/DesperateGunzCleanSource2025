...
bool MMatchChannel::Create(const MUID& uid, const char* pszName, const char* pszRuleName, 
						...
						const bool bIsTicketChannel, const DWORD dwTicketItemID, const bool bIsUseTicket, const bool bIsUseEvent, const char* pszNameStrResId)
{
	m_nEmptyPeriod		= 0;
	...
	// 채널룰에 대한 특성들 정의
	if (nType == MCHANNEL_TYPE_PRESET || nType == MCHANNEL_TYPE_DUELTOURNAMENT || nType == MCHANNEL_TYPE_LADDERWAR)
	{
		...
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_LEAD_STR))			m_nRuleType = MCHANNEL_RULE_LEAD;
#ifdef _CHANNELNEW
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_NEWUSER_STR))		m_nRuleType = MCHANNEL_RULE_NEWS;
#endif
...
bool MMatchChannel::CheckLifePeriod()
{
	...
	if (nType == MCHANNEL_TYPE_PRESET || nType == MCHANNEL_TYPE_DUELTOURNAMENT || nType == MCHANNEL_TYPE_LADDERWAR) return true;
						   ...
						   const bool bIsTicketChannel, const DWORD dwTicketItemID, const bool bIsUseTicket, const bool bIsUseEvent, const char* pszChannelNameStrResId)
{
	...
	pChannel->Create(uidChannel, pszChannelName, pszRuleName, nType, nMaxPlayers, nLevelMin, nLevelMax, bIsTicketChannel, dwTicketItemID, bIsUseTicket, bIsUseEvent, pszChannelNameStrResId);
