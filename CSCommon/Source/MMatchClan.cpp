...
void MMatchClan::Create(int nCLID, const char* szClanName)
{
	...
	m_SmartRefresh.AddCategory(new MRefreshCategoryClanMemberImpl(this, 0));	// Category 0 ·Î ÀE¼¸ñ·Ï ¸ğµÎ ´ã´E
}
...
void MMatchClanMap::AddObject(const MUID& uid, MMatchObject* pObj)
{
	...
	// ÀÓ½ÃÄÚµE.. AddObject()ÇÏ´Â ¾ûĞÀ°÷¿¡¼­ Àß¸øµÈ °ªÀ» °¡Á®¿Í¼­ ¼­¹EÅ©·¡½¬ À¯¹ß½ÃÅ°±E¶§¹®¿¡ ÀÏ´Ü ¿©±â¼­ ¸·À½
	// ¶ÇÇÑ AddObject()ÇÏ´Â°÷¿¡¼­ log¸¦ ³²°Ü ¹èÆ÷ÈÄ ¿øÀÎÀ» Àâ´Â °úÁ¤ÀÌ ÀÖÀ»°ÅÀÓ...20090224 by kammir
	if (nCLID <= -1) return;		// nCLID´Â Á¤¼öÇE
	if (nCLID >= 9000000) return;	// ÇöÀE20090224) ÇÑ±¹ ÃÖ°EÅ¬·£ID°¡ "1399559"ÀÌ´Ù.(DB¿¡¼­´Â 1¾¿Áõ°¡½ÃÅ´)
	...
	if (itor != end())
	{
		...
		// Custom: Force init to fix cwoption
