...
#ifdef _DYNAMIC
const char* MMatchItemDesc::GetDyn()
{
	if (strlen(m_szDyn) > 0)
		return m_szDyn;
	...
	m_nColor(0xFFFFFFFF), m_nImageID(0), m_nBulletImageID(0), m_nMagazineImageID(0), /*m_bIsCashItem(false),*/ m_bIsSpendableItem(false)
{
	...
	m_nCurrency.Set_MakeCrc(ITEM_BOUNTY);
...
#ifdef _DYNAMIC
	memset(m_szDyn, 0, sizeof(m_szDyn));
#endif
...
void MMatchItemDesc::CacheCRC32( MMatchCRC32XORCache& crc )
{
	...
	// ｸﾞｽｬｸ暲ｻ ｹﾙｲﾙｴﾂ ｸﾞｸｮﾇﾙ ｶｧｹｮｿ｡ ｸﾞｽｬｸ昉ｵ crcｿ｡ ﾆﾔ (ｵ鄧ﾟｰﾓﾀｸｷﾎ ﾄｮﾁ戓ﾔ)
...
void MMatchItemDesc::ShiftFugitiveValues()
{
	...
	m_nCurrency.ShiftHeapPos_CheckCrc();
	...
	// Custom: Don't check CRC for HP and AP
...
{
	// zitem.xmlﾀｻ ﾆﾄｽﾌﾇﾏｴﾂｵ･ ｸ鋻ﾊﾃﾊ ｰﾉｸｮｹﾇｷﾎ, ﾀﾌｸｦ ｹﾙﾀﾌｳﾊｸｮｷﾎ ﾄｳｽﾌﾇﾘ ｵﾐｰﾍﾀｻ ﾃ｣ｾﾆ ﾀﾐｴﾂｴﾙ. ｰｳｹﾟｺ蠡ﾏｶｧｸｸ ﾀﾛｵｿ.
	// ｸｸｾ・zitem.xmlﾀｻ ｺｯｰ貮ﾟﾀｻ ｶｧｿ｡ｴﾂ runtime/zitemxml.cache ﾆﾄﾀﾏﾀｻ ｻ霖ｦﾇﾏｸ・ｵﾈｴﾙ.
	// Custom: Don't read zitem cache for debug
	/*
#ifdef _DEBUG
...
{
	// ｴﾙﾀｽｹ・ｽﾇﾇ狄ﾃ ｷﾎｵﾓｵｵｸｦ ｳﾌｱ・ﾀｧﾇﾘｼｭ zitem.xmlﾀｻ ｹﾙﾀﾌｳﾊｸｮ ﾆﾄﾀﾏｷﾎ ﾄｳｽﾌﾇﾘ ｵﾐｴﾙ. ｰｳｹﾟｺ蠡ﾏｶｧｸｸ ﾀﾛｵｿ.
	// Custom: Don't write zitem cache for debug
	/*
#ifdef _DEBUG
...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	// default ｰｪ ﾀﾔｷﾂ
	pNewDesc->m_nCurrency.Set_MakeCrc(ITEM_BOUNTY);
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		else if (!stricmp(szAttrName, MICTOK_WEAPON))
		{
			...
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SPYCASE))					pNewDesc->m_nWeaponType.Set(MWT_SPYCASE);
			...
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_FLASHBANG_SPY))			pNewDesc->m_nWeaponType.Set(MWT_FLASH_BANG_SPY);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SMOKE_SPY))				pNewDesc->m_nWeaponType.Set(MWT_SMOKE_GRENADE_SPY);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_TRAP_SPY))					pNewDesc->m_nWeaponType.Set(MWT_TRAP_SPY);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_STUNGRENADE_SPY))			pNewDesc->m_nWeaponType.Set(MWT_STUN_GRENADE_SPY);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_LANDMINE_SPY))				pNewDesc->m_nWeaponType.Set(MWT_LANDMINE_SPY);
#ifdef _PORTALGUN 1
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_PORTALGUN))		pNewDesc->m_nWeaponType.Set(MWT_PORTAL_GUN);
#endif
			else _ASSERT(0);
		...
		else if (!_stricmp(szAttrName, MICTOK_CURRENCY))
		{
			if (strlen(szAttrValue) <= 0)									pNewDesc->m_nCurrency.Set(ITEM_BOUNTY);
			else if (!_stricmp(szAttrValue, MICTOK_CURRENCY_BOUNTY))	pNewDesc->m_nCurrency.Set(ITEM_BOUNTY);
			else if (!_stricmp(szAttrValue, MICTOK_CURRENCY_CASH))		pNewDesc->m_nCurrency.Set(ITEM_CASH);
			else if (!_stricmp(szAttrValue, MICTOK_CURRENCY_MEDAL))		pNewDesc->m_nCurrency.Set(ITEM_MEDAL);
			else if (!_stricmp(szAttrValue, MICTOK_CURRENCY_STAFF))		pNewDesc->m_nCurrency.Set(ITEM_STAFF);
			else if (!_stricmp(szAttrValue, MICTOK_CURRENCY_EVENT))		pNewDesc->m_nCurrency.Set(ITEM_EVENT);

			pNewDesc->m_nCurrency.MakeCrc();
		}
		...
		// ｿｩｱ箴ｭｺﾎﾅﾍｴﾂ ｾﾆｹﾙﾅｸ ｰ・ﾃｵﾈ ｰﾍｵ・.. ﾃﾟﾈﾄｿ｡ｴﾂ ｵ都ﾎ XMLｷﾎ ｰ・ｮﾇﾏｰ・ｽﾍﾀｺｵ･-_､ﾐ
...
#ifdef _DYNAMIC
		else if (!_stricmp(szAttrName, "get_elu"))
		{
		strcpy_s(pNewDesc->m_szDyn, szAttrValue);
		}
#endif
	}
	...
	if (tempitor != end()) {
		_ASSERT(0);		///< ｰｰﾀｺ IDﾀﾇ ｾﾆﾀﾌﾅﾛﾀﾌ ﾁｸﾀ酩ﾑｴﾙ.
		return false;
	}
...
{
	// MemoryFugitiveｴﾂ SetDataﾇﾒｶｧｸｸ ﾀｧﾄ｡ｰ｡ ｹﾙｲ・ﾙ. ｾﾆﾀﾌﾅﾛ ｼﾓｼｺｰｪﾀｺ ﾃﾖﾃﾊｿ｡ ﾇﾑｹｸ SetData ﾇﾏｹﾇｷﾎ
	// ｰ皺惕釥ｸｷﾎ ﾀｧﾄ｡ｰ｡ ｹﾙｲ錝・ｾﾊｰﾔ ｵﾈｴﾙ. ｸﾅﾇﾁｷｹﾀﾓｸｶｴﾙ ｾﾆﾀﾌﾅﾛ ｼﾓｼｺﾄ｡ｸｦ ｼｿﾀｸｷﾎ ｸﾞｸｮ ﾀｧﾄ｡ ｺｯｰ貮ﾑｴﾙ
	// ｼｺｴﾉ ｺﾎﾇﾏｰ｡ ｿｰｷﾁｵﾇｾ・ｿｩｷｯﾇﾁｷｹﾀﾓｿ｡ ｰﾉﾃﾄ ｾﾆﾀﾌﾅﾛ ｸ銧ｳｾｿ ﾂﾊｷﾎ ｿﾅｰﾜﾁﾘｴﾙ
...
MMatchItemDesc* MMatchItem::GetDesc() const
{
	if( NULL == m_pDesc )
	{
		// NHNﾅﾗｽｺﾆｮｸｦ ﾇﾏｱ・ｿ・ﾘｼｭ. 
		// ﾀﾌ ｷﾎｱﾗｴﾂ ﾅｬｶﾌｾｮｿ｡ｵｵ ｳｲｱ筝ｧｹｮｿ｡ ｺ・ﾈﾄ ﾁﾖｼｮ ﾃｳｸｮﾇﾔ.
...
bool MMatchEquipedItem::SetItem( MMatchCharItemParts parts, const MUID& uidItem, MMatchItem* pItem )
{
	...
	// partsｸｦ ﾆﾄｶﾌﾅﾍｷﾎ ｳﾑｰﾜｵｵ ｵﾇﾁｸ, ﾄﾚｵ蠡ﾇ ｸ柀ｮｼｺﾀｻ ﾀｧﾇﾏｿｩ!
...
bool MMatchEquipedItem::IsEquipedItem(const MUID& uidItem, MMatchCharItemParts& outParts) // ﾇﾘｴ・ｾﾆﾀﾌﾅﾛﾀﾌ ﾀ蠎ﾟﾀﾎﾁ・ﾃｼﾅｩ
{
	for( int i = 0; i < MMCIP_END; ++i )
	{
...
// Custom: Avatar Clothing Slot calculations
...
bool IsWeaponCharItemParts(MMatchCharItemParts nParts)
{
	if ((nParts == MMCIP_MELEE) || (nParts == MMCIP_PRIMARY) ||	(nParts == MMCIP_SECONDARY) 
		|| (nParts == MMCIP_CUSTOM1) ||	(nParts == MMCIP_CUSTOM2) || (nParts == MMCIP_CUSTOM3)
...
char* GetItemSlotTypeStr(MMatchItemSlotType nSlotType)			///< ｻ鄙・ﾇﾁ・ｾﾊｴﾂｴﾙ?
{
	static char st_SlotTypeStr[MMIST_END][32] = { "ｾｽ",		// MMIST_NONE
												"ｱﾙﾁ｢ｹｫｱ・",		// MMIST_MELEE
												"ｿﾅｸｮｹｫｱ・",	// MMIST_RANGE
												"ｾﾆﾀﾌﾅﾛ",		// MMIST_CUSTOM
												"ｸﾓｸｮ",			// MMIST_HEAD
												"ｰ｡ｽｿ",			// MMIST_CHEST
												"ｼﾕ",			// MMIST_HANDS
...
char* GetCharItemPartsStr(MMatchCharItemParts nParts)			///< ｻ鄙・ﾇﾁ・ｾﾊｴﾂｴﾙ?
{
	static char st_CharItemPartsStr[MMCIP_END][32] = {  "ｸﾓｸｮ",					// MMCIP_HEAD
														"ｰ｡ｽｿ",					// MMCIP_CHEST
														"ｼﾕ",					// MMCIP_HANDS
														"ｴﾙｸｮ",					// MMCIP_LEGS
														"ｹﾟ",					// MMCIP_FEET
														"ｿﾞﾂﾊｼﾕｰ｡ｶ・",			// MMCIP_FINGERL
														"ｿﾀｸ･ﾂﾊｼﾕｰ｡ｶ・",			// MMCIP_FINGERR
														"ｱﾙﾁ｢ｹｫｱ・",				// MMCIP_MELEE
														"ﾁﾖｹｫｱ・",				// MMCIP_PRIMARY
														"ｺｸﾁｶｹｫｱ・",				// MMCIP_SECONDARY
														"ｾﾆﾀﾌﾅﾛ1",				// MMCIP_CUSTOM1
														"ｾﾆﾀﾌﾅﾛ2",				// MMCIP_CUSTOM2
		                                                "ｾﾆﾀﾌﾅﾛ3",				// MMCIP_CUSTOM3
														};
...
#ifdef _PORTALGUN 1
	case RIT_PORTALGUN:		return MWT_PORTAL_GUN;
#endif
	default:
			// ｾﾂ ﾅｸﾀﾔ
			_ASSERT(0);
...
{ 
#ifdef _TDD
	MMatchItemDescMgrBase* p = (MMatchItemDescMgrBase*)ZTddGetGlobalSub(GI_MatchItemDescMgr);
	if (p) return p;
#endif
	return MMatchItemDescMgr::GetInstance(); 
}
