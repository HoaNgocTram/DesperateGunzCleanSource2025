...
void MMatchWorldItemDescMgr::ParseWorldItem(MXmlElement& element)
{
	...
	else if (!stricmp(szType, "flag"))		pNewWorldItemDesc->m_nItemType = WIT_FLAG;
#ifdef _PORTALGUN 1
	else if (!stricmp(szType, "portal_gun_portal"))	pNewWorldItemDesc->m_nItemType = WIT_PORTAL;
#endif
	else if ((!stricmp(szType, "mine") || !stricmp(szType, "spy_mine"))) pNewWorldItemDesc->m_nItemType = WIT_MINE;
	else if (!stricmp(szType, "weapon")) pNewWorldItemDesc->m_nItemType = WIT_WEAPON;
