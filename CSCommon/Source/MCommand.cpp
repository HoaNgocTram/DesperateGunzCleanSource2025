...
bool MCommand::AddParameter(MCommandParameter* pParam)
{
	...
	_ASSERT(nCount<nParamDescCount);				// Debug Mode ¿¡¼­´Â Assert·Î ¾ö°ÝÇÏ°Ô Ã¼Å©ÇÑ´Ù.
...
bool MCommand::SetData(char* pData, MCommandManager* pCM, unsigned short nDataLen)
{
	...
	if (nDataCount != nTotalSize)
	{
		// In log để mình còn biết đường mà sửa Server cho chuẩn
		mlog("DEBUG: Lenh ID %d bi lech byte! DataCount: %d, TotalSize: %d\n", nCommandID, nDataCount, nTotalSize);

		// Nếu là lệnh Response Login MC_MATCH_RESPONSE_LOGIN (1002) thì cho qua để hiện thông báo lỗi
		if (nCommandID == MC_MATCH_RESPONSE_LOGIN)
		{
			return true;
		}

		// Với các lệnh khác, nếu lệch thì vẫn phải chặn để tránh Crash hoặc Bug game
		return false;
	}
