...
{ 
	...
	//Ladder Wars
	m_pLadderWarsCharInfo = NULL;
	LadderWarsFriend = MUID(0, 0);
	LadderState = NotInvited;
	LadderWarsIdentifier = -1;
...
void MMatchObject::FreeLadderWarsInfo()
{
	if (m_pLadderWarsCharInfo)
	{
		m_pLadderWarsCharInfo = NULL;
	}
}
...
void MMatchObject::SetLadderWarsCharInfo(LadderWarsCharInfo* pLadderWarCharInfo)
{
	// 이미 있는 정보면 어쩌죠? -.,-;
	// 그럴 일은 없겠지만.. 지우고 다시 할당하자!
	if (m_pLadderWarsCharInfo) {
		delete m_pLadderWarsCharInfo;
		m_pLadderWarsCharInfo = NULL;
	}

	m_pLadderWarsCharInfo = new LadderWarsCharInfo();
	memcpy(m_pLadderWarsCharInfo, pLadderWarCharInfo, sizeof(LadderWarsCharInfo));;
}
...
const bool MMatchObject::IsHaveCustomItem()
{
	...
	if (NULL != GetCharInfo()->m_EquipedItem.GetItem(MMCIP_CUSTOM3)) return true;
...
#ifdef _PORTALGUN 1
const bool MMatchObject::IsHavePortalGunEquipped()
{
	if ((NULL != GetCharInfo()->m_EquipedItem.GetItem(MMCIP_PRIMARY) && GetCharInfo()->m_EquipedItem.GetItem(MMCIP_PRIMARY)->GetDesc()->m_nWeaponType.Ref() == MWT_PORTAL_GUN) ||
		(NULL != GetCharInfo()->m_EquipedItem.GetItem(MMCIP_SECONDARY) && GetCharInfo()->m_EquipedItem.GetItem(MMCIP_SECONDARY)->GetDesc()->m_nWeaponType.Ref() == MWT_PORTAL_GUN))
	{
		return true;
	}
