...
#ifdef _LIGHTURNOFF
bool MChannelRuleMapList::ExistAsLightMap(int nMapID)
{
	set<int>::iterator itor = m_Set.find(nMapID);
	...
	if (itor != m_Set.end())
	{
		...
		if (MGetMapDescMgr()->IsLightMap(id))
			return true;
...
bool MChannelRuleMapList::ExistAsLightMap(const char* pszMapName)
{
	for (set<int>::iterator itor = m_Set.begin(); itor != m_Set.end(); ++itor)
	{
		int id = (*itor);
...
void MChannelRuleMgr::AddRule(MChannelRule* pRule)
{
...
#ifdef _CHANNELNEW
	else if (!stricmp(MCHANNEL_RULE_NEWUSER_STR, pRule->GetName()))
	{
		m_RuleTypeMap.insert(map<MCHANNEL_RULE, MChannelRule*>::value_type(MCHANNEL_RULE_NEWS, pRule));
	}
#endif
	...
	else if (!_stricmp(MCHANNEL_RULE_LEAD_STR, pRule->GetName()))
	{
		m_RuleTypeMap.insert(map<MCHANNEL_RULE, MChannelRule*>::value_type(MCHANNEL_RULE_LEAD, pRule));
	}
	else
	{
