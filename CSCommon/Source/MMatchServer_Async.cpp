...
#include "../CSCommon/LadderWars/MAsyncDBJob_LadderWars.h"
...
void MMatchServer::ProcessAsyncJob()
{
	while(MAsyncJob* pJob = m_AsyncProxy.GetJobResult()) 
	{
		...
		case MASYNCJOB_UPDATE_LADDER_WAR_INFO:
		{
			OnAsyncUpdateLadderWars(pJob);
		}
		break;
		case MASYNCJOB_GET_LADDER_WAR_INFO:
		{
			OnAsyncGetLadderWarsCharInfo(pJob);
		}
		break;
		case MASYNCJOB_GET_LADDER_WAR_SIDE_RANKING:
		{
			OnAsyncResponse_GetLadderWarsSideRanking(pJob);
		}
		break;
		...
		case MASYNCJOB_INSERTQUESTCHALLENGEGAMELOG:
			{
...
void MMatchServer::OnAsyncUpdateLadderWars(MAsyncJob* pJobResult)
{
	MAsyncDBJob_UpdateLadderWarsCharInfo* pJob = (MAsyncDBJob_UpdateLadderWarsCharInfo*)pJobResult;

	if (pJob->GetResult() != MASYNC_RESULT_SUCCEED) {
		char szTime[128] = "";
		_strtime(szTime);

		mlog("[%s] Async DB Query(UpdateLadderWars) Failed\n", szTime);
		return;
	}
}
...
void MMatchServer::OnAsyncExpelClanMember(MAsyncJob* pJobResult)
{
	...
	if (IsEnabledObject(pMemberObject))
	{
		...
		RouteResponseToListener(pMemberObject, MC_MATCH_CLAN_ADMIN_RESPONSE_EXPEL_MEMBER, MERR_CLAN_MEMBER_KICKED);
		// 임시코드... 잘못된 MMatchObject*가 온다면 체크하여 잡기위함...20090224 by kammir
