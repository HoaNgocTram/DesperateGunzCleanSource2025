...
DWORD LadderWarsMgrExceptionHandler( PEXCEPTION_POINTERS ExceptionInfo, char* Place )
{
	...
	sprintf( szStageDumpFileName, "Log/MLadderWarsMgr[%s]_%d-%d-%d_%d-%d-%d.dmp"
		, Place
		, SystemTime.wYear
		, SystemTime.wMonth
		, SystemTime.wDay
...
void MLadderWarsMgr::UpdateQueueingCounters(unsigned long int NowTime)
{
	__try
	{
		if(NowTime - nLastTime1 >= 15000)
		{
			...
			{
				LadderWarsData* TempWaitingGames = GetWaitingGames((LadderWar)i);
...
//	{
		...
		{
			LadderWarsData* TempWaitingGames = GetWaitingGames((LadderWar)Type);
			if(!TempWaitingGames) continue;
			int PlayerCount= 0;
			switch((LadderWar)Type)
			{
			...
			while(TempWaitingGames->GetCount() >= PlayerCount)
			{
						...
						switch (pObj->LadderState)
						{
							...
							case MMatchObject::Inviter:
							{
								...
								{ 
									...
									if(chan->GetChannelType() == MCHANNEL_TYPE_LADDERWAR && pFriendObj->GetPlace() == MMP_LOBBY)
									{
									...
									else
									{
										...
										pFriendObj->LadderState = MMatchObject::NotInvited;
										pObj->LadderWarsFriend = MUID(0, 0);
										pObj->LadderState = MMatchObject::NotInvited;
									}
								} else { pObj->LadderWarsFriend = MUID(0, 0); pObj->LadderState = MMatchObject::NotInvited; }
								ReadyGame.insert(PlayerJoiningDataStruct::value_type(pObj->GetUID(), pObj->LadderWarsFriend));
								TempWaitingGames->RemovePlayer(pObj->GetUID());
								l++;
								continue;
				...
				{
					MapSelection(ReadyGame, (LadderWar)Type, NowTime);
				}
			}
		}
	//}
...
void MLadderWarsMgr::UpdateMapCountDown(unsigned long int NowTime)
{
	...
	//{
		for(map<unsigned long int, MapSelectionStruct*>::iterator i = WaitingMapSelectionGames.begin(); i != WaitingMapSelectionGames.end();)
		{
			...
			{	
				MCommand* pCommand = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_LADDERWARS_VOTE_UPDATE,  MUID(0,0));
...
void MLadderWarsMgr::MapSelection(PlayerJoiningDataStruct ReadyGame, LadderWar Type, unsigned long int NowTime)
{
	//__try
	//{
		...
		MCommand* pCommand = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_LADDERWARS_RANDOM_MAPS,  MUID(0,0));
