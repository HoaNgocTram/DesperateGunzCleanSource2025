...
protected:
	...
	MUID m_uidPlayer;
	LadderWarsCharInfo m_pLadderWarCharInfo;
public:
	MAsyncDBJob_GetLadderWarsCharInfo(void) : MAsyncJob(MASYNCJOB_GET_LADDER_WAR_INFO, MUID(0, 0)) {
		m_dwPlayerCID = 0;
	...
	LadderWarsCharInfo* GetPWCharInfo()	{ return &m_pLadderWarCharInfo; }	
};
...
public:
	MAsyncDBJob_UpdateLadderWarsCharInfo(void) : MAsyncJob(MASYNCJOB_UPDATE_LADDER_WAR_INFO, MUID(0, 0)) {
		m_dwPlayerCID = 0;
	}
...
public:
	MAsyncDBJob_GetLadderWarsSideRankingInfo(void) : MAsyncJob(MASYNCJOB_GET_LADDER_WAR_SIDE_RANKING, MUID(0, 0)) {
		m_dwPlayerCID = 0;
		m_uidPlayer = MUID(0, 0);
	}
