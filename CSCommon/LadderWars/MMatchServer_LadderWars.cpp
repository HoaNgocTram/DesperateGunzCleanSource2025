...
void MMatchServer::LaunchLadderWarsGame(PlayerJoiningDataStruct Data, LadderWar Type, int MapID)
{
	//	__try
	//{
	bool error = false;
	MUID uidStage = MUID(0, 0);
	if (StageAdd(NULL, "Ladder_Wars", true, "", &uidStage) == false) {
		error = true;
	}
...
void MMatchServer::PostLadderWarsCharInfo(MUID uidPlayer, LadderWarsCharInfo *pLadderWarCharInfo)
{
	if( pLadderWarCharInfo == NULL ) return;
	MCommand *pCmd = CreateCommand(MC_MATCH_LADDERWARS_CHARINFO, uidPlayer);
	pCmd->AddParameter(new MCommandParameterInt(pLadderWarCharInfo->Ranking));
	pCmd->AddParameter(new MCommandParameterInt(pLadderWarCharInfo->Wins));
	pCmd->AddParameter(new MCommandParameterInt(pLadderWarCharInfo->Losses));
	pCmd->AddParameter(new MCommandParameterInt(pLadderWarCharInfo->Draws));
	pCmd->AddParameter(new MCommandParameterInt(pLadderWarCharInfo->Score));
	Post(pCmd);
}
...
void MMatchServer::UpdateLadderWarsQueueingCounts(MTD_LadderWarsCounterInfo CounterInfo)
{
		...
		for(MMatchObjectList::iterator i=m_Objects.begin(); i!=m_Objects.end(); i++) 
		{
				...
				if( pObj != NULL && pObj->GetPlace() == MMP_LOBBY) 
				{
					...
					if(pChannel && pChannel->GetChannelType() == MCHANNEL_TYPE_LADDERWAR)
					{	
