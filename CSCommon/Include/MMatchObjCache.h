...
protected:
	...
	int						m_nPWPoints;        //Custom: LadderPoints Rank
...
public:
	MMatchObjCache()				{ 
		...
		ResetFlag();
		m_nDTGrade = 0;
		m_nPWPoints = 0; //Custom: LadderPoints Rank
	}
	...
	int GetCountryFlag()   { return m_nCountryFlag; }

	//Custom: LadderPoints Rank
	int GetPWPoints() { return m_nPWPoints; }
	void SetPWPoints(int nValue) { m_nPWPoints = nValue; }

	void SetInfo(const MUID& uid, const char* szName, const char* szClanName, int nLevel, 
				 MMatchUserGradeID nUGrade, MMatchPremiumGradeID nPGrade, int nCountryFlag
				 , const unsigned int nRank, const int nKillCount, const int nDeathCount, int nDTGrade, int nPWPoints) //Custom: LadderPoints Rank)
	{
		m_uidObject = uid;
		...
		m_nUGrade		= nUGrade;
		m_nPGrade		= nPGrade;
		m_nCountryFlag  = nCountryFlag;
		m_nPlayerFlags	= 0;
		m_nRank			= nRank;
		m_nKillCount	= nKillCount;
		m_nDeathCount	= nDeathCount;
		m_nDTGrade		= nDTGrade;
		m_nPWPoints     = nPWPoints; //Custom: LadderPoints Rank
	}
