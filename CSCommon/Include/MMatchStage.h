...
#ifdef _PORTALGUN 1
#define PORTAL_BLUE_ID 301
#define PORTAL_RED_ID 302
#endif
...
struct LadderWarsInfos
{
	MMatchTeam Team;
	MUID Player;
	LadderWarsInfos(MMatchTeam t, MUID p)
	{
		Team = t;
		Player = p;
	}
...
typedef	map<int, LadderWarsInfos*> LadderWarsListDef;
...
private:
	...
	int						m_nVIPObjectCount;				// Custom: VIP Members
	unsigned long			m_nSilenceEndTime;				// Custom: Silence Room
	...
	LadderWarsListDef LadderWarsList;
...
public:
	MUIDRefCache GetObjectsNow() { return m_ObjUIDCaches; }
	void LadderWarsAdd(int cid, MMatchTeam Team, MUID Player) { LadderWarsList.insert(LadderWarsListDef::value_type(cid, new LadderWarsInfos(Team, Player))); }
	MMatchTeam LadderWarsTeam(int cid) { LadderWarsListDef::iterator i = LadderWarsList.find(cid); if (i != LadderWarsList.end()) return i->second->Team; return MMT_ALL; }
...
public:
	...
	int GetVIPObjCount()					{ return m_nVIPObjectCount; }

	unsigned long GetSilenceEndTime() { return m_nSilenceEndTime; }
	void SetSilenceEndTime(unsigned long nEndTime) { m_nSilenceEndTime = nEndTime; }


	void AddBanList(int nCID);
	void RemoveBanList(int nCID); // Custom: Unban kicked players
	...
	bool UnbanKickedPlayer(const char* pszName); // Custom: Unban kicked players
...
#ifdef _PORTALGUN 1
	void ObtainPortalItem(MMatchObject* pObj, const int nItemID);
#endif
...
#ifdef _BOXLEAD
	void SetIsLeading(bool bIsLeading)								{ m_StageSetting.SetLead(bIsLeading); }	
#endif

#ifdef _ROTATION
	void SetIsRoBase(bool bIsRobase) { m_StageSetting.SetRoBase(bIsRobase); }
#endif
...
public:
	...
	//Custom: access suicide list
	vector< MMatchStageSuicide >& GetSuicideList() {
		return m_SuicideList;
	}
