...
public:
	// ｰ霖､, ﾄｳｸｯﾅﾍ ｰ・ｮ ｰ・ﾃ
	bool GetLoginInfo(const TCHAR* szUserID, unsigned int* poutnAID, TCHAR* poutPassword/*, TCHAR* poutSalt*/); // Custom: Salt
	...
	int IsLadderWarsOpen();
	bool InsertBZGameLog(TCHAR* Winners, TCHAR* Losers, int WinningScore, int LosingScore, int MapID);
	...
	bool CreateAccount(const TCHAR* szUserID,					// ｻ鄙・ﾚID
		const TCHAR* szPassword,					// ﾆﾐｽｺｿ・
		const int nCert,							// ｽﾇｸ暲ﾎﾁ・
		const TCHAR* szName,						// ｻ鄙・ﾚﾀﾌｸｧ
		const int nAge,							// ｳｪﾀﾌ
		const int nSex);							// ｼｺｺｰ 0 - ｿｩﾀﾚ, 1 - ｳｲﾀﾚ

	bool CreateAccount_Netmarble(const TCHAR* szUserID,			// ｻ鄙・ﾚID
								const TCHAR* szPassword,		// ｻ鄙・ﾚ CN
		const int nAge,					// ｳｪﾀﾌ
		const int nSex,					// ｼｺｺｰ 0 - ｿｩﾀﾚ, 1 - ｳｲﾀﾚ
	...
	bool CreateCharacter(int* pnOutResult,						// ｰ皺嵓ｪ
		const int nAID,						// AID
		const TCHAR* szNewName,				// ﾄｳｸｯﾅﾍ ﾀﾌｸｧ
		const int nCharIndex,					// ｰ霖､ﾀﾇ ﾄｳｸｯ ｽｽｷﾔ ﾀﾎｵｦｽｺ
		const int nSex,						// ｼｺｺｰ
		const int nHair,						// ﾇ・・
		const int nFace,						// ｾｼ
		const int nCostume);					// ﾃﾊｱ・ﾄﾚｽｺﾆｬ
	...
	bool UpdateCharLC(const int nCID, const int nLCInc);
	bool UpdateCharInfoData(const int nCID, const int nAddedXP, const int nAddedBP, const int nAddedLC,
		const int nAddedKillCount, const int nAddedDeathCount, const int nAddedPlayTime);
	// ｾﾆﾀﾌﾅﾛ ｰ・ﾃ	
	bool GetCharItemInfo(MMatchCharInfo* pCharInfo);	// ﾄｳｸｯﾅﾍﾀﾇ ｸ・ｾﾆﾀﾌﾅﾛﾀｻ ｰ｡ﾁｮｿﾂｴﾙ
	...
	bool SetEquipPreset(const unsigned long nCID, MMatchCharItemParts parts);
	...
	bool InsertQuestChallengeGameLog(const char* pszStageName,
	...
	// Custom: GetStageBestTime
	bool GetStageBestTime(const char* szScenarioName, MMatchStageSetting* pOutStageSetting);
	...
	bool InsertGameLog(const unsigned int nMasterCID, const char* szMap, const char* szGameType, const char* szGameName, unsigned int* poutID);
	...
	bool CreateClan(const TCHAR* szClanName,
	...
	{
		ER_OK =				1,		// ｼｺｰ・
		ER_NO_MEMBER =		0,		// ﾇﾘｴ・ﾅｬｷ｣ｿﾌ ｾﾙ
		ER_WRONG_GRADE =	2		// ｱﾇﾇﾑﾀﾌ ｸﾂﾁ・ｾﾊｴﾙ.
	};
	...
	bool GetLadderWarsCharInfo(DWORD dwCID, LadderWarsCharInfo* pLadderWarCharInfo);
	bool UpdateLadderWarsCharInfo(DWORD dwCID, int Win, int Loss, int Draw, int Score, int Mode);
	bool GetLadderWarsSideRankingInfo(DWORD dwCID, list<PWRankingInfo*>* pRankingList);

	// Duel Tournament ｰ・ﾃ - Added By ﾈｫｱ簔ﾖ(2009-09-25)
	...
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
	// -- AccountItem ｰ・ﾃ - Added and Modified By ﾈｫｱ簔ﾖ

	// ﾃ｢ｰ・ｾﾆﾀﾌﾅﾛﾀｻ ｰ｡ﾁｮｿﾂｴﾙ	
	...
	// ﾁﾟｾﾓﾀｺﾇ狒｡ｼｭ ｳｻ ﾄｳｽｬ ｾﾆﾀﾌﾅﾛ ｰ｡ﾁｮｿﾀｱ・
	...
	bool DeleteExpiredAccountItem(const int nAID, const int nAIID);											// ｱ箍｣ ｸｸｷ盞ﾈ ｾﾆﾀﾌﾅﾛ ﾁ・・
	...
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
	// -- CharacterItem ｰ・ﾃ - Added and Modified By ﾈｫｱ簔ﾖ
	...
	bool UpdateCharItemCountInfo(const int nCIID, const int nItemCount);	//< ｾﾆﾀﾌﾅﾛ ｰｳｼ・ｺｯｰ・

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
	// -- zItem.xmlｰ・ｵ･ﾀﾌﾅﾍｺ｣ﾀﾌｽｺ ｺｳ ｰ・ﾃ - Added By ﾈｫｱ簔ﾖ
	bool GetItemTable(map<int, MMatchItemDescForDatabase*>& ItemMapFromDatabase);

	// 2010. 08. 10 - Added By ﾈｫｱ簔ﾖ
	// ｿ錞ｵﾀﾚ ﾆﾐｳﾎﾆｼ ｰ・ﾃｵﾈ ﾇﾔｼ・
	...
	bool AdminGiveCoinsCash(const int nAID, const int nCash);
	bool AdminGiveCoinsMedals(const int nCID, const int nLC);
	...
	bool UpdateLadderWars();
	bool UpdateSurvivalRanking();
	bool UpdateGunzSystem();
	//bool UpdateCharSystem();

#ifdef _GRADECHANGE
	bool GetChangeGrade(const int nAID, const int nUGradeID);
#endif

	//Banderas
#ifdef _COUNTRYFLAG
	bool UpdateCountryCode(const int nAID, const int nCountryCode);
#endif
#ifdef _DEBUG
	bool test();
#endif	
#ifdef _HWID
	bool UpdateHwid(int nAID, char* szHwid);
	bool IsBannedHwid(char* szHwid);
	bool BanHwid(int nAID, char* szHwid, int nAutoReason);
	bool GetWarnings(int nAID, int* poutWarnings);
	bool BanAccount(const int nAID, bool bAuto);
#endif
};
#endif