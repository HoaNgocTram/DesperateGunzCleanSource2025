...
// m_uidPlayer와 기타 uid들은 위조 커맨드를 생성하기 위해 알아내야 하는 값이다.
	...
	char				m_szAgentIP[32];
   	char				m_szLocatorIP[32];	// Custom: locator IP address.
...
public:
	...
	// Custom: set locator IP address.
	void SetLocatorAddr(const char* szIP) {
		strcpy(m_szLocatorIP, szIP);
	}
	char* GetLocatorIP() { return m_szLocatorIP; }
...
enum MMatchCountryCode
{
	//Country codes: https://msdn.microsoft.com/en-us/library/windows/desktop/dd374073(v=vs.85).aspx
