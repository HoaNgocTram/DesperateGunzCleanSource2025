...
MPOINT MEvent::LatestPos = MPOINT(0, 0);	// ¸¶Áö¸· À§Ä¡
...
int MEvent::TranslateEvent(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_MOUSEMOVE:
		...
		LatestPos = Pos;
		break;
	case WM_MOUSEWHEEL:	// ÈÙÀº Àý´ëÁÂÇ¥·Î ³¯¾Æ¿È
		{
			POINT pos;
			pos.x = LOWORD(lparam);
			pos.y = HIWORD(lparam);
	...
	case WM_INPUTLANGCHANGE:
		// ¿ø·¡´Â ¿©±â¿¡ Input Language ÀÌº¥Æ® º¯È­¸¦ ¹Þ¾Æ¾ß ÇÏÁö¸¸, Mint¿¡¼­´Â Æú¸µ ¹æ½ÄÀ¸·Î
		// GetKeyboardLayout()À» ÀÌ¿ëÇÏ¿© ÇÊ¿äÇÑ ¶§ ¾ò¾î¼­ »ç¿ëÇÑ´Ù.
		// Mint::IsNativeIME() Âü°í
		// 2005-08-18, ÀÌÀåÈ£
...
//			OutputDebugString("WM_IME_STARTCOMPOSITION");
		...
		return EVENT_PROCESSED;	// Ã³¸®°¡ µÇ¹Ç·Î
...
//			OutputDebugString("WM_IME_ENDCOMPOSITION");
	...
	case WM_IME_NOTIFY:
		{
			Mint* pMint = Mint::GetInstance();
			if(bIMESupport==true && pMint->IsEnableIME()==true){	// IsEnableIME() Ã¼Å©´Â Ã¢ ¹Û¿¡¼­ Candidate List°¡ ¾È³ª¿À°Ô ÇÏ±â À§ÇØ
				/*
				...
				*/
				if(wparam==IMN_OPENCANDIDATE || wparam==IMN_CHANGECANDIDATE){
					// IMN_CHANGECANDIDATE ¶ÇÇÑ Open°ú ¸¶Âù°¡Áö·Î CandidateList¸¦ ¾ò¾î¾ß ÇÔ
