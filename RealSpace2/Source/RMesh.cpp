...
void RMesh::Init()
{
	...
	//m_data.resize(MAX_MESH_NODE_TABLE); // FIX
...
void RMesh::TrimStr(const char* szSrcStr, char* outStr)
{
	...
	for (int i = 0; i < nSrcStrLen; i++)
	{
		...
		{
			//strcpy(szInputMapName, &szSrcStr[i]);	// 에러를 찾기위해 공백 제거하지 않는다.
			break;
		}
		else
		{
...
void RMesh::SubCalcBBox(struct D3DXVECTOR3* max, struct D3DXVECTOR3* min, struct D3DXVECTOR3* v)
{
