...
#ifdef _SWORDCOLOR 1
void RVisualMesh::GetEnChantColor(u32* color) //Custom: Sword Trail Colors
#else
...
#endif
{
	...
	else
	{
		...
		color[1] = 0x0fFFFF00;
#ifndef _TRAILCOLOR
		color[0] = g_dwColorValue[0];
		color[1] = g_dwColorValue[1];
#endif
	}
}
...
void RVisualMesh::DrawTracks(bool draw, RVisualMesh* pVWMesh, int mode, rmatrix& m)
{
	...
	if (draw && m_bDrawTracksMotion[mode]) {
		...
		// Ä® Á¾·ù¿¡ µû¶ó¼­ ±æÀÌ°¡ Æ²¸®µµ·Ï Àû¿EØº¸±E.
		// ÀÌµµ·E, Ä«Å¸³ª , ´EË.. ÀÌµµ·ùÀÇ ÈÖ¾ûÝEÄ®ÀÌ ¹®Á¦..
		...
		vpos[0].y = vmax.y;
		vpos[1].y = vmin.y + 35.f; // ¼ÕÀâÀÌ ¸¸Å­ •û±E.
		...
		if (g_nTrackSize != m_nTrackSize) {
			...
			m_nTrackSize = g_nTrackSize;
		}

		if (!m_pTracks[mode]) {
			m_pTracks[mode] = new RWeaponTracks;
			m_pTracks[mode]->Create(g_nTrackSize);
		}
		// Sword tracks. End.
