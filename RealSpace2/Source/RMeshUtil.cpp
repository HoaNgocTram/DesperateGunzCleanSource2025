...
RIndexBuffer::RIndexBuffer()
{
	...
	if (!g_isDirect3D9ExEnabled)
		...
		m_dwPool = D3DPOOL_DEFAULT;
	m_dwLockFlag = 0;

	// ¿ë·®ÀÌ ¾ó¸¶ ¾ÈÇÏ´Ï... ¾ÆÁÖ Àú»ç¾çÀÌ ¾Æ´Ï¶ó¸é µÑ´Ù»ý¼º...
...
bool RIndexBuffer::Create(int size, WORD* pData, DWORD flag, DWORD Usage, D3DPOOL Pool)
{
	...
	if (m_bUseHWVertex) {
		if (FAILED(RGetDevice()->CreateIndexBuffer(sizeof(WORD) * size, Usage, D3DFMT_INDEX16, g_isDirect3D9ExEnabled ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED, &m_ib, 0))) {
			//		return false; 
			//		soft ·Î µ¹¼öµµ ÀÖ´Ù..
...
void RVertexBuffer::Init() {
	...
	m_nRenderCnt = 0;
	m_pVert = NULL;
	m_bUseSWVertex = false;//±âº»Àº µÑ´Ù»ç¿ë,. ¿É¼Ç¿¡¼­ ¹Ù²Ü¼öµµ ÀÖÀ¸¹Ç·Î..
	m_bUseHWVertex = false;
}
...
bool RVertexBuffer::Create(char* pVertex,DWORD fvf,int VertexSize,int VertexCnt,DWORD flag,DWORD Usage,D3DPOOL Pool) 
{
	...
	// ¹öÆÛ´Â ÇÊ¿ä¿¡ ÀÇÇØ¼­ Ã¤¿ì°í

	if (m_bUseHWVertex) {
		if (FAILED(RGetDevice()->CreateVertexBuffer(m_nBufferSize, Usage, fvf, g_isDirect3D9ExEnabled ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED, &m_vb, 0))) {
			//			return false; // tnt µîÀÇ °æ¿ì soft ·Î µ¹¼öµµ ÀÖ´Ù..
		}
	}
...
bool RVertexBuffer::Update(char* pVertex,DWORD fvf,int VertexSize,int VertexCnt) 
{
	...
	if(m_nBufferSize != BufferSize) {
		if(m_nRealBufferSize < BufferSize) {// ÁøÂ¥ ¹öÆÛº¸´Ù ÀÛ´Ù¸é ±×³É ¾²ÀÚ..
			if(Create(pVertex,fvf,VertexSize,VertexCnt,m_dwUsage,m_dwPool)==false)
				return false;
		}
		else 
		{
			m_nBufferSize = BufferSize; // m_nRealBufferSize ´Â µû·Î´Ù..
			m_nVertexCnt = VertexCnt;
		}
	}
...
// soft ¹öÆÛ¸¸ Áö¿øÇÑ´Ù..  Vertex Type ÀÎ°æ¿ì..lvert blendvert
// ¿ì¼±Å×½ºÆ®ÇØº»´Ù..±¦ÂúÀ¸¸é ¹öÆÛ¸¦ ¸¸µé´ø°¡ ÇÑ´Ù..
void RVertexBuffer::ConvertSilhouetteBuffer(float fLineWidth)
{
	if(!m_pVert) return;

	// normal ÀÌ ¾øÀ¸¸é ÀÇ¹Ì ¾ø´Ù..
...
{
	if (!ib) return;
	if (IsBadReadPtr(ib, sizeof(RIndexBuffer))) return;
	if (!ib->m_ib) return;
	if (ib->m_size <= 0) return;

	LPDIRECT3DDEVICE9 dev = RGetDevice();
	if (!dev) return;

	// Kiểm tra device lost
	HRESULT hrTest = dev->TestCooperativeLevel();
	if (hrTest == D3DERR_DEVICELOST || hrTest == D3DERR_DEVICENOTRESET) {
		OutputDebugStringA("[RenderIndexBuffer] Device lost - skipping draw\n");
		return;
	}

	int faceCount = ib->GetFaceCnt();

	// Kiểm tra dữ liệu hợp lệ
	if (faceCount <= 0 || faceCount * 3 > m_nVertexCnt) {
		char buf[256];
		sprintf_s(buf, "[RenderIndexBuffer] Skipped invalid draw: faces=%d, vtx=%d, ib_size=%d\n",
			faceCount, m_nVertexCnt, ib->m_size);
		OutputDebugStringA(buf);
		return;
	}

	// Gán index buffer
	hrTest = dev->SetIndices(ib->m_ib);
	if (FAILED(hrTest)) {
		OutputDebugStringA("[RenderIndexBuffer] SetIndices failed\n");
		return;
	}

	// Gọi draw với kiểm tra an toàn
	HRESULT hr;
	__try {
		hr = dev->DrawIndexedPrimitive(m_PrimitiveType, 0, 0, m_nVertexCnt, 0, faceCount);
		if (FAILED(hr)) {
			char buf[128];
			sprintf_s(buf, "[RenderIndexBuffer] DrawIndexedPrimitive failed (0x%08X)\n", hr);
			OutputDebugStringA(buf);
		}
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		OutputDebugStringA("[RenderIndexBuffer] Invalid buffer - skipped draw\n");
	}
}
...
//  D3DRS_MULTISAMPLEANTIALIAS ¾ÈÆ¼¾Ë¸®¾î½º.. Á¡À» ÂïÀ»¶§ ÁÖº¯Á¡°úÀÇ °ü°è¸¦ »ý°¢ÇØ¼­ ÇÈ¼¿ÀÇ »öÀ» º¯°æÇÏ´Â°Í.. °£´ÜÇÔ..	Å¸°Ù¹öÆÛ¿¡ ÇÃ·¹±× ¼³Á¤ÇÔ,../

//	D3DRS_COLORVERTEX
//	D3DRS_DIFFUSEMATERIALSOURCE
//	D3DRS_COLORWRITEENABLE		   D3DCAPS9 ±¸Á¶Ã¼ÀÇ PrimitiveMiscCaps ¸â¹ö·Î D3DPMISCCAPS_COLORWRITEENABLE ´É·Â ºñÆ®°¡ ¼³Á¤µÇ¾î ÀÖ´Â °æ¿ì¿¡ ÀÌ¿ëÇÒ ¼ö ÀÖ´Ù. ÀÌ ·»´õ¸µ ½ºÅ×ÀÌÆ®´Â, Å¬¸®¾î Ã³¸®¿¡´Â ¿µÇâÀ» ÁÖÁö ¾Ê´Â´Ù
...
void draw_box(rmatrix* wmat , rvector& max,rvector& min,DWORD color)
{
	...
	rvector t_vec_d[24];
//	int		t_index[100];	//ÀÎµ¦½º ¹æ½ÄÀ¸·Î ³ªÁß¿¡ °³·®
	rvector t_vec[8];
