...
//		pMeshNode->ConnectToNameID();//이름을 ID 로 바꾼다..
		...
		else if (NCMPNAME("eq_wd_spycase", 13)) {
			pMeshNode->m_isWeaponMesh = true;
			pMeshNode->m_PartsType = eq_parts_right_katana;
		}
	}
...
#define MZF_READ_XOR(x,y) {if(!mzf->Read((x),(y))) return false;}
	...
	if(count_mtrl)
	{
		for(i=0;i<count_mtrl;i++) {
			...
			// todo: check with secrets
			/*
			int twoside=0;
			MZF_READ_XOR(&twoside,sizeof(int) );
			node->m_bTwoSided = twoside ? true : false;

			int additive = 0;
			MZF_READ_XOR(&additive,sizeof(int) );
			...
			*/
			...
			//node->m_bTwoSided = twoside ? false : true;//true : false;
			char additive = 0;
			MZF_READ_XOR(&additive,sizeof(char) );
			node->m_bAdditive = (additive < 0) ? false : true;
			//node->m_bAdditive = additive ? false : true;//true : false;
			int alpha_test = 0;
			MZF_READ_XOR(&alpha_test,sizeof(int) );
			node->m_nAlphaTestValue = alpha_test;
			...
			//node->m_bDiffuseMap = diffuse ? false : true;//true : false;
			char alpha_map = 0;
			MZF_READ_XOR(&alpha_map,sizeof(char) );
			node->m_bAlphaMap = (alpha_map < 0) ? false : true;
			//node->m_bAlphaMap = alpha_map ? false : true;//true : false;
