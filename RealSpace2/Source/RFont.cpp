...
// ¸ðµç rfont ´Â g_FontTexture ÇÏ³ª¸¸À» »ç¿ëÇÑ´Ù. ( 16x16 ¼¿·Î ÀÌ·ç¾îÁø ÇÏ³ªÀÇ ÅØ½ºÃÄ¸¦ ¹öÆÛ·Î ¾´´Ù )

#define TEXTURE_SIZE	512
//#define CELL_SIZE		16	//Å« ÆùÆ®¸¦ ¼ÒÈ­ÇÏ±â À§ÇØ¼­ ÀÌÁ¦´Â 16À¸·Î °íÁ¤ÇÒ ¼ö ¾ø´Ù. °¡º¯ÀûÀ¸·Î ¹Ù²î¾î¾ßÇÒ °ÍÀÌ´Ù.
...
bool RFontTexture::Create()
{
	...
	if (g_isDirect3D9ExEnabled) {
		hr = RGetDevice()->CreateTexture(m_nWidth, m_nHeight, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pTexture, NULL);
	}
	else {
		hr = (LPDIRECT3DDEVICE9(RGetDevice())->CreateTexture(m_nWidth, m_nHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &m_pTexture, NULL));
	}
...
void BlitRect(BYTE* pDest, int x1,int y1,int x2,int y2,int w, int h, DWORD* pBitmapBits, INT Pitch)
{
	...
	for(int y=y1; y<y2; y++) {
		...
		for(int x=x1; x<x2; x++) {
			...
			if ( dwPixel & 0x00ffffff)	//»öÀÌ °ªÀÌ ÀÖ´Ù¸é~
				dwPixel |= 0xff000000;
...
bool RFontTexture::UploadTexture(RCHARINFO *pCharInfo,DWORD* pBitmapBits,int w,int h)
{
	...
	if(hr == D3D_OK)
	{
		...
		// ¸Ç ¾Õ¿¡ ÀÖ´Â ³ÑÀÌ °¡Àå ¿À·¡Àü¿¡ »ç¿ëµÈ ³ÑÀÌ´Ù.
...
bool RFontTexture::IsNeedUpdate(int nIndex, int nID)		// °»½ÅµÇ¾î¾ß ÇÏ´ÂÁö °Ë»ç
{
...
bool RFontTexture::MakeFontBitmap(HFONT hFont, RCHARINFO *pInfo, const TCHAR* szText, int nOutlineStyle, DWORD nColorArg1, DWORD nColorArg2)
{
	...
	else if (nOutlineStyle == 1)	// ¾Æ¿ô¶óÀÎÀÌ ±×·ÁÁø´Ù
	{
		GraphicsPath path;
	...
	else if (nOutlineStyle == 2)	// ±×¸²ÀÚ¸¸ Æ÷ÇÔµÈ´Ù. ÁÂÃøÀÇ ÇÑÇÈ¼¿ ¿©¹éÀÌ ÀÖÀ¸¹Ç·Î ¿ÞÂÊÀ¸·Î ÇÑÇÈ¼¿. ÀÚ°£Àº °°´Ù
	{
...
bool RFontTexture::MakeFontBitmap(HFONT hFont, RCHARINFO * pInfo, const wchar_t* szText, int nOutlineStyle, u32 nColorArg1, u32 nColorArg2)
{
	HFONT hPrevFont = (HFONT)SelectObject(m_hDC, hFont);
	...
	int nWidth = min(int(size.cx), m_nCellSize);
...
int RFontTexture::GetCharWidth(HFONT hFont, const TCHAR* szChar)
{
	SIZE size;
	HFONT hPrevFont = (HFONT)SelectObject(m_hDC, hFont);
	GetTextExtentPoint32A(m_hDC, szChar, _tcslen(szChar), &size);
	SelectObject(m_hDC, hPrevFont);
	return size.cx;
}
...
static	FONT2DVERTEX	g_FontVertexBuffer[4*MAX_FONT_BUFFER];//±×¸²ÀÚ±îÁö 200ÀÚÁ¤µµ
...
//	m_nSamplingMultiplier = nSamplingMultiplier;	// ÇÑ¹æÇâ »ùÇÃ¸µ È½¼ö, 2ÀÌ¸é 2*2Å©±â¸¦ »ùÇÃ¸µÇÑ´Ù.
	...
	m_hFont = CreateFont(-nHeight, 0, 0, 0, bBold==true?FW_BOLD:FW_NORMAL, bItalic==true?TRUE:FALSE, 
		FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, /*DEFAULT_QUALITY*/ DRAFT_QUALITY, DEFAULT_PITCH, szFontName);
	...
	while (true)
	{
		...
		if (pInfo != NULL) {
			...
			if (m_nOutlineStyle == 1)
				x += min(int(pInfo->nWidth*fScale + 1), CELL_SIZE);
			else
...
//
////	bool bPrevInFont = m_bInFont;	// ÇÏ´Ü¿¡¼­ ºÎÀûÀýÇÏ°Ô »ç¿ëÁßÀÌ´Ù.
//	bool bPrevInFont = false;	
//
//	BeginFont();
//
...
//	}
//
//	// ÀÌÀü¿¡ BeginFont - EndFont »óÅÂ°¡ ¾Æ´Ï¾ú´Ù¸é º¹±¸ÇÑ´Ù.
//	if(!bPrevInFont)
//		EndFont();
//}
...
void RFont::DrawTextWSV(float x, float y, const StringView& Text, u32 Color, float Scale) {
	return DrawTextImpl(x, y, Text, Color, Scale);
}
void RFont::DrawTextWSV(float x, float y, const WStringView& Text, u32 Color, float Scale) {
	return DrawTextImpl(x, y, Text, Color, Scale);
}
