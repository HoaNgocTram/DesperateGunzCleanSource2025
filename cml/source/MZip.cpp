...
void DesperateEncrypt(char* pData, int _size)
{
#ifdef _THEMIDACML
	VM_START
#endif
	...
	{
		b = *pData;
		b = b ^ ZKEY[i % (sizeof(ZKEY) / sizeof(char))];
		*pData = b;
		pData++;
	}
#ifdef _THEMIDACML
...
void DesperateEncrypt(char* pData, int _size)
{
	...
	{
		b = *pData;
		b = b ^ Keyz[i % (sizeof(Keyz) / sizeof(char))];
		*pData = b;
		pData++;
	}
#ifdef _THEMIDACML
...
bool MZip::Initialize(FILE* fp, unsigned long ReadMode)
{
...
#ifdef _ENCRYPTXOR
		DesperateEncrypt((char*)&dh, sizeof(MZIPDIRHEADER));
#else
	...
	if (m_nZipMode >= ZMode_Mrs2)
#ifdef _ENCRYPTXOR
		DesperateEncrypt((char*)m_pDirData, dh.dirSize);
#else
...
bool MZip::ReadFile(int i, void* pBuffer, int nMaxSize)
{
	...
	if (m_nZipMode >= ZMode_Mrs2)
#ifdef _ENCRYPTXOR
		DesperateEncrypt((char*)&h, sizeof(h));
#else
...
static char _fileheaderReader[1024 * 16];
static int	_fileheaderReaderSize = 0;

bool MZip::UpgradeMrs(char* mrs_name)
{
...
#ifdef _ENCRYPTXOR
		DesperateEncrypt((char*)&h, sizeof(h));
		DesperateEncrypt(_fileheaderReader, _fileheaderReaderSize);
#else
		RecoveryChar((char*)&h, sizeof(h));
...
#ifdef _ENCRYPTXOR
	DesperateEncrypt((char*)pDirData, dir_data_size);
#else
	RecoveryChar((char*)pDirData, dir_data_size);
#endif
	fseek(fp, dir_data_pos, SEEK_SET);
...
bool MZip::RecoveryMrs2(FILE* fp)
{
	...
	fread(&dh, sizeof(dh), 1, fp);
#ifdef _ENCRYPTXOR
	DesperateEncrypt((char*)&dh, sizeof(dh));
#else
...
void FFileList::ConvertNameMRes2Zip()
{
	...
	for (node = begin(); node != end(); ++node) {
		...
		strcat(_buf_rename, _EXTFILEMRS_ZIP);
...
void FFileList::ConvertNameZip2MRes()
{
	...
	for (node = begin(); node != end(); ++node) {
		...
		_buf_rename[len - 3] = NULL;
		strcat(_buf_rename, _EXTFILEMRS);
