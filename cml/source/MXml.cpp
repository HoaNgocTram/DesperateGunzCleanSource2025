...
void MXmlElement::GetAttribute(int index, char* szoutAttrName, char* szoutAttrValue)
{
	MXmlDomNamedNodeMapPtr pAttributes = m_pDomNode->attributes;
...
bool MXmlElement::GetChildContents(unsigned long* iOutValue, const char* sChildTagName)
{
	char szBuf[256];
	if (GetChildContents(szBuf, sChildTagName))
	{
...
void MXmlElement::GetContents(unsigned int* ipOutValue)
{
	char sTemp[256];
	memset(sTemp, 0, 256);
...
bool MXmlDocument::LoadFromFile(const char* filename)
{
    ...
    // Bắt buộc gọi bản UTF-8, không dùng LANG_KOREAN
...
bool MXmlDocument::LoadFromMemory(const char* szBuffer, LANGID)
{
	...
	//--- tiny helper: check if valid UTF8 ---
	auto IsUTF8 = [](const char* s)
		{
	...
	//--- remove BOM optional ---
	if ((unsigned char)szBuffer[0] == 0xEF &&
		(unsigned char)szBuffer[1] == 0xBB &&
		(unsigned char)szBuffer[2] == 0xBF)
	{
		szBuffer += 3;
	}
	...
	{
		// fallback: ANSI → UTF16
		int wlen = MultiByteToWideChar(CP_ACP, 0, szBuffer, -1, NULL, 0);
		if (wlen <= 0) return false;

		wxml.resize(wlen);
		MultiByteToWideChar(CP_ACP, 0, szBuffer, -1, &wxml[0], wlen);
	}
